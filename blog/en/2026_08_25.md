...
- Minor fixes: duplicate tiles in Quick Access when multiple `web` instances are installed (v8.0.5), save button and validation in Custom Settings (v8.0.2–8.0.4), folder selection in the Alias dialog.
...
For writing, there are `set_state` and `set_states`. The latter sets multiple values in a single call, i.e., "all lights off" as a single action. Creating objects, generating scenes, and writing files are also possible, but disabled by default.
...
Integration and self-consumption control for **SunEnergyXT-500 hybrid storage** via the local HTTP API – without manufacturer cloud.
...
- Alternatively, the device's own control can be used, but then only with selected meters.
...
- States for limit value, dimming duration and the legally limited daily dimming time to two hours
...
- Switch pump on and off, set speed
...
- The write commands have been fixed. Originally, the API rejected commands for start, parking, cutting height, time, and headlights due to a faulty request body.
...
- Consumable runtimes for charging contacts, cameras and knives with reset buttons
...
Turns any **existing switching object** into a time-controlled automatic feeder – socket, relay or GPIO, as long as it can be switched.
...
- Control of power, volume, mute, channel, source, app launch and any keys – `control.key` understands both `KEY_*` codes and short forms like `up` or `volup`
...
- Buttons per camera for recording, snapshot, motion detection, arming and PTZ, plus system-wide profiles for present, absent and night.
...
- Write the question to a data point, the answer comes back as plain text, along with the recognized intent.
...
Controls professional **iiyama ProLite displays** of the LH series via RS232 or network – interesting for info screens and wall panels.
...
- Considerate: If the web interface logs in with the same user, the adapter pauses instead of reclaiming the session.
...
**Category:** Energy / Solar **Developer:** IcedTea82
...
- **Trip tracking** with daily mileage and individual trip history, consumption statistics and charging cost estimate
...
- The optional LiveView web grid requires the JavaScript adapter, installed `ffmpeg`, and a Linux host.
...
#### Scope of functions:
- Race calendar with next race and countdown in days and hours, as well as the complete season as a JSON file
...
#### Special features:
- Requires a cloud account for the Dreame or MOVA app
...
#### Features:
HTTP and HTTPS servers run in parallel; the protocol is decided for each backend – with a certificate, HTTPS plus redirect; without a certificate, pure HTTP.
...
- PSK devices (typical IoT hardware) are not recognized
...
Those who have waited to use it can now do so without hesitation.
