...
The height of summer 2026 was anything but a slow period! Since the last review, a whopping **23 new adapters** have been added to the repository – more than in a long time. The focus is once again on **energy and e-mobility** (Hoymiles, Victron GX, Solakon ONE, Tesla Wall Connector, go-e Charger), but **cameras, multimedia, and system tools** are also well represented.

And there are a few real rarities: Formula 1 live data, flight tracking from your own window, and a sauna control system with a learning heating prediction feature.
...
**Category:** Energy / Solar **Developer:** sadam6752-tech

The adapter sends **Telegram messages about the charge status of a PV battery storage system** and provides daily, weekly and monthly statistics.

#### Features:
- Notifications at full (100%) and empty (0%) battery levels, as well as at freely configurable intermediate levels, each in % and kWh.
- **Night mode** (default 11pm–6am) and **Sleep mode** (default 12pm–3pm)
- Daily statistics at a fixed time or optionally at sunset, plus weekly and monthly statistics
- Custom statistics: Full cycles per day/week, maximum/minimum SoC of the day, current energy content
- Minimum intervals per message type to prevent message spam
- Optional weather integration for action recommendations such as "charge your electric car now"

#### Special features:
- Requires the **telegram** and **sourceanalytix** adapters
- All source data points are manually assigned in the configuration.
...
**Category:** Multimedia / Gaming **Developer:** Lucky-ESA

This adapter controls **PlayStation 4 and 5** on the local network and reads extensive account and community data via the PSN login.

#### Features:
- **Wake and Standby** for PS4 and PS5; keyboard input, button presses and app/game launch currently only for PS4
- PSN data: Friends list, online status, blocked members, requests, subscriptions and profile
- Trophies, game and playlists, and game and member search
- Groups and messages, including sending text messages
- Directly accept or decline friend requests
- Since version 0.0.4, **PlayStation Stars** and **PlayStation Store** have also been added.

#### Special features:
- So far only tested with a PS4, PS5 storage information is in progress.
- Requires a PSN login

🔗 [GitHub page of the adapter](https://github.com/Lucky-ESA/ioBroker.playstation)
...
- **Map rendering** as Base64 PNG with zones, paths and restricted areas; updated every 30 seconds during mowing.
...
Additionally, **ioBroker.motioneye** (inventwo) has reached version 1.3.1 after the release of `stable`.

Users who have been running the camera connection from the Latest repository can now switch to the stable branch.
...
23 new adapters in just over six weeks – that's quite a statement, even by ioBroker standards.
