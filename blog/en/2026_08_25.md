...
The height of summer 2026 was anything but a slow season, and this edition reflects that: we've compiled **72 adapters** for you.

**22** of these are new to the `stable` repository and ready to use immediately, while the remaining **50** are located in `latest` and are waiting for testers.
...
And there are a few real rarities included: Formula 1 live data, flight tracking from your window, and a sauna control system with a learning heating prediction feature.
...
**Category:** Voice Assistant **Developer:** Just Bad Luck
...
- Monitoring whether the switch was actually activated, via Telegram message
...
##### Features:
- Login using the manufacturer's app access data, brand selection between Electrolux and AEG
...
- **Hail Alarm Group Commands**: close, open or stop all blinds, bundled into a single command per device
