...
function structuredData(
    ...
    if (route === '/') {
        ...
        const adapter = entry as (JsonPage & { latestVersion?: string }) | undefined;
        blocks.push({
            '@context': 'https://schema.org',
    ...
    return blocks
        .map(
            block =>
                `<script data-structured-data type="application/ld+json">${JSON.stringify(block).replace(/</g, '\\u003c')}</script>`,
        )
        .join('\n        ');
}
 ...
 */
const NOT_FOUND_TITLE: Translated = {
    en: 'Page not found',
 ...
 */
function dayOfPage(source: string | undefined, entry: JsonPage | undefined): string | undefined {
    const written = source ? /^lastChanged:\s*"?(\d{2})\.(\d{2})\.(\d{4})"?/m.exec(source.slice(0, 600)) : null;
    if (written) {
        return `${written[3]}-${written[2]}-${written[1]}`;
    }
    const value = (entry as (JsonPage & { latestVersionDate?: string }) | undefined)?.latestVersionDate ?? entry?.date;
