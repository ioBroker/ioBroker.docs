...
import { LANGUAGES, documentTitle, escapeHtml, readJson, text, walk, withLanguage, } from './siteData.js';
const LANGUAGE_NAMES = { de: 'Deutsch', en: 'English', ru: 'Русский' };
...
function wordsFor(frontEndSrc, lang) {
    const load = (language) => frontEndSrc ? readJson(path.join(frontEndSrc, 'i18n', `${language}.json`)) : undefined;
    const own = load(lang);
...
function children(branch) {
    return Object.entries(branch ?? {}).filter((entry) => typeof entry[1] === 'object');
}
...
function homeContent(say, branch, link) {
    const heading = (prefix) => `${say(`${prefix}.title1`)} ${say(`${prefix}.title2`)}`.trim();
    const items = (key, tag) => list(tag, children(branch(key)).map(([, item]) => `<strong>${esc(str(item, 'title'))}</strong> ${esc(str(item, 'text'))}`));
    const explain = Object.entries(branch('home.demo.explain') ?? {})
        .filter((entry) => typeof entry[1] === 'string')
        .map(([editor, value]) => [say(`home.demo.editors.${editor}`), value]);
    const faq = children(branch('home.faq.items')).map(([, item]) => [
        str(item, 'q'),
        str(item, 'a'),
    ]);
    return [
        paragraphs(say('home.hero.subtitle')),
        section(heading('home.platform'), paragraphs(say('home.platform.p1')) +
        ...
        section(heading('home.benefits'), `${items('home.benefits.items', 'ul')}<p>${link('/docs/README.md', say('home.benefits.more'))}</p>`),
        section(heading('home.start'), `${paragraphs(say('home.start.lead'))}${items('home.start.steps', 'ol')}<p>${link('/installation', say('home.start.install'))} · ${link('/docs', say('home.start.docs'))}</p>`),
        section(heading('home.adapters'), `${paragraphs(say('home.adapters.text'))}<p>${link('/adapters', say('home.adapters.explore'))}</p>`),
        section(heading('home.community'), `${paragraphs(say('home.community.mainText'))}<p><a href="${FORUM}">${esc(say('home.community.forum'))}</a></p>`),
        section(say('home.faq.label'), terms(faq)),
    ].join('\n');
}
function installationContent(input, say, link) {
    const parts = [paragraphs(say('installation.subtitle'))];
    const targets = input.frontEndSrc
        ? readJson(path.join(input.frontEndSrc, 'config', 'installation.json'))
        : undefined;
    if (!targets) {
        return parts.join('');
    }
    const linuxDocument = documentTitle(input.publicDir, input.lang, targets.linuxInfo.replace(/^\/docs\//, '')) || 'Linux';
    parts.push(section('Linux', `${paragraphs(say('installation.linux.simplest'))}<pre><code>${esc(targets.command)}</code></pre>${paragraphs(say('installation.linux.hint'))}<h3>${esc(say('installation.linux.imageTitle'))}</h3>${paragraphs(say('installation.linux.imageText'))}<p>${link(targets.linuxInfo, linuxDocument)}</p>`));
    for (const platform of targets.platforms) {
        ...
        const actions = [
            ...
            .filter(Boolean)
            .join(' · ');
        parts.push(section(name, `${paragraphs(say(`${key}.detailsValue`)) +
            terms(platform.rows.map((row) => [say(`${key}.${row}`), say(`${key}.${row}Value`)])) +
            paragraphs(say(`${key}.passwordHint`))}<p>${actions}</p>`));
    }
    return parts.join('\n');
}
/** every adapter with its description, by category - the one page that links them all */
function adaptersContent(input, say, link) {
    const adapters = readJson(path.join(input.publicDir, 'adapters.json'));
    const sections = Object.entries(adapters?.pages ?? {}).map(([categoryKey, category]) => {
        ...
        walk(category, (key, page) => {
            if (page.content && !seen.has(key)) {
                ...
                items.push(`${link(`/adapters/${encodeURIComponent(key)}`, text(page.title, input.lang) || key)}${description ? ` – ${esc(description)}` : ''}`);
            }
        });
...
function docsTree(pages, lang, link) {
    return list('ul', Object.entries(pages ?? {}).map(([key, page]) => {
        const label = text(page.title, lang) || key;
        return ((page.content ? link(`/docs/${encodeURI(page.content)}`, label) : esc(label)) +
            docsTree(page.pages, lang, link));
    }));
}
function blogContent(input, say, link) {
    const posts = Object.entries(readJson(path.join(input.publicDir, 'blog.json'))?.pages ?? {}).sort(([, a], [, b]) => (b.date ?? '').localeCompare(a.date ?? ''));
    return (paragraphs(say('blog.subtitle')) +
        list('ul', posts.map(([id, post]) => {
...
function productContent(product) {
    ...
    const parts = [
        typeof features === 'object'
            ? list('ul', children(features).map(([, feature]) => `<strong>${esc(str(feature, 'title'))}</strong>${str(feature, 'text') ? ` – ${esc(str(feature, 'text'))}` : ''}`))
            : '',
    ...
    for (const [key, child] of children(product)) {
        if (key === 'setup') {
            ...
            parts.push(`<h3>${esc(str(child, 'title'))}</h3>${paragraphs(str(child, 'text'))}${list('ul', items)}${paragraphs(str(child, 'footer'))}`);
        }
    }
    return parts.join('');
}
 ...
 */
function licensesContent(say, branch) {
    const parts = [
        paragraphs(say('productOverview.welcome')),
        paragraphs(say('productOverview.intro')),
        list('ul', children(branch('productOverview.categories')).map(([, category]) => `<strong>${esc(str(category, 'title'))}</strong>: ${esc(str(category, 'text'))}`)),
    ];
    const support = branch('productOverview.support');
    if (support) {
        parts.push(section(str(support, 'lead'), paragraphs(str(support, 'text')) +
            paragraphs(str(support, 'accent')) +
            paragraphs(str(support, 'thanks'))));
    }
    for (const [, product] of children(branch('productOverview')).filter(([, child]) => typeof child.features === 'object')) {
        parts.push(section(str(product, 'title'), productContent(product)));
    }
    const faq = children(branch('productOverview.faq'))
        .map(([, group]) => `<h3>${esc(str(group, 'title'))}</h3>${terms(children(group).map(([, item]) => [str(item, 'q'), str(item, 'a')]))}`)
        .join('');
    parts.push(section(say('productOverview.faq.title'), faq));
    return parts.join('\n');
}
function statisticsContent(input, say, link) {
    const parts = [paragraphs(say('statistics.subtitle'))];
    const statistics = readJson(path.join(input.publicDir, 'data', 'statistics.json'));
    if (!statistics) {
        return parts.join('');
    }
    const format = new Intl.NumberFormat(input.lang === 'en' ? 'en-GB' : input.lang);
    const ranked = (map, count) => Object.entries(map ?? {})
        .map(([name, value]) => [name, Number(value)])
        .filter(([name, value]) => value > 0 && !NOT_RANKED.includes(name))
        .sort((a, b) => b[1] - a[1])
        .slice(0, count);
    const snapshot = statistics.date && !Number.isNaN(Date.parse(statistics.date))
    ...
    ]), section(say('statistics.adapters.title'), list('ol', ranked(statistics.adapters, 15).map(([name, count]) => `${link(`/adapters/${encodeURIComponent(name)}`, name)} – ${esc(format.format(count))}`))), section(say('statistics.countries.title'), list('ol', ranked(statistics.countries, 10).map(([name, count]) => `${esc(name)} – ${esc(format.format(count))}`))));
    return parts.join('\n');
}
/**
 ...
 */
export function crawlerBody(input) {
    const { route, lang, kind, title } = input;
    const { say, branch } = wordsFor(input.frontEndSrc, lang);
    const link = (target, label) => `<a href="${esc(withLanguage(target, lang))}">${esc(label)}</a>`;
    const menu = (key, fallback) => say(key) || fallback;
    const navigation = [
        link('/', 'ioBroker'),
    ...
    const languages = LANGUAGES.map(language => language === lang
        ...
        : `<a href="${esc(withLanguage(route, language))}" hreflang="${language}" lang="${language}">${LANGUAGE_NAMES[language]}</a>`).join(' · ');
    let heading = title;
    let content = '';
    let parent = null;
    if (kind === 'plain') {
        switch (route) {
            ...
            case '/adapters':
                ...
                break;
            case '/docs':
                content = docsTree(readJson(path.join(input.publicDir, 'content.json'))?.pages, lang, link);
                break;
            case '/blog':
...
//# sourceMappingURL=crawlerPages.js.map