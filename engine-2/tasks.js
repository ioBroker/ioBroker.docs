...
async function _1_blog() {
    await blog.build();
}

// download all adapters
async function _2_downloadAdapters() {
    await adapters.buildAdapterContent();
}

function _3_downloadJsonConfig() {
    return axios('https://raw.githubusercontent.com/ioBroker/ioBroker.admin/master/packages/jsonConfig/README.md').then(
        result => fs.writeFileSync(path.join(consts.SRC_DOC_DIR, 'en/dev/adapterjsonconfig.md'), result.data),
    );
}

function _4_downloadVisCordova() {
    return axios('https://raw.githubusercontent.com/ioBroker/ioBroker.vis.cordova/master/README.md').then(result => {
        fs.writeFileSync(
        ...
        return axios('https://raw.githubusercontent.com/ioBroker/ioBroker.vis.cordova/master/README.ru.md').then(
            result => {
                fs.writeFileSync(
                    path.join(consts.SRC_DOC_DIR, 'ru/viz/app.md'),
                    result.data.replace('# ioBroker.vis.cordova', '# vis App'),
                );
                return axios(
...
async function _5_syncDocs() {
    await new Promise(resolve => documentation.syncDocs(resolve));
}

// combine
async function _6_faq() {
    await faq.processFiles(consts.SRC_DOC_DIR);
}

// Build content.md file
async function _7_documentation() {
    // build content
...
async function _10_build() {
    ...
    if (fs.existsSync(SRC_DATA_DIR)) {
        ...
        fs.readdirSync(SRC_DATA_DIR).forEach(name =>
        ...
        if (!fs.existsSync(`${__dirname}/front-end/node_modules`)) {
            ...
            child.on('exit', (_code, signal) => {
                ...
                child_.on('exit', (code, signal) => {
                    ...
                    fs.existsSync(TGT_DATA_DIR) &&
                        fs
                            .readdirSync(TGT_DATA_DIR)
                            .forEach(name =>
            ...
            child.on('exit', () => {
                ...
                if (fs.existsSync(TGT_DATA_DIR)) {
                    fs.readdirSync(TGT_DATA_DIR).forEach(name =>
                        fs.writeFileSync(`${SRC_DATA_DIR}/${name}`, fs.readFileSync(`${TGT_DATA_DIR}/${name}`)),
                    );
                }
...
if (process.argv.includes('--i18n=>flat')) {
    ...
    translateTask().then(() => console.log('Done'));
    ...
    adapters.buildAdapterContent(ADAPTER_NAME).then(content => {
        documentation.syncDocs(`iobroker.${ADAPTER_NAME}`, () =>
            Promise.all(consts.LANGUAGES.map(lang => adapters.copyAdapterToFrontEnd(lang, ADAPTER_NAME))).then(() =>
                console.log('Done'),
            ),
        );
    });
    ...
    return adapters.buildAdapterContent('shelly').then(content => {
    ...
    _1_blog().then(() => console.log('Done'));
} else if (process.argv.includes('--2.downloadAdapters')) {
    // download all adapters
    _2_downloadAdapters().then(content => console.log(JSON.stringify(content)));
} else if (process.argv.includes('--3.downloadJsonConfig')) {
    _3_downloadJsonConfig().then(content => console.log(JSON.stringify(content)));
} else if (process.argv.includes('--4.downloadVisCordova')) {
    _4_downloadVisCordova().then(content => console.log(JSON.stringify(content)));
} else if (process.argv.includes('--5.syncDocs')) {
    // translate all documents: adapters and docu
    _5_syncDocs().then(content => console.log(JSON.stringify(content)));
} else if (process.argv.includes('--6.faq')) {
    _6_faq().then(content => console.log(JSON.stringify(content)));
} else if (process.argv.includes('--7.documentation')) {
    // Build content.md file
    // build content
    _7_documentation().then(content => console.log(JSON.stringify(content)));
} else if (process.argv.includes('--8.copyFiles')) {
    // copy all docs/LN/adapterref/* => engine/front-end/public/LN/adapterref/*
    _8_copyFiles().then(content => console.log(JSON.stringify(content)));
} else if (process.argv.includes('--9.createSitemap')) {
    _9_createSitemap();
} else if (process.argv.includes('--10.build')) {
    _10_build().then(content => console.log(JSON.stringify(content)));
    ...
    _1_blog() // translate and copy blogs
        ...
        .then(() => _10_build()); // build react site
} else {
    _0_clean(); // clean dir
    _1_blog() // translate and copy blogs
        .then(() => _2_downloadAdapters()) // download all adapters and create adapter.json
        .then(() => _3_downloadJsonConfig()) // download app documentation
        .then(() => _4_downloadVisCordova()) // download app documentation
        .then(() => _5_syncDocs()) // translate documents and adapters
        .then(() => _6_faq()) // combine FAQ
        .then(() => _7_documentation()) // create content for documentation
        .then(() => _8_copyFiles()) // copy all adapters and docs to the public
        .then(() => _9_createSitemap()) // create site-map for google
        .then(() => _10_build()); // build react site
}
