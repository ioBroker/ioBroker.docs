 ...
 * Date: 2019-11-11
 *
 * This file is started directly as TypeScript (`node tasks.ts ...`), so node.js >= 22.19 is required,
 * because node strips the types on the fly.
 * It stays a CommonJS module, because the helpers in `build-lib/*.js` decide via `module.parent`,
 * whether they export their API or run as a standalone script.
 */
...
/** Language code of the UI translations */
type LanguageCode = 'en' | 'de' | 'ru' | 'pt' | 'nl' | 'fr' | 'it' | 'es' | 'pl' | 'uk' | 'zh-cn';

/** Words of one language: word => translated text */
type LanguageWords = Record<string, string>;

/** Words of all languages: word => language => translated text */
type MultiLanguageWords = Record<string, LanguageWords>;

interface BuildConsts {
    FRONT_END_DIR: string;
    SRC_DOC_DIR: string;
    SRC_BLOG_DIR: string;
    GITHUB_EDIT_ROOT: string;
    LANGUAGES: LanguageCode[];
}

interface BuildUtils {
    delDir(source: string): void;
}

interface BuildBlog {
    build(): Promise<unknown>;
    buildRSS(): Promise<unknown>;
}

interface BuildFaq {
    processFiles(root: string, lang?: LanguageCode): Promise<unknown>;
}

interface BuildAdapters {
    buildAdapterContent(adapter?: string, noDownload?: boolean): Promise<unknown>;
    copyAdapterToFrontEnd(lang: LanguageCode, adapter: string): Promise<unknown>;
    copyAllAdaptersToFrontEnd(): Promise<unknown>;
}

interface BuildDocumentation {
    processContent(filePath: string): Promise<unknown>;
    processFiles(root: string, lang?: LanguageCode, originalRoot?: string): Promise<unknown>;
    syncDocs(testDir?: string | (() => void), cb?: () => void): void;
}

interface BuildTranslation {
    translateText(fromLang: string, text: string, toLang: string): Promise<string>;
}

const documentation: BuildDocumentation = require('./build-lib/documentaion');
const faq: BuildFaq = require('./build-lib/faq');
const adapters: BuildAdapters = require('./build-lib/adapters');
const blog: BuildBlog = require('./build-lib/blog');
const consts: BuildConsts = require('./build-lib/consts');
const utils: BuildUtils = require('./build-lib/utils');
const translation: BuildTranslation = require('./build-lib/translation');
...
async function _4_downloadVisCordova(): Promise<void> {
    const resultEn = await axios('https://raw.githubusercontent.com/ioBroker/ioBroker.vis.cordova/master/README.md');
    fs.writeFileSync(
    ...
    const resultRu = await axios('https://raw.githubusercontent.com/ioBroker/ioBroker.vis.cordova/master/README.ru.md');
    fs.writeFileSync(
        path.join(consts.SRC_DOC_DIR, 'ru/viz/app.md'),
        resultRu.data.replace('# ioBroker.vis.cordova', '# vis App'),
    );

    const resultDe = await axios('https://raw.githubusercontent.com/ioBroker/ioBroker.vis.cordova/master/README.de.md');
    fs.writeFileSync(
...
async function _7_documentation(): Promise<void> {
    // build content
    await documentation.processContent(path.join(consts.SRC_DOC_DIR, 'content.md'));
}
...
function _10_build(): Promise<void> {
    ...
    const buildFrontEnd = (): Promise<void> =>
    ...
    if (!fs.existsSync(`${__dirname}/front-end/node_modules`)) {
        return new Promise<void>(resolve => {
            const child = exec('npm install -f', {
                stdio: [process.stdin, process.stdout, process.stderr],
                cwd: `${__dirname}/front-end`,
            });
            child.on('exit', () => buildFrontEnd().then(() => resolve()));
        });
    }
...
async function buildOnly(): Promise<void> {
    ...
    await _7_documentation(); // create content for documentation
