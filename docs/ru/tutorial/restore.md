...
В принципе, однако, вы можете сказать заранее одно: восстановление может быть выполнено за несколько минут, если оно выполнено правильно, и никому не нужно этого бояться.
...
Если вы не уверены, использовался ли Redis, а доступ к старой системе все еще есть, используйте команду `iobroker status`, чтобы вызвать необходимую информацию. «При использовании Redis вывод выглядит следующим образом:
...
Поскольку здесь не требуется никаких знаний о Linux и все это делается через веб-интерфейс Iobroker, вариант автоматического восстановления с использованием [Резервное копирование](https://github.com/simatec/ioBroker.backitup/blob/master/README.md) имеет место первым.
...
Резервное копирование также может выполнять восстановление с NAS, Dropbox или Google Drive, но локальный вариант имеет наименьшее количество проблем.
...
Вы можете восстановить базу данных Redis, базу данных Zigbee, базу данных mySql и данные своей истории, выполнив те же действия, что описаны выше.
...
Здесь также создается резервная копия, созданная в старой системе, и, при необходимости, резервная копия Redis, резервная копия zigbee и т. Д.
...
Теперь это сделано, и система переустановлена, и все настройки, сценарии, визуализации и т. Д. Восстановлены.
