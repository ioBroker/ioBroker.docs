...
Кроме того, ПЕРЕД запросом на форум проверьте эту страницу устранения неполадок и FAQ - ССЫЛКА - включена ли там тема. Если вы уже пробовали действия из этих документов, запишите их одновременно.
...
Уровень журнала экземпляра может быть установлен в Admin WebUI. Для этого активируйте экспертный режим в разделе «Экземпляры» и установите уровень журнала в одноименном столбце для экземпляра.
После изменения уровня журнала экземпляр автоматически перезапускается.

** Внимание: ** В зависимости от уровня журнала (особенно отладочного и глупого) файл журнала на диске может быть довольно большим. Обратите внимание на доступное место для хранения вещей.

## После обновлений операционной системы ioBroker больше не работает (проверьте версии узлов и что-то еще)
## Адаптер / ioBroker больше не запускается с ошибкой «Неверная версия собственного пакета узла. Мля»? (восстановление npm и т. д.)
