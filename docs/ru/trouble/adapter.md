...
Эта глава посвящена исключительно... **проблемы, специфичные для адаптера**Для решения общих системных проблем (ioBroker не запускается, блокировки базы данных, обновления Node.js) см.: [ioBroker больше не работает.](/docs/trouble/RunsNoMore.md)
...
**Важный:** После переключения с последней версии на стабильную, установленные бета-версии будут удалены. **нет** Произошло автоматическое понижение версии. Вам необходимо дождаться, пока стабильная версия "догонит" старую, или понизить версию вручную.

```
iobroker upgrade <adaptername>@<stable-version>
```

**Понимание различий между репозиториями:**
→ См. [Что такое репозиторий?](/docs/basics/repositories.md)
...
⚠️ **Уведомление:** Эта команда очищает весь кэш и должна использоваться только в случае реальных проблем с кэшем.
...
⚠️ **Важный:** `iobroker fix` Восстанавливает всю установку ioBroker, включая:

- Права доступа к файлам для всех каталогов
- Системные пользователи и группы
- Зависимости и связи
- Является **всегда** Это возможно и должно быть первым шагом в случае возникновения проблем.
...
→ Возможно, неисправна SD-карта! См. [Диагностика оборудования](/docs/trouble/RunsNoMore.md)
...
**Проблема:** Соединение с блоком управления прервано.\
**Решение:**
...
**Решение:** Видеть [Проблемы с резервным копированием в Docker](https://docs.buanet.de/de/iobroker-docker-image/docs/)

### MQTT

**Проблема:** Журналы событий переполнены сообщениями.\
**Решение:**
...
**Проблема:** Представления не загружаются; ошибка 404 для /vis-views/
**Решение:**
...
2. Репозиторий на **стабильный** набор (не самый новый!)
...
- Репозиторий на **стабильный** создание производственных систем
...
**В случае возникновения дальнейших проблем:** Создайте тему на форуме с подробным описанием. `iob diag`-Выходные и адаптерные журналы.