...
Если ioBroker перестал запускаться или стал недоступен, это обычно происходит по одной из следующих причин: **семь основных проблем** В этой подборке систематически рассматриваются все известные и повторяющиеся ошибки с помощью проверенных решений.
...
- Процессы ioBroker продолжаются после сбоя.
...
**Важный:** Базу данных объектов ВСЕГДА следует восстанавливать из резервной копии, а не удалять! Удалять можно только состояния, если это необходимо.

### 1.2 Проблемы с базой данных Redis

**Симптомы:**

- ioBroker работает крайне медленно.
- Административный интерфейс загружается за считанные минуты.
...
**Уведомление:** Оптимизация памяти в Redis работает лишь в ограниченной степени и сильно зависит от конфигурации.
...
- "Соединение отклонено" или ошибка тайм-аута
...
**Проблема:** Использование нескольких веб-адаптеров на одном порту или конфликты портов.
...
- `SyntaxError: Unexpected token` во время обновления js-контроллера (также может быть повреждена файловая система!)
- После обновления Node.js адаптеры перестали запускаться.
...
Подробную информацию об обновлении Node.js можно найти в \[ссылка на документацию]. [Руководство по обновлению Node.js](/docs/install/updatenode.md).
...
**Важный:** NPM автоматически и корректно устанавливается вместе с Node.js. Если у вас возникнут проблемы с NPM, НЕ пытайтесь установить его вручную!

Дополнительную информацию об установке Node.js и NPM можно найти в \[ссылка на документацию]. [Руководство по установке Node.js](/docs/install/nodejs.md).
...
**Ошибка "npm не найден" после обновления Node.js:**
...
**Важный:** НИКОГДА не пытайтесь вручную удалить или переустановить NPM! Это обычно приводит к дальнейшим проблемам. Используйте вместо этого. `iob nodejs-update`который NPM настраивает автоматически.
...
- Система зависает, когда доступно менее 20% свободной оперативной памяти.
...
- SSD вместо SD-карты для повышения производительности

### 4.2 Проблемы с жестким диском

**Симптомы:**

- `/` показывает 100% заполняемость
- Размер файлов журналов достигает гигабайтов.
...
- `getaddrinfo ENOTFOUND` Ошибка
- Адаптеры не могут подключаться к внешним сервисам.
- `iob fix` и `iob diag` Они не работают
...
- Соединения WebSocket не работают.
- Пути Socket.io перенаправлены некорректно.
...
- Ошибка резервного копирования, несмотря на `chmod 777`
- Адаптеры не могут записывать

**ВАЖНЫЙ:** Никогда `chmod 777` Используйте это! Это представляет собой угрозу безопасности и часто не решает проблему.
...
**Проблема:** MQTT заполняет журналы ненужными сообщениями.
...
**Новая установка находится по адресу:**

- Повреждена установка Node.js после некорректных обновлений.
- Масштабный ущерб системе, вызванный операциями с правами root.
...
1. Всегда первый `iob fix` выполнять
2. Перед выполнением важных операций создавайте резервные копии.
3. Никогда не удаляйте базу данных объектов без резервной копии.
4. `chmod 777` никогда не использовать
5. Обратите внимание: при использовании Redis команда FLUSHALL удалит все данные!
6. После изменения прав доступа: снова войдите в систему.
7. Проблемы NPM, связанные с `iob nodejs-update` Решите проблему, не экспериментируйте вручную.