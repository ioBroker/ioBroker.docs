...
**Адаптер ioBroker только для чтения для [Блуэтти](https://www.bluettipower.com) Электростанции — телеметрия от батарей, солнечных батарей, сети и нагрузки из облака BLUETTI.**
...
- 🔋 **Телеметрия батареи и питания** — уровень заряда, входная мощность фотоэлектрической системы, входная мощность сети, выходная мощность переменного/постоянного тока
...
- 🩺 **Состояния здоровья и взаимосвязи** — доступность, последовательные сбои и консервативный сигнал подозрения на отключение электроэнергии для автоматизированных систем ИБП.
...
- ioBroker с **js-controller ≥ 6.0.11** и **администратор ≥ 7.6.20**
...
> Адаптер пока отсутствует в репозитории ioBroker. После его принятия вы сможете установить его непосредственно из административного интерфейса ioBroker.**Адаптер** → поиск по запросу "bluetti".

1. Установите адаптер и создайте `bluetti.0` пример.
2. Откройте конфигурацию экземпляра в административной панели ioBroker.
3. Нажмите **Аутентификация с помощью BLUETTI** и завершите авторизацию в открывшемся окне браузера. Адаптер использует встроенные учетные данные клиента BLUETTI, поэтому поля идентификатора/секрета клиента не отображаются в административном интерфейсе.
4. Выберите своё устройство из списка **селектор устройств**.
5. **Сохранять.** Опрос запускается автоматически; `info.connection` повороты `true` как только первый опрос пройдет успешно.
...
Этот адаптер считывает данные с **облако Блуэтти**Таким образом, это зависит от вашего интернет-соединения и доступности серверов BLUETTI.

Адаптер, работающий только в облаке **само по себе не может доказать отключение электросети.**&#x42D;то может выявить только доказательства — устаревшие данные телеметрии, доступность облака/устройства и повторяющиеся сбои опроса. Для надежной автоматизации при отключении электроэнергии необходимо объединить эти состояния как минимум с одним из них. **местный** сигнал, например, проверка маршрутизатора/ping, интеллектуальный счетчик, счетчик электроэнергии Shelly или выделенный сигнал ИБП.
...
- [Примечания к API BLUETTI Home Assistant](docs/research/bluetti-ha-api-notes.md) — Результаты проверки подлинности, токены, устройства и телеметрии, полученные на основе исходных данных от вышестоящего источника.
- [Процесс аутентификации, ввода токена и выбора устройства.](docs/auth-flow.md) — Архитектура OAuth/токен/устройство, текущий статус реализации указан вверху.
