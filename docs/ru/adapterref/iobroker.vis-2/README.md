...
![Демонстрационный интерфейс](../../../en/adapterref/iobroker.vis-2/packages/iobroker.vis-2/img/user0.png)
![Демонстрационный интерфейс](../../../en/adapterref/iobroker.vis-2/packages/iobroker.vis-2/img/user7.png)

[Онлайн-демонстрации](https://iobroker.click/)

## Связывание объектов

Обычно большинство виджетов имеют атрибут ObjectID, и этот атрибут может быть связан с некоторым значением идентификатора объекта. Но есть и другой способ привязки. _любой_ присвойте атрибут виджету некоторый ObjectID.
...
Вы можете использовать _любой_ Функции JavaScript (браузера). Аргументы должны быть обозначены двоеточием (':'), в противном случае они будут интерпретированы как формула.
...
- `date` - Формат даты соответствует заданному формату. Формат тот же, что и в [iobroker.javascript](https://github.com/iobroker/iobroker.javascript/blob/master/README.md#formatdate)
...
- `momentDate(format, useTodayOrYesterday)` - Отформатировать значение как дату с помощью Moment.js. [Утвержденные форматы должны соответствовать библиотеке moment.js.](https://momentjs.com/docs/#/displaying/format/). С `useTodayOrYesterday=true` тот `moment.js` формат `ddd`/`dddd` перезаписываются данными за сегодняшний день / вчерашний день
...
**Помнить** что определения стилей будут интерпретироваться как привязки, поэтому используйте `{{style: value}}` или просто
...
- `alert` - Отобразить окно предупреждения в vis-2. Файл "control.data" имеет следующий формат: "message;title;jquery-icon". Заголовок и jquery-icon являются необязательными. Названия иконок можно найти в файле. [здесь](http://jqueryui.com/themeroller/)Чтобы отобразить значок "ui-icon-info", напишите `Message;;info`.
...
- `playSound` - Воспроизвести звуковой файл. Ссылка на файл указана в `control.data`например, <http://www.modular-planet.de/fx/marsians/Marsiansrev.mp3>Вы можете загрузить свой собственный файл в Vis-2 и воспроизвести его, например, так: `/vis-2.0/main/img/myFile.mp3`.
  **Важный** Браузер не может воспроизводить аудио, пока пользователь не щелкнет хотя бы один раз по ссылке на странице. Это политика безопасности браузера. [Здесь](https://github.com/Hugo22O/chrome-autoplay) Вы можете прочитать больше.
...
Поэтому каждый раз, когда `index.html` (без `#viewName`Если вызывается ), откроется наиболее подходящее для данного разрешения представление. Если доступно только одно представление, _"По умолчанию"_ флаг, чтобы это окно открывалось независимо от разрешения или ориентации экрана.
