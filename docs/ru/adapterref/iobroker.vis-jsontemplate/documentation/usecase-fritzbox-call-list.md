...
editLink: https://github.com/ioBroker/ioBroker.docs/edit/master/docs/ru/adapterref/iobroker.vis-jsontemplate/documentation/usecase-fritzbox-call-list.md
title: без названия
hash: ueIraqV3FO0JrSNyH0mN0wPMvJlDTJfRtTkXlnuWKYI=
---
#### Пример использования адаптивного списка звонков FRITZ!Box

##### **Введение**

В этом примере описывается, как отобразить список вызовов FRITZ!Box.`ioBroker VIS` или`VIS-2` с`JSONTemplate` виджет.

**Темный**![fritzbox-call-list dark](../../../../en/adapterref/iobroker.vis-jsontemplate/documentation/fritbox-call-list-dark.png)

**Свет**![световой индикатор списка вызовов fritzbox](../../../../en/adapterref/iobroker.vis-jsontemplate/documentation/fritbox-call-list-light.png)

Шаблон предоставляет:

- адаптивный макет карточки
- Индикаторы входящих и исходящих вызовов
- имя звонящего, номер телефона, пункт назначения, устройство, дата, время и продолжительность
- Выбираемые светлая и темная темы
- индивидуально настраиваемые цвета
- сообщение о пустом состоянии и недопустимых данных

Пример основан на обсуждении на [форуме ioBroker](https://forum.iobroker.net/topic/84984/vis-2-widget-f%C3%BCr-anzeige-der-anrufliste/8) .

---

##### **Источник данных**

Он`ioBroker.tr-064` Адаптер предоставляет список вызовов FRITZ!Box в формате JSON, например, в следующем виде:

```text
tr-064.0.calllists.all.json
```

Точка данных должна содержать массив JSON. Упрощенный пример выглядит так:
...
В этом примере,`sym` определяет направление:

- `>` исходящий звонок
- `<` : входящий звонок
- любое другое значение: неизвестное направление

Если ваш адаптер использует другие значения, отрегулируйте их.`getDirection()` функция в шаблоне.

---

##### **Интеграция в VIS**

1. Установите`vis-jsontemplate` адаптер, если он еще не установлен.
2. Добавить`JSON Template` виджет в представление.
3. В поле « **Точка данных»** выберите точку данных JSON списка вызовов.
4. Вставьте следующий код в поле **«Шаблон»** .
5. При необходимости установите **CSS Common → overflow-y** в значение`auto` .
...
##### **Настройка темы и цвета**

Выберите предопределенную тему в начале шаблона:
...
Цвет, используемый виджетом, можно изменить в настройках.`themes` объект. Допустимые значения включают шестнадцатеричные значения цветов.`rgb()` ,`rgba()` , и`transparent` .

---

##### **Прокрутка и размер виджета**

Сам список вызовов использует`overflow-y: auto` Если контейнер виджета VIS продолжает выходить за пределы заданной высоты, также установите:

```text
CSS Common → overflow-y → auto
```

Это ограничивает размер контента высотой виджета и отображает вертикальную полосу прокрутки, если количество доступных записей о вызовах превышает возможности отображения.

---

##### **Примечания**

- Единственным обязательным условием является привязка данных в формате JSON; виджет обновляется при изменении этого состояния.
- Шаблон принимает массив напрямую, а также проверяет общие свойства обертки.`data` ,`items` , и`result` .
- Все CSS-селекторы имеют область видимости с помощью`widgetid` Таким образом, в одном представлении можно использовать несколько виджетов списка вызовов.
- Измените сопоставление полей в разделе рендеринга, если ваш адаптер списка вызовов возвращает другую структуру JSON.