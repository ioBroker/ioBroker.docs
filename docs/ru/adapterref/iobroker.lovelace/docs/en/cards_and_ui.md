...
Дополнить произведения Лавлейса можно самодельными открытками (`custom cards` Они поставляются в виде файла JavaScript (\*.js), который загружается на сайт. `Custom Cards` вкладка настроек экземпляра или в `lovelace.<instance>/cards` через `Files` Раздел «Администрирование». На вкладке отображается список установленных карт с указанием версии, которую каждая карта предоставляет.

Для загрузки из командной строки, где установлен iobroker:

`iobroker file write PATH_TO_FILE\bignumber-card.js /lovelace.0/cards/`

Все файлы из `cards` Каталоги автоматически включаются в список, и адаптер отслеживает их содержимое: добавленная, замененная или удаленная карта (здесь — в файловом браузере ioBroker или из командной строки) автоматически обнаруживается. После этого перезагрузите браузер, чтобы использовать только что добавленную карту — перезапуск адаптера не требуется. `Reload cards` Вкладка выполняет те же действия по запросу, если изменение было пропущено.
...
### Язык, числовой и формат даты

Язык панели управления берется из адаптера. `Language` параметр (`same as ioBroker` (использует системный язык ioBroker). Это всего лишь значение по умолчанию: язык, выбранный во фронтенде в _разделе «Профиль»_ , имеет приоритет и сохраняется для каждого пользователя, поэтому язык браузера больше не используется.

Цифры, формат времени и даты, часовой пояс и первый день недели — это отдельные настройки, также в разделе _«Профиль»_ . Каждую из них можно оставить включенной. `language` (следуйте языку пользовательского интерфейса) или установите фиксированный формат, чтобы панель управления отображалась на английском языке с немецким форматированием чисел.

Без активированной аутентификации все браузеры используют один профиль, поэтому такое изменение применяется ко всем браузерам.
...
Приведённые выше фрагменты кода работают только внутри темы оформления. Если вы не хотите создавать собственную тему, вот небольшой, самодостаточный пример темы (`no-bell-icon`) который примерно соответствует стандартному темному оформлению и убирает колокольчик. Вставьте его в конфигурацию темы, затем выберите его (например, через состояние). `lovelace.0.instances.set_theme` Тема становится доступной для выбора только после того, как точка данных будет создана и адаптер перезапущен.
