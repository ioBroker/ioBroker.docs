...
Хотя датчики обычно состоят из одного состояния ioBroker (поэтому подойдет и ручная настройка), автоматическое обнаружение все же рекомендуется.`device_class` Атрибут заполнен, и Lovelace устанавливает правильную иконку и единицу измерения.
