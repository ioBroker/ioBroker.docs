...
title: Сущности
hash: Ks3vX8i0X5tpPiKR8Nztv2eGb0HNdDlwnaGuyOggpk4=
---
...
#### Эксперт: любой атрибут из состояния

В нижней части диалогового окна настроек таблица _«Эксперт: атрибуты из состояний»_ сопоставляет любой атрибут Home Assistant сущности с состоянием ioBroker. Введите имя атрибута, выберите состояние, готово: значение будет соответствовать этому состоянию, как и у любого другого атрибута.

Используйте его для атрибутов, не имеющих собственных настроек, например, для `battery_level` на `media_player` Имя, написанное с использованием точек, обозначает вложенный атрибут. `forecast.0.temperature`). Запись переопределяет атрибут, созданный самим адаптером, поэтому это также способ указать атрибуту на другое состояние. Имена атрибутов не проверяются, неизвестное имя просто игнорируется фронтендом.

Состояние типа `array` или `object` Передаётся в виде реального массива или объекта: ioBroker хранит их как строку JSON, а адаптер её анализирует, поэтому карточки, которые итерируют массив (например, flex-table-card), получают свои строки. `mixed` Состояние также анализируется, если его значение соответствует формату JSON, в противном случае оно передается без изменений.
...
Базовая карта создается с использованием тайлов из OpenStreetMap Foundation, которые адаптер загружает и кэширует для браузера. Для работы требуется исходящий доступ в интернет на хосте ioBroker, но не требуется учетная запись и ключ API. (Ранее тайлы поставлялись из CARTO, теперь же они помечаются водяным знаком "ТРЕБУЕТСЯ КЛЮЧ API".)
