...
- Адаптер Shelly запускает собственный MQTT-брокер, который работает на порту `1882`, чтобы избежать конфликтов с другими MQTT-брокерами в той же системе (стандартный порт для MQTT — `1883`).
