...
### Запрос на поддержку новой модели.

Адаптер выбирает библиотеку устройств из первой части идентификатора модели, той части, которая находится перед первой точкой:`roborock` и`rockrobo` используйте библиотеку Roborock.`viomi` Библиотека Виоми,`dreame` и`xiaomi` Библиотека Dreame/MIOT. Модель из одного из этих семейств, не указанная выше, обычно работает с той же библиотекой: базовое управление, состояние, батарея, расходные материалы и история уборки доступны, когда робот использует стандартные свойства, а свойства, которые робот не отображает, остаются пустыми. Если такая модель работает для вас, создайте заявку с идентификатором модели и версией прошивки, чтобы ее можно было добавить в таблицу.

Если в журнале отображается`Model <id> not supported! You can try to setup manually a library in settings.` Идентификатор модели начинается с префикса, который адаптер не знает (например,`ijai` или`xtl` Адаптер не создает никаких состояний, хотя экземпляр запущен. Вы можете принудительно использовать одну из библиотек с помощью параметра **Manager** в настройках экземпляра; устройства, использующие протокол MIOT, иногда работают с библиотекой Dreame. Для новой библиотеки от нового производителя требуется спецификация MIOT модели.`https://home.miot-spec.com/spec/<model id>` ) и кто-то, кто владеет устройством и может его протестировать.

В каждом запросе или сообщении об ошибке должны содержаться идентификатор модели и номер прошивки (`deviceInfo.model` ,`deviceInfo.fw_ver` ), версии адаптера, js-контроллера и Node.js, какие состояния работают, а какие нет, а также отладочный лог, связанный с проблемой, особенно строки, содержащие`Device model detected` ,`MIIO request` и`Status update:` Для решения картографических задач добавьте значение`auth.status` и результат`npm ls canvas` Из каталога ioBroker. Перед публикацией удалите токены, ссылки для входа, облачные сессии и IP-адреса.
...
**Этот адаптер использует библиотеки Sentry для автоматического сообщения разработчикам об исключениях и ошибках в коде.** Более подробную информацию и инструкции по отключению сообщений об ошибках см. в [документации плагина Sentry](https://github.com/ioBroker/plugin-sentry) . Сообщения Sentry доступны в js-controller версии 3.0 и новее.
...
2. Откройте конфигурацию экземпляра и выберите вкладку **«Подключение»** .
3. Выберите регион Xiaomi, в котором зарегистрирован пылесос.
4. Нажмите **на ссылку «Создать учетную запись Xiaomi»** .
5. Откройте указанную ссылку и подтвердите вход в систему Xiaomi в браузере.
6. Вернитесь в ioBroker после того, как статус облака изменится на **«Аутентифицировано»** .
...
Локальное управление не зависит от активной сессии Xiaomi Cloud. Если локальный токен, IP-адрес и модель уже известны, введите их в **разделе «Ручные настройки»** :
...
- **Собственный порт:** локальный UDP-порт, используемый этим экземпляром адаптера, обычно`53421`
...
- **Функция получения списка устройств** становится доступной только после аутентификации.
...
- **Интервал запроса статуса:** как часто запрашивается текущий статус робота. Очень короткие интервалы увеличивают нагрузку на сеть и робота.
- **Интервал запроса состояния Wi-Fi:** как часто обновляется информация о сигнале.
- **Включить карты из Xiaomi Cloud:** позволяет загружать карты из Xiaomi Cloud. Требуется авторизованная облачная сессия.
- **Включить Valetudo:** использует совместимый локальный источник карт Valetudo.
- **Отправка собственных команд:** создание экспертных состояний`control.X_send_command` и`control.X_get_response` .
- **Добавить состояния Alexa/IoT:** дополнительно создает`control.pauseResume` для голосовых помощников и интеграции с устройствами Интернета вещей.`control.clean_home` всегда существует.
- **Отправить паузу перед возвращением на базу:** отправляет паузу перед командой возврата на док-станцию для моделей, которые этого требуют.
- **Возобновление приостановленной уборки зоны с помощью кнопки «Пуск»:** возобновляет прерванную уборку зоны вместо начала полной уборки.
- **Расширенное диагностическое логирование:** добавляет подробную, отредактированную отладочную информацию. Включайте его только временно во время устранения неполадок.
...
- **Интервал сохранения карты:** определяет, как часто записывается сгенерированный PNG-файл.
- **Новый формат карты с цветовой гаммой помещений:** обеспечивает сегментированное отображение помещений там, где это поддерживается.
...
Для использования облачных карт Xiaomi Cloud требуется как **включение карты в Xiaomi Cloud** , так и действующий вход в облако. Локальные команды робота продолжают работать, даже если облачная сессия недоступна.
...
2. Откройте **приложение «Таймер»** и нажмите **«Добавить»** .
3. Выберите день недели, час, минуту, комнаты и/или каналы, относящиеся к комнате.
4. Включите таймер и нажмите **«Сохранить таймеры»** .
...
Если включена **опция «Отправлять собственные команды»** , команды можно записывать в`control.X_send_command` Ответы появляются в`control.X_get_response` Это предназначено для опытных пользователей. Недопустимые или несовместимые с моделью команды могут привести к неожиданному поведению робота.
...
Выберите набор виджетов **mihome-vacuum** и добавьте **панель управления пылесосом с картой, информацией о техническом обслуживании и историей** . Выберите **состояние** (`info.state` Сначала заполните все остальные пустые атрибуты состояния вашего экземпляра: виджет заполнит все остальные пустые атрибуты состояния этого экземпляра, включая варианты Viomi и Dreame для уровня воды, режима швабры и состояния док-станции.
...
Выберите набор виджетов **Mi Home Vacuum** и добавьте **управление пылесосом с картой** . Его настройки сгруппированы по категориям: общие параметры, состояния и элементы управления, обслуживание, комнаты и история.
...
- **Тема оформления:** виджет соответствует светлой или темной теме и основному цвету вашего проекта VIS 2. Дополнительный **акцентный цвет** может переопределять основной цвет.
- **Уровни всасывания:** выбираемые уровни определяются с помощью...`control.fan_power` Состояние вашего робота, поэтому каждая модель отображает свои уровни. Три числовых резервных значения используются только в том случае, если для состояния нет каталога уровней.
- **Тексты состояния и ошибок:** берутся из определений состояния адаптера и переводятся там, где существует перевод.
- **История:** количество отображаемых циклов очистки можно настроить.
- **Настройки уборки:** уровень воды, режим влажной уборки и режим уборки ковров отображаются в качестве элементов управления, когда ваш робот выдает соответствующие состояния. При выборе экземпляра также находятся варианты этих состояний для Viomi и Dreame.
- **Док-станция:** отображает состояние док-станции и предлагает опорожнить пылесборник, а также вымыть и высушить швабру для роботов-пылесосов, оснащенных такой станцией.
- **Карты:** роботы, имеющие несколько карт, получают селектор карты на изображении карты, а **функция «Перезагрузить карту»** получает текущую карту от робота.
- **Расписание:** отображает состояние «Не беспокоить», следующий таймер и все таймеры, созданные в конфигурации адаптера. Таймер можно включить или выключить, пропустить один раз или запустить немедленно из виджета. Этот раздел можно скрыть с помощью **параметра «Показать расписание»** .
...
При **включенной функции автоматического определения комнат** (по умолчанию) виджет VIS 2 отображает все комнаты, созданные адаптером, ниже.`rooms.*` включая собственный уровень всасывания в месте опоры робота. Отключите возможность ручной настройки до шести комнат с отображаемым названием, начальным состоянием и состоянием мощности вентилятора. Виджет VIS 1 всегда использует ручную настройку комнат.
...
- Перед нажатием кнопки **«Получить устройства»** завершите авторизацию в браузере.
- Xiaomi`401` или`403` В результате выполнения команды сохраненная сессия становится недействительной, и требуется повторный явный вход в систему.
...
Для каждого робота создайте отдельный экземпляр адаптера. Каждый экземпляр должен использовать свой **собственный порт** , например...`53421` ,`53422` , и так далее.
...
Используйте [систему отслеживания ошибок GitHub](https://github.com/iobroker-community-adapters/ioBroker.mihome-vacuum/issues) для сообщения о воспроизводимых ошибках и запросов на добавление новых функций.
