...
- `isLeaveActive` (ro) Вход для выхода установлен, никакие тревоги выполняться не будут.
...
- `level` (ro) идентификатор текущего аварийного уровня
    - 1 = Без звука
    - 2 = Акустический
    - 3 = оптический
    - 4 = внутренний
...
- `armDelay` (ro) задержка постановки управления тревогой
...
Это сделано для предотвращения многократного изменения цвета при вводе одним пользователем.
...
- `value-formatted` (ro) текущее форматированное значение как текст
- `needsActivation` (ro) доступно только в том случае, если необходимо активировать элемент управления
- `resetActive` (ro) остается активным, пока активен вход сброса таймера.
...
### Фрониус
Обеспечивается монитором энергии.
...
- производство энергии `prodCurrYear` (ro) за текущий год
- Производство энергии `prodTotal` (ro) с момента создания
...
- `EarningsDay` (ro), сколько денег было заработано сверх текущего, либо потребляя произведенную электроэнергию самостоятельно, а не из сети, либо экспортируя неиспользованную произведенную энергию в сеть.
...
- `gridCurr` (ro) текущая потребляемая / поставляемая мощность сети. Если отрицательный, мощность подается в сеть.
- `batteryCurr` (ro) текущая мощность зарядки / использования аккумулятора. Если отрицательный, аккумулятор заряжается.
...
### Ворота
Обеспечивается воротами управления.
...
- `Осталось` (ro) сколько секунд осталось до следующего обслуживания
...
- `active` (ro) независимо от того, активен ли счетчик в данный момент
- `overdueSince` (ro) секунд с момента превышения поддерживаемого интервала
- `reset` (wo) вызовет сброс следующих значений
    - осталось до maintenanceInterval
    - просрочено до 0
    - просрочено С до 0
- `resetAll` (wo) как` reset`, но также устанавливает
    - всего 0
    - lastActivation to 0
...
- `value` (ro) значение (номер) состояния элемента управления
...
- `additionalMoods` (ro) список нежелательных названий настроений
...
### Радио
Обеспечивается переключателями (8x и 16x).
...
- «осадка в формате»: значение осадков в формате, указанном в единицах измерения.
