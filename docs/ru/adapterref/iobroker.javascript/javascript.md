...
title: без названия
hash: JSqZ5CHaxTKhGdZ4lant3x9YTM0t0uaW04Xn+G7+48k=
---
## Содержание
...
- [Функции](#the-following-functions-can-be-used-in-scripts)
...
**Внимание:** для вызова этой функции необходимо включить параметр _"exec" (Enable command)_ .
...
**Примечание:** `state` ранее назывался `newState` Это всё ещё работает.
...
|                          |                      |                                                                                                                                                                                                |
| тс                       | число                | Новая метка времени должна совпадать с заданной (state.ts == ts).                                                                                                                              |
| tsGt                     | число                | Новая метка времени должна быть больше заданной (state.ts > ts).                                                                                                                               |
| tsGe                     | число                | Новая временная метка должна быть больше или равна заданной (state.ts >= ts).                                                                                                                  |
| tsLt                     | число                | Новая метка времени должна быть меньше заданной (state.ts < ts).                                                                                                                               |
| tsLe                     | число                | Новая временная метка должна быть меньше или равна заданной (state.ts <= ts).                                                                                                                  |
|                          |                      |                                                                                                                                                                                                |
| oldTs                    | число                | Предыдущая метка времени должна совпадать с заданной (oldState.ts == ts).                                                                                                                      |
| oldTsGt                  | число                | Предыдущая метка времени должна быть больше заданной (oldState.ts > ts).                                                                                                                       |
| oldTsGe                  | число                | Предыдущая метка времени должна быть больше или равна заданной (oldState.ts >= ts)                                                                                                             |
| oldTsLt                  | число                | Предыдущая метка времени должна быть меньше заданной (oldState.ts < ts)                                                                                                                        |
| oldTsLe                  | число                | Предыдущая метка времени должна быть меньше или равна заданной (oldState.ts <= ts)                                                                                                             |
|                          |                      |                                                                                                                                                                                                |
| лк                       | число                | Отметка времени последнего изменения должна совпадать с указанной (state.lc == lc).                                                                                                            |
| lcGt                     | число                | Отметка времени последнего изменения должна быть больше заданного значения (state.lc > lc).                                                                                                    |
| lcGe                     | число                | Отметка времени последнего изменения должна быть больше или равна указанной (state.lc >= lc)                                                                                                   |
| лкЛт                     | число                | Отметка времени последнего изменения должна быть меньше заданной (state.lc < lc)                                                                                                               |
| lcLe                     | число                | Отметка времени последнего изменения должна быть меньше или равна указанной (state.lc <= lc)                                                                                                   |
|                          |                      |                                                                                                                                                                                                |
| oldLc                    | число                | Предыдущая метка времени последнего изменения должна совпадать с указанной (oldState.lc == lc)                                                                                                 |
| oldLcGt                  | число                | Предыдущая метка времени последнего изменения должна быть больше заданной (oldState.lc > lc)                                                                                                   |
| oldLcGe                  | число                | Предыдущая метка времени последнего изменения должна быть больше или равна указанной (oldState.lc >= lc)                                                                                       |
| oldLcLt                  | число                | Метка времени последнего изменения должна быть меньше заданной (oldState.lc < lc)                                                                                                              |
| oldLcLe                  | число                | Предыдущая метка времени последнего изменения должна быть меньше или равна заданной (oldState.lc <= lc)                                                                                        |
|                          |                      |                                                                                                                                                                                                |
...
|                          | Множество            | В заданном списке идентификаторов перечислений присутствует один из идентификаторов перечислений (enum ID) данного штата.                                                                      |
|                          |                      |                                                                                                                                                                                                |
...
Если вы начнете `common` флаг `alias` к `true` тогда будет создан псевдоним с тем же именем (но в `alias.0` пространство имен) как состояние. Псевдоним создается только в том случае, если он еще не существует.
...
- `'upload'` - Начинается загрузка адаптера
- `'rebuildAdapter'`
- `'readBaseSettings'`
- `'writeBaseSettings'`
- `'addNotification'`
...
**Внимание:** для вызова этой _команды необходимо включить параметр "sendToHost"_ .
...
`SECRETS` Доступен только для чтения и всегда актуален: если учетные данные добавляются, изменяются или удаляются в административном интерфейсе, новое значение используется немедленно — перезапускать ни адаптер, ни скрипт не требуется.
