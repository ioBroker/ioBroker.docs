...
|             | Массив | Одно перечисляемое имя состояния находится в данном списке перечисляемых имен |
|             |            |                                                                                                                                                     |
...
Позволяет создавать расписание на основе значения состояния. Если значение состояния изменится, старое расписание будет удалено, а новое расписание будет создано автоматически.
...
Чтобы подавить предупреждение, проверьте, существует ли состояние перед вызовом getState (см. [существуетСостояние](#existsState)).
...
* m, м(кириллица) - короткие минуты, например, 4
...
Возвращает, если сценарий включен или отключен. Обратите внимание, что это не дает информации о том, запущен скрипт сейчас или нет.
...
Вторым параметром может быть объект с дополнительными опциями (необязательно). Все опции являются необязательными. Поддерживаемые флаги:
