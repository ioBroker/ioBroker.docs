...
Node.js использует /bin/sh для выполнения команд. Если вы хотите использовать другую оболочку, вы можете использовать объект параметров, как описано в \[ссылка на описание]. [Документация Node.js](https://nodejs.org/api/child_process.html#child_processexeccommand-options-callback) Что касается child\_process.exec, рекомендуется всегда указывать пути к командам, чтобы гарантировать выполнение правильной команды.

**Уведомление:** вы должны включить _Включить команду "setObject"_ вариант для этого.
...
|                          | "не"                 | (не равно) Новое значение не должно быть равно старому (state.val != oldState.val) **Если шаблон представляет собой строку идентификатора, то это значение используется по умолчанию.**               |
...
| valNe                    | смешанный            | Новое значение не должно быть равно заданному.                                                                                                                                                        |
...
| q                        | число                | Состояние кода качества нового значения совпадает с заданным. Для сопоставления с любым кодом можно использовать символ '\*'. **Если значение не указано, в качестве шаблона устанавливается q = 0!** |
|                          |                      |                                                                                                                                                                                                       |
...
Все изменения _stateId1_ будет написано _stateId2_.
...
_Уведомление:_ По умолчанию в функцию обратного вызова передаются только состояния с качеством 0x00. Если вы хотите получить все события, добавьте `{q: '*'}` к структуре шаблона.

_Уведомление:_ Обратите внимание, что по умолчанию значение параметра "change" равно "any", за исключением случаев, когда задан только идентификатор в виде строки (например, `on('id', () => {});`В последнем случае значение будет установлено на "ne".

_Уведомление:_ Если вы хотите, чтобы удаление/истечение срока действия состояний также служило триггером, вам необходимо использовать команду change with. `ne` или `any` И q с `*` в качестве фильтра!

_Уведомление:_ Начиная с версии 4.3.2, в качестве второго параметра можно указать тип триггера: `on('my.id.0', 'any', obj => log(obj.state.val));`

### один раз

Регистрирует разовую подписку, которая автоматически отменяется после первого вызова. Аналогично [на](#on---subscribe-on-changes-or-updates-of-some-state)но выполнено было всего один раз.

```js
once(pattern, callback);
```

### подписаться - то же самое, что **[на](#on---subscribe-on-changes-or-updates-of-some-state)**
...
Шаблон может быть строкой, содержащей [Синтаксис Крон](http://en.wikipedia.org/wiki/Cron)которая состоит из 5 (без секунд) или 6 (с секундами) цифр:
...
Начальное и конечное значения определяют объект Date, строку DateString или количество миллисекунд с 1 января 1970 года 00:00:00 UTC. Правило представляет собой строку расписания. [Синтаксис Крон](http://en.wikipedia.org/wiki/Cron) или объект:
...
**Примечание:** В некоторых местах может случиться так, что ночи/ночного конца вообще не будет. Пожалуйста, прочтите. [здесь](https://github.com/mourner/suncalc/issues/70) об этом.

**Примечание:** Для планирования можно использовать функцию «on» с небольшими изменениями:
...
Возвращает список всех заданий и расписаний CRON (кроме astro). Аргумент должен быть `true` если вы хотите получить список для **каждый запущенный скрипт**В противном случае будут возвращены только расписания из текущего скрипта.
...
Если **нет** Используется функция "astro", отмену расписания можно произвести позже. Для этого необходимо сохранить объект расписания:
...
`clearSchedule` принимает всё `schedule` Возвращает (объект задания CRON или идентификатор расписания мастера расписания), а также записи [getSchedules](#getschedules):
...
Возвращает объект JavaScript Date для указанного имени астрономического объекта (например, `"sunrise"` или `"sunriseEnd"`Допустимые значения см. в списке разрешенных значений в \[ссылка на список]. [Астро](#astro--function) раздел в _расписание_ функция.

Возвращаемый объект Date вычисляется для переданного значения. _дат&#x430;_&#x415;сли дата не указана, используется текущий день.
...
Вы можете использовать такие веб-страницы, как... [suncalc.net](http://suncalc.net) проверить правильность указанных временных точек.
...
_Примечани&#x435;_&#x421;ледующие команды идентичны.
...
Пожалуйста, обратитесь к <https://github.com/ioBroker/ioBroker/wiki/Adapter-Development-Documentation#commands-and-statuses> для использования `ack`. Короткий:
...
Если состояние отсутствует, в логах и объекте будет выведено предупреждение. `{ val: null, notExist: true }` будет возвращено. Чтобы подавить предупреждение, проверьте, существует ли состояние, прежде чем вызывать getState (см. [существуетState](#existsState)).
...
Возвращает в формате enumId все комнаты, в которых запрошенный объект является участником. Вы можете установить значение "true" в качестве enumName, чтобы получить в ответ _все_ перечисления.
...
Вам следует использовать это для **изменить** существующий объект, который вы прочитали ранее, например:
...
Эти сокращенные формы являются причиной того, почему объект на второй позиции находится в таком виде. **никогда** считывается как начальное значение. `createState('myDatapoint', {}, { type: 'object' })` Следовательно, он делает не то, что кажется на первый взгляд: `{}` становится `common`, и `{ type: 'object' }` переходит к `native`.
...
- `common`: общее описание объекта-псевдонима см. описание [здесь](https://github.com/ioBroker/ioBroker/blob/master/doc/SCHEMA.md#state)Значения, указанные здесь, будут иметь приоритет над общим определением объекта идентификатора псевдонима для чтения. Примечание: определения псевдонимов нельзя задавать в рамках этого общего параметра, см. параметр alias!
- `native`: исходное описание объекта. Любая конкретная информация.
- `callback`Вызывается после создания и инициализации состояния.
...
**Уведомление:** необходимо включить _Включить команду "setObject"_ вариант для этого.
...
- `millisecondsOrDate`: количество миллисекунд из файла state.ts или state.lc (количество миллисекунд с 1970.01.01 00:00:00) или JavaScript _new Date()_ объект или количество миллисекунд из _(new Date().getTime())_
...
В качестве имени могут использоваться: состояние, канал, устройство или расписание.
`idfilter` может содержать символы-заменители '\*'

Префиксы _**(Не реализовано — следует обсудить)**_ :
...
- `$('.switch §"Living room")` - Выберите состояния со всеми переключателями в режиме «Гостиная» _**(Не реализовано — следует обсудить)**_
- `$('channel .switch §"Living room")` - Выберите состояния со всеми переключателями в «Гостиной» _**(Не реализовано — следует обсудить)**_
...
Аргумент _адаптер_ может быть опущено.
...
В функции обратного вызова будет передан необязательный код ошибки. Аргумент _адаптер_ Можно опустить. fileName — это имя файла в базе данных. Все файлы хранятся в папке "javascript". Если вы хотите записывать данные в другие папки, например, в "/vis.0/", используйте для этого setFile.
...
**Важный**Эта функциональность доступна только при наличии <js-controller@4.1.x> или более новые.
...
Для отправки сообщения JavaScript-скрипту, который затем будет получен этим обработчиком, используйте [messageTo](#messageTo).
...
_Важный:_ Во избежание бесконечных циклов нельзя выводить в обработчике сообщения с одинаковым уровнем серьезности.
...
Управление учетными данными осуществляется в административном интерфейсе. **Основные настройки** -> **Реквизиты для входа**У каждого учетного документа есть идентификатор (например, `CameraPassword`) и имеет либо один **ключ** (например, ключ API или пароль) или
**авторизоваться**/**пароль** пара. Секретные поля хранятся в зашифрованном виде с использованием системного секрета и передаются скриптам уже расшифрованными:
...
- В настройках экземпляра JavaScript-адаптера, в разделе **Доступные учетные данные** Перечисляет все учетные данные с указанием их полей и готового к копированию выражения.
...
Blockly имеет **учетные данные** блок для той же цели — см.
[Блочная документация](blockly.md#credential).

Доступ можно отключить с помощью параметра экземпляра. **Разрешите скриптам считывать учетные данные.**.
`SECRETS` В этом случае поле пустое, и в журнал записывается предупреждение.
...
2. Адаптер подписывается каждый раз на указанный ID, когда `on/subscribe` Вызывается метод. В этом режиме адаптер получает обновления только для желаемых состояний. Этот вариант требует меньше оперативной памяти и более эффективен, но синхронный доступ к состояниям через метод getState невозможен. **Для доступа к состояниям необходимо использовать обратные вызовы или промисы.**:
