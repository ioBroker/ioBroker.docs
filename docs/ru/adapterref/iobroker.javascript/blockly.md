...
- [блоки](#блоки)
    ...
    - [Блоки даты и времени] (#дата-и-время-блоки)
    ...
    - [триггеры](#триггеры)
        - [Запуск при изменении состояния](#триггер при изменении состояния)
    ...
    - [Математика](#математика)
        ...
        - [Четно, нечетно, простое, целое, положительное, отрицательное, делится на](#is-четно-нечетное-простое-целое-положительно-отрицательно-делимо-на)
...
Этот блок ничего не делает, кроме записи строки в лог. Вы можете использовать его для отладки скрипта, как этот:
...
ID объекта выбирается в диалоге, значение для отправки должно быть указано. В зависимости от типа точки данных значение может иметь тип [строка](#строковое значение), [число](#числовое значение) или [логическое](#ogical-value-trueflase).
...
Этот блок похож на [блок управления](#steuere-state), но устанавливает только текущее значение. Никакая команда не отправляется для управления оборудованием.
...
Этот блок создает глобальные состояния, и если он уже существует, команда игнорируется. Поэтому этот блок можно без риска использовать при любом запуске скрипта.
...
- Объект ошибки, если команда из модуля JavaScript не может быть выполнена
...
Чтобы отправить сообщение конкретному экземпляру, вы должны выбрать установленный экземпляр адаптера (обычно `sayit.0`), иначе сообщение будет отправлено всем существующим экземплярам.
...
Конечно, вы можете использовать пользовательские функциональные блоки, чтобы делать что-нибудь сумасшедшее, а также отправлять сообщения.
...
- hours - возвращает часы текущего дня от 0 до 23,
...
- короткий год - год от 0 до 99, например на 2016 год результат будет 16.
...
Атрибут "смещение" представляет собой смещение в минутах. Он также может быть отрицательным, чтобы определить время до астрономического события.
...
Преобразование "Объект даты" в строку. Он имеет те же параметры формата, что и [Получить фактическое время в определенном формате](#get-actual-time-im-specific-format).
...
Это тот же блок, что и "Триггер при изменении состояния", но без возможности использовать несколько идентификаторов объектов для срабатывания (для совместимости версий).
...
- state value - фактическое значение активированного состояния
...
- последнее изменение состояния - временная метка последнего изменения этого значения
...
Дополнительной функцией является установка интервала с помощью переменной, просто замените «мс» на предопределенную переменную: ![Выполнение по интервальной переменной](../../../de/adapterref/iobroker.javascript/img/Timer_variable_en.PNG)
...
### Математические константы: pi, e, phi, sqrt(2), sqrt(1/2), бесконечность
