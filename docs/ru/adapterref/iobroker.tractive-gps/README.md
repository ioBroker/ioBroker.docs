...
## Виджеты VIS
Адаптер включает классический `PetTrackerCard` для VIS 1 и нативный React `PetTrackerCard` для VIS 2. Добавьте по одному виджету для каждого питомца или трекера и назначьте необходимые состояния в настройках виджета.

На карте можно отобразить:

- имя питомца, порода, пол, возраст и вес.
