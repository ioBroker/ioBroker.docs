...
> [Немецкая документация](/#/docs/adapterref/iobroker.tractive-gps/README_DE.md)
...
- VIS 1 или VIS 2 версии 2.12.8 или новее, при использовании встроенного виджета.
- Учетная запись Tractive, имеющая как минимум один связанный трекер.

## Функции

- Получает фактические имена и данные о домашних животных, связанных с учетной записью.
- Предоставляет текущие GPS-координаты, высоту, скорость, точность определения местоположения, расстояние от заданного местоположения ioBroker и время последнего обновления.
- При необходимости преобразует координаты в читаемый адрес.
- Предоставляет информацию об уровне заряда батареи, состоянии зарядки и используемом источнике данных о положении батареи.`KNOWN_WIFI` /`GPS` ), статус «дома/вне дома», состояние сети и режим энергосбережения.
- Предоставляет информацию о модели, прошивке, версии оборудования, возможностях, поле, дате рождения, росте, весе и другие доступные данные.
...
- Включает в себя интерактивные карточки для VIS 1 и VIS 2 с изображением питомца, интерактивной картой, отображением диапазона, статусом трекера и элементами управления.
- Поддерживается использование изображения, предоставленного Tractive, или пользовательского изображения, загруженного в ioBroker.
...
| Параметр                          | Описание                                                                                                  |
| --------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Электронная почта                 | Адрес электронной почты, указанный в учетной записи Tractive.                                             |
| Пароль                            | Пароль от учетной записи Tractive. Он хранится в стандартном зашифрованном формате конфигурации ioBroker. |
| Интервал обновления               | Интервал между регулярными обновлениями местоположения. Доступны значения от 2 до 60 минут.               |
| Преобразование координат в адрес. | Запрашивает читаемый адрес для текущих координат. Отключите эту опцию, если адрес не требуется.           |

Используйте **функцию «Проверить соединение»** , чтобы подтвердить введенные учетные данные. Сохраните все настройки с помощью обычной кнопки **«Сохранить»** в нижней части страницы конфигурации ioBroker.
...
- `info.dataFresh` : Указывает, доступны ли в данный момент пригодные для использования данные.
...
- `info.currentApi` Полный JSON-снимок доступных в данный момент данных о тяговом движении.

### Домашние животные

Ниже перечислены штаты.`pets.<pet-id>.*` Содержит полезную информацию о профиле питомца, назначение трекера, цели активности и фотографию профиля. Пустые поля и внутренние поля API опущены.

### Следопыты

Ниже перечислены штаты.`trackers.<tracker-id>.*` Содержит идентификационные данные трекера, информацию о рабочем и онлайн-статусе, местоположение, источник данных о местоположении, расстояние от местоположения системы ioBroker, адрес, информацию о батарее и поддерживаемые команды.`location.sensorUsed` Содержит источник данных о положении тяги.`status.home` получено из`KNOWN_WIFI` или`GPS` Дубликата нет.`connectionType` Состояние. Широта и долгота ioBroker задаются в системных настройках.

### Полные данные API

В качестве отдельных состояний создаются только значения, полезные для скриптов, автоматизации и визуализации. Пустые значения, метаданные API, внутренние поля версий и дублирующиеся представления опускаются. Полный, неизмененный объединенный ответ остается доступным в виде одного значения JSON.`info.currentApi` Пароли для входа и токены доступа никогда к нему не добавляются.
...
Установите желаемое состояние на`true` или`false` Состояние подтверждается после того, как компания Tractive принимает команду.

## Виджеты VIS

В комплект адаптера входит классический`PetTrackerCard` для VIS 1 и нативного React`PetTrackerCard` Для VIS 2. Добавьте по одному виджету для каждого питомца или трекера и назначьте необходимые состояния в настройках виджета.

На карте можно отобразить:

- имя питомца, вид, пол, возраст и вес.
- Название трекера и состояние онлайн-доступа.
- изображение питомца из местного`media.localProfilePictureUrl` состояние,
- Интерактивная карта Leaflet/OpenStreetMap.
- радиус позиции, указанный в отчете или заданный вручную.
...
Для использования командных переключателей назначьте соответствующие параметры.`trackers.<tracker-id>.commands.*` Состояния отображаются в разделе « **Команды»** виджета. Команды отключены во время редактирования представления VIS и активируются в режиме выполнения.
...
- Выбранная информация об учетной записи и подписке хранится в логическом дереве объектов. Полные полученные данные API хранятся локально.`info.currentApi` . Обеспечьте надлежащую защиту доступа к дереву объектов ioBroker.
...
- Система обработки ошибок Sentry работает в соответствии с глобальной конфигурацией ioBroker Sentry.
...
- **Домашние животные и трекеры не отображаются:** убедитесь, что трекеры назначены настроенной учетной записи Tractive, затем перезапустите экземпляр адаптера.
- **Данные не обновлены:** Проверьте`info.status` ,`info.dataFresh` , и`info.lastSuccessfulSync` .
- **Сообщается об ошибке HTTP 429:** Оставьте экземпляр запущенным. Адаптер приостанавливает запросы и автоматически повторяет их после истечения лимита Tractive.
...
- **Изображение питомца отсутствует:** Назначить`localProfilePictureUrl` к виджету или выберите собственное изображение.

## Документация для разработчиков

Информация для участников проекта доступна в [документации для разработчиков](/#/docs/adapterref/iobroker.tractive-gps/docs/DEVELOPMENT.md) .

## Кредиты

Первоначально создан [xXBJXx](https://github.com/xXBJXx) и поддерживается организацией ioBroker community adapters.
