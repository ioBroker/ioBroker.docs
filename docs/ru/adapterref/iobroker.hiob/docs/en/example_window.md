...
- `window_states` Теперь необходимо добавить и присвоить имена состояниям, которые следует отслеживать. Затем эти объекты создаются в псевдониме alias.0.<имя\_псевдонима>. Если необходимо изменить или удалить объекты, внесите соответствующие изменения в Blockly. Объекты, не добавленные сюда, автоматически проверяются в 14:00 и могут быть удалены. Проверку можно настроить ниже.
...
- "emoji\_opened" выводится, когда окно открыто.
- `emoji_closed` Излучается при закрытом окне.
