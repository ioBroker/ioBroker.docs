...
Например, DPT5.001 кодируется как целое число без знака с 8 битами. Это дает одно значение. DPT3.007 (управление затемнением) кодируется как 1 бит (логическое значение) + 3 бита (беззнаковое целое).
...
4. Проверка флага в конфигурации устройства:
   ...
   флаги обрабатываются следующим образом:
    ...
    | х | - | - | х | х | отправка любого значения в этом состоянии запускает GroupValueRead|
