...
- [Конфигурация адаптера](#adapter-configuration)
  ...
  - [Объекты](#objects)
  - [Использование](#usage)
  - [Типы точек данных (ТТД)](#data-point-types-dpt)
  - [Как работает импорт](#how-the-import-works)
  - [Избегание проблем](#avoidance-of-problems)
- [GA-Tool](#ga-tool)
  - [Прямая связь между состоянием, не относящимся к KNX, и состоянием, относящимся к KNX, и наоборот.](#direct-link-non-knx-state-to-knx-vice-verse)
- [Запланированные функции](#planned-features)
- [Список изменений](#changelog)

## Описание

Этот адаптер позволяет импортировать`knxproj` Файлы из ETS. Он генерирует преобразование между адресами KNX-групп и ioBroker и размещает устройства в комнатах (специально для MobileUI).
...
**Внимание: в связи с переходом на KNX-адаптер версии 2.x, условия лицензирования изменились. Новую лицензию можно получить на [сайте https://iobroker.net](https://iobroker.net/)**

**Также вам следует обновить js-контроллер и административную панель iobroker до последней версии.**
...
- Облачные сервисы и интернет не требуются
...
\###Установка Этот адаптер устанавливается только с помощью npm. Установка через GitHub **не** работает.

\##Настройка адаптера После установки этого адаптера откройте его конфигурацию.

\###Установка лицензии Первый шаг — применение лицензии. Если лицензия не установлена, то будет применено 500 точек данных.
...
Если вы уже создали новую лицензию на [https://iobroker.net](https://iobroker.net/) , то можете вставить ее в (2), ИЛИ вы можете получить ее напрямую онлайн, нажав на (1).
...
1. IP-адрес KNX-шлюза: IPv4 шлюза KNX-LAN.
...
4. Количество пакетов KNX в секунду: этот параметр ограничивает скорость передачи пакетов. Если шлюз KNX Lan Gateway слишком часто переподключается или временно недоступен, уменьшите этот параметр.
5. Локальный IP-адрес iobroker: выберите IP-адрес/интерфейс, к которому будет привязан адаптер.
6. Уровень логирования: обычно это уровень "Информация", для отладки повысьте уровень.
7. Импортировать только новые точки данных: эта функция включена по умолчанию. В случае ее отключения будут созданы новые GA, а также будут пересозданы существующие GA.
8. Кнопка загрузки файла: здесь доступна функция перетаскивания, или же вы можете загрузить файл, нажав на соответствующую кнопку в диалоговом окне выбора файла. Здесь вы можете загрузить свой экспорт ETS.`knxproj` формат. После успешного импорта появится диалоговое окно с указанием количества импортированных объектов. Теперь нажмите «Сохранить и закрыть», и адаптер должен запуститься. Во время запуска адаптер считывает все групповые адреса с флагами чтения и записи. Это может занять некоторое время и создать высокую нагрузку на вашу KNX-шину. Но значения в вашем vis обновляются после запуска. Загрузка файла, защищенного паролем, пока недоступна.
9. Host-ID: это специальный идентификатор хоста iobroker. Этот идентификатор необходим для генерации и проверки лицензии.
10. GA-Tools: набор инструментов для быстро меняющихся генетических алгоритмов.
...
1. Значение или строка
2. Значения, разделённые запятыми, или массив значений (пока я не знаю, какой способ лучше использовать).

Например, DPT5.001 кодируется как беззнаковое целое число с 8 битами. Это дает одно значение. DPT3.007 (управление затемнением) кодируется как 1 бит (логическое значение) + 3 бита (беззнаковое целое число). В результате получается, например, значение типа "0,5", где "0" означает "уменьшение", а "5" — количество интервалов.

### Как работает импорт

1. Чтение всех ссылок на объекты связи (COR): объединение идентификатора ссылки на групповой адрес с DPT соответствующей ссылки на объект связи (если она существует).

2. Создание структуры групповых адресов (GAS): создание GAS на основе идентификаторов GAR и установка DPT (если это еще не сделано).

3. Поиск адресов штатов и актов: в ets-exports нет информации об адресах штатов и актов. Адаптер анализирует все GA типа "status" или "state". Если есть два GA с уровнем сходства более 90%, то один адрес будет принадлежать акту, а другой — штату. Также проверяется сходство DPT. Поэтому найти пару непросто, если именование GA не согласовано.

4. Проверка флагов в конфигурации устройства: обработка флагов осуществляется следующим образом:

   \| KNX | KNX | KNX | ioBroker | ioBroker | | |-------|-------|----------|----------|----------|----------------------------------------------------------| | Чтение | Запись | Передача | Чтение | Запись | Пояснение | | - | - | - | - | - | значение будет обновлено с помощью GroupValueRead | | x | - | - | x | x | отправка любого значения в этом состоянии запускает GroupValueRead | | - | x | - | - | x | запись значения в KNX с помощью GroupValueWrite | | - | - | x | x | - | значение состояния будет обновлено с помощью GroupValueResponse | | x | - | x | x | x | отправка любого значения в этом состоянии запускает GroupValueRead |

5. Создание узлов точек данных (DPP): узел DPP будет создан, если GA, GAR и DPT действительны. Это узлы DPP, с которыми работает адаптер. Если DPT отсутствует в GA, поскольку его не удалось найти, узел DPP не будет создан. Это можно сделать с помощью GA-Tool.

6. При запуске адаптера: Все GA, отмеченные флагом "Чтение", проверяются в начале работы. Это может повлиять на более высокую нагрузку на шину. В итоге все состояния обновляются.

### Избегание проблем

- чистое программирование ETS, и что еще важнее, чистое программирование ETS, и самое главное, чистое программирование ETS.
- Назначьте врачей-специалистов по физиотерапии!!
- единая маркировка названий GA (eB «EG Wohnen Decke Licht schalten» и «EG Wohnen Decke Licht schalten status»)
...
- В связи с возможностью запроса статуса необходимо отметить один момент: следует убедиться, что ioBroker генерирует не более 40 запросов в секунду, поскольку в этом случае они могут быть физически сгенерированы и больше не передаваться на шлюз адаптером.
...
1. Отображает дерево генетического алгоритма и выбранный генетический алгоритм.
2. В разделе свойств указывается название выбранного GA.
3. установить флаги iobroker
4. установить GA DPT
5. признанный акт GA
6. признанный штат Джорджия

![knxV2-3-2-GATools-mod](../../../en/adapterref/iobroker.knx/docs/pictures/knxV2-3-2-GATools-mod.jpg)

1. показать взаимосвязь государства и акта
2. Если связь существует, то её можно удалить.

Если связь отсутствует, то можно создать новую, щелкнув (2) для выбранного GA (1). В диалоговом окне (3) можно выбрать партнера.
...
### Прямая связь между состоянием, не относящимся к KNX, и состоянием, относящимся к KNX, и наоборот.

Начиная с версии адаптера 2.0.6, появилась возможность напрямую связывать состояние ioBroker, не относящегося к KNX, с GA. Это можно использовать для передачи времени, даты, любых состояний или информации в KNX. (Небольшая подсказка: вы можете напрямую связать любой из ваших IoT-компонентов с GA в KNX (например, связать кнопку Homematic с KNX GA или датчик кнопки KNX с...)\
&#x20;(ваш плеер Sonos)). Состояния можно считывать с помощью GroupValueRead, и если состояние изменится, оно будет автоматически обновлено в KNX. Кроме того, если вы измените состояние в KNX, оно обновит связанное с ним IoT-устройство, не использующее KNX.

![knxV2-3-7-GATools-Directlink-mod](../../../en/adapterref/iobroker.knx/docs/pictures/knxV2-3-7-GATools-DirectLink-mod.jpg)

1. выберите Google Analytics для подключения
2. показать выбранный GA
3. Этот GA должен иметь атрибут **записи.**
4. Выберите допустимый тип данных (если они не совпадают, это не сработает).
5. Не допускается наличие отношений «действие-государство».
6. кнопка для выбора объекта, не относящегося к KNX, для связи с ним.
...
Теперь KNX-GA **(1)** напрямую связан с не-KNX iobroker **(2)** . С помощью **(3)** вы можете удалить эту связь.
