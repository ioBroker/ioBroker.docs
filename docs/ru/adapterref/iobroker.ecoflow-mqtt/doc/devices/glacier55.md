...
editLink: https://github.com/ioBroker/ioBroker.docs/edit/master/docs/ru/adapterref/iobroker.ecoflow-mqtt/doc/devices/glacier55.md
title: Штаты для GLACIER55
hash: trXrIutgJeuVWAqIkmYoc2vDsIecRJ6VTUSnwL3aXTg=
---
# Штаты для GLACIER55
...
| batTemp102        | температура батареи            |
...
| ------------------- | :---------------------------------: | ------------------------------------------------------------------------------------------------------------------------ |
| sysStatus           |          Состояние системы          | {0: устройство выключено, 1: устройство включено, 2: устройство в режиме ожидания}                                       |
| cmsChgDsgState      |      Состояние зарядки/разрядки     | {0:не заряжается и не разряжается,1:разряжается,2:заряжается}                                                            |
| plugInInfoPvFlag    |    Информация о плагине, флаг PV    | {0: Адаптер питания переменного тока с разъемом XT60 снят, 1: Адаптер питания переменного тока с разъемом XT60 вставлен} |
| plugInInfoPvType    |     Информация о плагине Тип PV     | {0: Шнур питания отсоединен, только батарея, 1: Шнур питания вставлен}                                                   |
| plugInInfoDcpInFlag |    вставить информацию dcp в флаг   | {0: Батарея не вставлена, 1: Батарея вставлена}                                                                          |
| tempUnit            |    Единица измерения температуры    | {0:не задано?,1:°C (Цельсий),2:°F (Фаренгейт)}                                                                           |
| lidStatus           |        состояние крышки/двери       | {0:Закрыто,1:Открыто}                                                                                                    |
| zoneStatus          | Состояние зон (разделитель отсеков) | {0: Двухзонный, 1: Однозонный (в однозонном режиме управление осуществляется левой задаточной точкой)}                   |
| tempAlert           | Сигнализация о перепаде температуры | {0:выкл,1:вкл}                                                                                                           |
...
| ЖК-свет                |  0  |  100  |    %    | 0.390625 | яркость экрана                             |
| screenOffTime          |  0  |  1800 |    с    |     1    | Время ожидания экрана (с)                  |
...
| cmsBattDesignCap       |  0  |  1000 |   Что   |     1    | cms Battery Design Cap                     |
| cmsBattPowOutMax       |  0  |  4000 |    В    |     1    | cms batt pow out max                       |
| cmsBattPowInMax        |  0  |  4000 |    В    |     1    | мощность батареи cms в максимальном режиме |
| tempMonitorLeft        | -30 |   40  |    °С   |     1    | Датчик температуры слева                   |
| tempMonitorRight       | -30 |   40  |    °С   |     1    | Датчик температуры справа                  |
| uptime749              |  0  |  н/д  |    с    |   0.001  | продолжительность ледового времени         |
| входное напряжение 777 |  0  |   40  |    В    |     1    | входное напряжение                         |

### уровень

| Состояние      | Мин |  Макс | Единица | Мульт | Имя                             | команда                                              |
| -------------- | :-: | :---: | :-----: | :---: | ------------------------------- | ---------------------------------------------------- |
| devStandbyTime |  0  | 96400 |   мин   |   1   | Тайм-аут устройства             | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 3} |
...
| setPointLeft   | -20 |   20  |    °С   |   1   | Уставка температуры слева       | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 6} |
| setPointRight  | -20 |   20  |    °С   |   1   | Уставка температуры.            | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 6} |
| batProtect     |  0  |   3   |         |   1   | Защита стартерного аккумулятора | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 3} |

### выключатель

| Состояние          | выключенный |  на | Имя                                    | команда                                              |
| ------------------ | :---------: | :-: | -------------------------------------- | ---------------------------------------------------- |
| enBeep             | выключенный |  на | Включение/выключение звукового сигнала | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 2} |
| блокировка ребенка | выключенный |  на | блокировка от детей                    | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 3} |
| простой режим      | выключенный |  на | простой режим                          | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 3} |
| режим охлаждения   | выключенный |  на | режим охлаждения                       | {назначение: 2, cmdFunc: 254, cmdId: 17, dataLen: 3} |
