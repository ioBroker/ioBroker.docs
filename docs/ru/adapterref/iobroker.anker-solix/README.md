...
1. Включите протокол **Modbus TCP** в приложении Anker (солнечная батарея: система / управление сторонними сервисами; **X1**: профессиональное приложение → настройки связи; **зарядное устройство для электромобилей V1**: настройки → интеграции).
...
Официальный образ поставляется с **Python 3.11**. Начиная с **0.10.87**, адаптер принимает его как **наилучший вариант** в контейнерах Bookworm на Debian 12 - пользовательский образ не требуется. **3.12+** по-прежнему рекомендуется (используется разработчиками из основного репозитория) и по-прежнему требуется на физических серверах и хостах без Bookworm. Руководство: **[docs/docker-buanet.md](/#/docs/adapterref/iobroker.anker-solix/docs/docker-buanet.md)** (необязательные файлы 3.12 в [`docs/docker/`](https://github.com/MatthiasUlrich1/ioBroker.anker-solix/blob/main/docs/docker/), PDF: [docs/Anker-Solix-buanet-Docker-Anleitung.pdf](https://github.com/MatthiasUlrich1/ioBroker.anker-solix/blob/main/docs/Anker-Solix-buanet-Docker-Anleitung.pdf)).
...
| **ev_charger** | Интеллектуальное зарядное устройство для электромобилей V1 | В основном MQTT | **Modbus TCP** (локально) |
| **транспортное средство** | Виртуальные электромобили для зарядных устройств | ориентирован на чтение | - |
| **PowerPanel** / **HES** | Панель управления электропитанием США, X1 HES | Ограниченный API | **X1 Modbus TCP** (локальный) |
...
Облако: тот же путь опроса, что и у других солнечных батарей (API + опционально MQTT). **Ежедневные кВт·ч** (`statistics.daily_*`) считываются при **подробных опросах** (каждые `deviceDetailMultiplier` циклов, по умолчанию ~10), а не каждую минуту - проверьте журнал на наличие `Daily kWh statistics updated`. **С Power Dock/комбинатором:** значения отображаются только в `combiner_box.<SN>.statistics.*`, а не в каждом `solarbank.*`. Перезапустите адаптер после включения **Объекты → Дневная статистика**. Итоговые данные за неделю/месяц/год запускаются по вечернему расписанию (23:00 / 23:15 / 23:30 Европа/Берлин).

**Локальный Modbus TCP** (официальные карты): включите Modbus в приложении Anker (система / управление сторонними сервисами), затем Администрирование → **Modbus (локальный)**. Типичные коды моделей включают AE103 (SB4). Состояния: `anker-solix.0.modbus.<name>.sensors.*` и `.control.*` (режим работы, ограничения уровня заряда батареи, заданное значение заряда батареи в **управлении сторонними сервисами**). **Только Modbus** пропускает облачные сервисы/Python; светодиод экземпляра горит зеленым, когда подключено хотя бы одно устройство Modbus.
...
В редакторе VIS/VIS-2 установлен виджет **anker-solix**:

| Виджет | Назначение |
|--------|---------|
| **Энергоэффективный дом** | Фотореалистичный фон дома, отображение состояния солнечных батарей / дома / сети / аккумулятора / электромобиля в реальном времени (ручная настройка состояния) |
| **HTML-панель управления** | Любое состояние `dashboard.sites.*.html` (в рабочем состоянии, энергопотребление, настройки и т. д.) |
| **Обзор многосайтовой конфигурации** | Привязать к `anker-solix.0.dashboard.overview.html` |
| **Обзор многосайтовой конфигурации** | Привязка к `anker-solix.0.dashboard.overview.html` |

**Важно:** Виджеты поставляются только с **основной версией GitHub / 0.10.100+**. npm **0.10.90** их **не** включает.

Начиная с версии **0.10.104**, адаптер копирует `widgets/` в файловое хранилище VIS/VIS-2 при запуске и запускает перестройку VIS-2. После установки или обновления:

1. Перезапустите экземпляр **anker-solix** (или дождитесь появления строки в логе автоматической синхронизации).
2. Перезагрузите редактор VIS/VIS-2 (**F5**).
3. В окне выбора виджетов откройте набор **anker-solix**.
...
Затем снова загрузите редактор. Для **Energy Home** назначьте состояния в настройках виджета: **Привязка состояний**, **Потоки сетки**, **Батарея**.
...
Вдохновлено **[ioBroker.solix4](https://github.com/michihorn64/ioBroker.solix4)** от **Майкла Хорна ([@michihorn64](https://github.com/michihorn64))** - спасибо за оригинальную концепцию панели управления! Подробности: [CREDITS.md]](/#/docs/adapterref/iobroker.anker-solix/CREDITS.md).

После каждого успешного опроса адаптер записывает **самодостаточный HTML-код** (темная тема, поток энергии в реальном времени, настройки, суточная норма кВтч, диагностика, список устройств) в строковый файл states с ролью `html`:

| Состояние | Содержание |
|-------|---------|
| `anker-solix.0.dashboard.sites.<siteKey>.live.html` | Текущий поток энергии (Солнечная энергия → Дом ↔ Сеть, батарея) |
| `…energy.html` | Ежедневные показатели потребления кВт·ч + автаркия / собственное потребление |
...
| `anker-solix.0.dashboard.overview.html` | Сравнение на нескольких площадках |
| `anker-solix.0.dashboard.overview.html` | Сравнение сайтов |

`<siteKey>` - это первые 8 символов идентификатора сайта Anker (аналогично solix4).

**VIS / VIS-2:** добавьте виджет **HTML Dashboard** (установите **anker-solix**) и привяжите его, например, к `anker-solix.0.dashboard.sites.<siteKey>.dashboard.html`, или используйте универсальный виджет VIS **HTML**. Измените размер до размера планшета (~900×700 пикселей). HTML-код обновляется при каждом опросе адаптера.

Включите **Объекты → Tagesstatistiken** для плиток кВтч; включите **Leistungsflüsse** для значений активной мощности.
...
**Перед каждым релизом** (обеспечивается `npm run test:package` → `test/io-package-policy.js`; выполняется локально с помощью `npm run verify:ci` перед каждым push):
