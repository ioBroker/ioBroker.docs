...
editLink: https://github.com/ioBroker/ioBroker.docs/edit/master/docs/ru/adapterref/iobroker.asterisk/docs/SIP_FRITZBOX.md
...
## Установка и настройка Asterisk с Fritzbox с использованием SIP.
...
После завершения настройки раздела «Параметры Asterisk» перейдите на вкладку «Параметры SIP». Выберите **SIP** в качестве службы. Теперь вам нужно ввести следующее:
...
Оставьте вкладку "SSH" пустой, если Asterisk и iobroker работают на одной машине (сервере).
...
Вместо автоматического создания конфигурационных файлов вы можете сделать это самостоятельно. Теперь вам нужно отредактировать следующие конфигурационные файлы Asterisk. Удалите старые данные из этих 4 файлов! Не изменяйте права доступа и полномочия пользователей в этих файлах.
...
Необходимо изменить в файле _/etc/asterisk/sip.conf_ _хост_ (IP-адрес Fritzbox или VoIP-провайдера), _секретный ключ_ , _имя пользователя_ и _имя отправителя_ на имя пользователя, настроенное в Fritzbox или VoIP-провайдере. Замените идентификатор _вызывающего абонента_ на ваш номер телефона, настроенный в Fritzbox. Важно: имя пользователя Fritzbox должно состоять только из цифр. Пример: 12345689, 00004711 или 47110815 !!
...
Для запуска сервера Asterisk введите команду _/etc/init.d/asterisk start._ Теперь необходимо подключить ioBroker к серверу Asterisk. Если ioBroker и сервер Asterisk используют IP-адрес 192.168.1.2, необходимо настроить этот IP-адрес, а также порт, имя пользователя и пароль из файла _/etc/asterisk/manager.conf_ и имя пользователя из вашего файла sip.conf (например, 123456789). Необходимо указать путь для временных аудиофайлов. Этот путь должен быть доступен и авторизован для Asterisk и ioBroker.