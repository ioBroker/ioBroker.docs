...
# IoBroker.jeelink
**Тесты:** ![Тестирование и выпуск](https://github.com/foxthefox/ioBroker.jeelink/workflows/Test%20and%20Release/badge.svg)

Это адаптер для ioBroker, позволяющий интегрировать RFM12B/RFM69 через Jeelink.
Jeelink можно использовать с предустановленным программным обеспечением (rfmdemo) для считывания показаний датчиков OpenEnergy (emon).
...
## Конфигурация:
быть сделано в админке

- определение порта USB
- установка скорости передачи данных

* определить адрес датчика, который принимается в эфире
* определить уникальный адрес датчика в адаптере (LaCrosse меняет адрес в эфире после замены батареи, поэтому просмотрите журнал отладки и скорректируйте адрес датчика после замены батареи)
...
| EC3000 | EC3000 | OK 22 ... | Счетчик электроэнергии |
...
- добавление нового датчика в конфигурацию, после чего он становится видимым на странице администрирования/конфигурации
- HMS100TF Температура ниже 0°C и низкий уровень заряда батареи будут реализованы
