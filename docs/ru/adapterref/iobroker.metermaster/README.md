...
MeterMaster — это мост между [приложением MeterMaster для Android](https://play.google.com/store/apps/details?id=com.propertymanagement.metermaster) и вашим умным домом. Записывайте показания счетчиков электроэнергии, газа, воды или тепла на своем смартфоне; адаптер сохраняет их в виде состояний ioBroker с правильными метками времени и полной историей — готовыми для использования в скриптах, визуализациях и рабочих процессах выставления счетов.
...
1. Установите **MeterMaster** из официального списка адаптеров ioBroker и создайте экземпляр.
...
- **HTTP-приемник** — принимает показания из приложения MeterMaster для Android (по одному или в пакетном режиме).
- **Автоматические состояния** — объекты «дом/квартира/счетчик» создаются при первой синхронизации.
- **Правильные временные метки** —`readings.latest` использует фактическую дату чтения в качестве государственного документа.`ts`
- **Полная история** — каждый счетчик сохраняет данные.`readings.history` JSON-массив
- **Базовая аутентификация** — необязательная защита по имени пользователя/паролю для точек записи.
- **Встроенный веб-интерфейс** — вкладки «Данные», «Импорт», «Журналы» и «Система» на немецком/английском языках.
- **Удаление через веб-интерфейс** — удаление квартир/счетчиков из ioBroker (подтверждение пароля)
- **Складные секции** — складывайте блоки домов/квартир на вкладке «Данные».
- **Графики и CSV-файлы** — исторические графики, ежемесячное потребление и экспорт данных по счетчику в формате CSV.
- **Импорт резервных копий** — восстановление резервных копий приложения MeterMaster (схема 2.0) с помощью перетаскивания.

Дополнительно: [модули ESP32 OLED-дисплея](#optional-esp32-display-nodes) могут отображать выбранные значения измерительных приборов на небольшом экране.

---

## Приложение MeterMaster для Android

Адаптер представляет собой часть ioBroker приложения [MeterMaster](https://play.google.com/store/apps/details?id=com.propertymanagement.metermaster) — локального приложения для Android, предназначенного для управления счетчиками коммунальных услуг.
...
Используйте **функцию проверки соединения** . В случае успеха результат будет выглядеть так:`MeterMaster adapter reachable ✓`
...
В качестве **дополнительной опции** адаптер может управлять [узлами MeterMaster ESP32](https://github.com/MPunktBPunkt/esp32.MeterMaster) , которые отображают выбранные значения показаний счетчика на небольшом OLED-дисплее.
...
- Вкладка **«Узлы»** веб-интерфейса: статус подключения, IP-адрес, выбор счетчика, управление светодиодами, прошивка.

Для использования адаптера или приложения для Android вам **не** потребуется ESP32.
