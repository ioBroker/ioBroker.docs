...
В настоящее время адаптер распознает категории устройств. `heatPump` и `waterPump` Неизвестные категории регистрируются и пропускаются.
...
Адаптер опубликован на npm как `iobroker.fairland`.

Адаптер доступен в **последнем** репозитории ioBroker и может быть установлен непосредственно из списка адаптеров в административной панели ioBroker при выборе этого репозитория. Включение в **стабильный** репозиторий находится в стадии разработки.
...
- `iGarden account e-mail`: имя учетной записи, используемое в приложении iGarden
- `iGarden password`: пароль учетной записи
- `Login country`: необязательный код страны, используемый для входа в iGarden. Оставить `Automatic` если ваш аккаунт работает без явного указания кода страны.
- `Scan interval`: интервал опроса в секундах, минимум 10 секунд, максимум 3600 секунд
- `Courtyard ID`: опциональный динамический выбор из облака iGarden. Оставить `Automatic` использовать первый внутренний двор, возвращенный облаком.
- `Create raw dpId states`: необязательные диагностические состояния в рамках `devices.<device>.raw.dp_<id>`

Адаптер автоматически определяет правильный региональный API-сервер:

- ЕВРОСОЮЗ: `api-eu.fairlandiot.com`
- НАС: `api-us.fairlandiot.com`
...
Доступные для записи состояния сопоставляются с соответствующим Fairland. `dpId` Адаптер сохраняет оптимистичные значения в течение короткого периода после записи, поскольку облаку iGarden может потребоваться несколько секунд, чтобы сообщить о вновь записанных значениях.
...
- облачный вход и автоматическое определение регионального сервера
- обнаружение двора и устройства
- специфичный для категории `dpId` сопоставления
- Анализ масштаба и единиц измерения `dpProperty`
...
Точка входа скомпилированного адаптера —`build/main.js`.
