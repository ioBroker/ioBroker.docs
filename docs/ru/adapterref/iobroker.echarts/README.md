...
**Этот адаптер использует библиотеки Sentry для автоматического сообщения разработчикам об исключениях и ошибках в коде.** Для получения более подробной информации и сведений о том, как отключить отчеты об ошибках, см. [Документация по плагину Sentry](https://github.com/ioBroker/plugin-sentry#plugin-sentry)!
...
![Радар](../../../en/adapterref/iobroker.echarts/img/radar.png)
Для получения прогнозируемого результата используйте агрегацию "Фактическое значение".

### Один столбик на каждую точку данных

Обычно ось X столбчатой диаграммы — это время, и каждый столбец соответствует одному интервалу. При этом... **Настройки полосы → Одна полоса на строку** Вместо этого ось X становится списком линий: каждая линия получает ровно один столбец, который показывает последнее значение этой линии. Вместе с ним добавляется агрегированное значение «Фактическое значение», которое представляет собой текущее значение каждого состояния, например, потребления каждого устройства.

**Горизонтальные перекладины** Диаграмма поворачивается на 90°, так что имена располагаются по оси Y. Это лучший вариант для длинных имен или для большого количества строк.

## Использование

Добавьте это после перезапуска вкладки в админке:
![Администратор](../../../en/adapterref/iobroker.echarts/img/admin.png)
