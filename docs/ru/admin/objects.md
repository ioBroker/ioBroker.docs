...
### 1 - Обновить вид
Если только что созданные объекты не видны, щелчок по этому значку поможет обновить состояние страницы.

### 2 - изменить отображение
Эта кнопка изменяет отображение объектов на этой странице.

Когда кнопка активна, все объекты перечислены в алфавитном порядке по идентификатору. Если эта кнопка не активна, объекты отображаются в иерархическом алфавитном порядке в соответствии с экземплярами в виде древовидной структуры.
...
Следующие два значка также будут отображаться в древовидной структуре.
...
Здесь необходимо выбрать имя для нового объекта, при этом устройство, канал или точка данных будут доступны в качестве типа в соответствии с иерархической структурой. Логическое значение, переключатель, строка символов, число, список значений, поле, объект и смешанные доступны как типы точек данных.
