...
---
Все управляемые объекты находятся под этой вкладкой. Папка создается для каждого экземпляра, в котором созданные ею точки данных расположены в иерархической структуре. Здесь также можно создавать и удалять объекты вручную. Можно загружать или скачивать целые структуры объектов. Другая кнопка позволяет отображать экспертное представление.
...
Это используется для сортировки в папке по идентификатору или имени.
...
Это также используется, среди прочего, для фильтрации при поиске объектов.
...
Более подробную информацию можно найти в описании адаптера истории.

Используя гаечный ключ в строке заголовка, это действие можно выполнить одновременно для всех точек данных, соответствующих текущим критериям фильтра. Поэтому важно внимательно проверить, выбраны ли критерии фильтрации на этой странице таким образом, чтобы были включены только желаемые точки данных.
