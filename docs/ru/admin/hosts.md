...
если есть обновление js-контроллера для установленного репозитория, появится другой значок:

![Обновление контроллера](../../de/admin/media/ADMIN_Hosts_update.png)

Когда вы щелкаете по этому значку, в отличие от значка для адаптеров, обновление не запускается, потому что для этого необходимо закрыть ioBroker. Вместо этого появляются инструкции о дальнейших действиях.