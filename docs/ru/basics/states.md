...
В редакторе объектов все это _находится_ на вкладке «Состояние». Флаг «ack» там означает «подтверждено» — на изображении он показан красным цветом, поскольку значение представляет собой **команду** , и обратной связи пока нет.
