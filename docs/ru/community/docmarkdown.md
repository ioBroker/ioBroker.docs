...
Читаемость здесь - главный приоритет. Документ в формате Markdown должен иметь возможность публиковаться в его базовой форме, не будучи помеченным или отформатированным (как в случае с HTML).
...
Амперсанд особенно непрактичен для веб-разработчиков. Если вы хотите написать о «AT&T», вы должны написать «`AT&amp;T`». Амперсанд даже в URL-адресах должен быть замаскирован. В ссылке на страницу
...
делать. Однако угловые скобки и амперсанд *всегда* кодируются в блоках кода или диапазона. Это упрощает запись поверх HTML в Markdown (в отличие от необработанного HTML, где кодировать все `<` и `&` в основном кошмар).
...
Правило «одной или нескольких строк» подразумевает одно: Markdown поддерживает «жесткий разрыв» абзацев. Это сильно отличается от большинства других средств форматирования текста в HTML (включая параметр «Преобразовать разрывы строк» в подвижном типе), которые форматируют каждый разрыв строки в абзаце как `<br />`.
...
### Цитаты
Markdown использует, как и электронные письма, символ `>` для блоков цитат. Если у вас есть опыт работы с цитатами в электронной почте, вы также будете знать, как создавать цитаты в Markdown. Лучше всего будет, если вы поместите текст в каждую строку и поставите `>` перед каждой строкой:
...
В следующем примере генерируется тот же код, но менее аккуратно:
...
Атрибут title также можно разместить на следующей строке с дополнительным отступом или табуляцией. Это выглядит лучше с длинными URL-адресами:
...
Для сравнения следующий абзац с использованием встроенных ссылок Markdown:
...
Стиль можно выбрать произвольно. Единственное ограничение - один и тот же символ должен использоваться для открытия и закрытия зоны стресса.
...
В настоящее время в Markdown нет синтаксиса для указания размера изображения. Если это необходимо, можно использовать обычный HTML-тег `<img>`.
