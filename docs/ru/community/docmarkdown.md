...
---
# Markdown: синтаксис
?> Markdown был выбран в качестве упрощенного языка разметки, чтобы документацию ioBroker можно было быстро и легко прочитать. Следующее руководство поможет вам узнать синтаксис и возможности Markdown и преобразовать их в отличные документы.
...
Левая угловая скобка используется для открытия HTML-тегов, амперсанд используется для описания именованных символов (сущностей). Если эти символы должны использоваться как «сами себя» в документах HTML, они должны быть замаскированы как объекты, то есть как `&lt;` и `&amp;`.
...
Markdown позволяет использовать эти символы в обычном режиме. Он регулирует само кодирование. Если в сущности используется амперсанд, он не кодируется, в противном случае он преобразуется в `&amp;`.
...
делать. Однако угловые скобки и амперсанд *всегда* кодируются в блоках кода или диапазона. Это упрощает запись поверх HTML в Markdown (в отличие от необработанного HTML, где кодирование каждого `<` и `&` - в основном кошмар).

## Блочные элементы
### Абзацы и разрывы строк
Абзац - это просто одна или несколько строк текста, разделенных одной или несколькими пустыми строками. (Пустая строка - это любая строка, которая *выглядит* как пустая строка - строка, не содержащая ничего, кроме пробелов и табуляции, рассматривается как пустая.) Обычные абзацы не должны иметь отступы с пробелами или табуляциями.
...
Многострочные [Котировки] (# цитаты) и [список записей](#listen), похожие на электронную почту Markdown, работают лучше всего - и выглядят лучше - при форматировании с разрывами строк.
...
Заголовки типа Atx используют 1-6 ромбовидных символов в начале строки, что соответствует уровням 1-6. Например:
...
### Котировки
Markdown использует, как и в электронных письмах, символ `>` для блоков цитат. Если у вас есть опыт работы с цитатами в электронной почте, вы также будете знать, как создавать цитаты в Markdown. Лучше всего будет, если вы поместите текст в каждую строку и поставите `>` перед каждой строкой:
...
сядьте, амет, посвятите себя вкусу элитных блюд. Aliquam hendrerit mi posuere lectus.
...
### Горизонтальные линии Тег горизонтальной линии (`<hr />`) может быть сгенерирован путем написания трех или более дефисов или звездочек в одной строке. Также разрешены пробелы между символами. Все следующие примеры генерируют горизонтальную линию:
    * * *
...
Определения ссылок можно сделать в любом месте документа Markdown. В общем, рекомендуется делать их после абзаца, в котором они используются. Однако, как и сноски, все они могут быть перечислены вместе в конце документа.
...
Идея ссылочных ссылок не в том, что их легче написать. Идея в том, что они делают документы более читабельными. Примерный абзац содержит всего 80 символов со ссылочными ссылками и 181 символ со ссылочными ссылками; 239 символов в формате HTML, больше разметки, чем содержимого.
...
Разделители обратных апострофов вокруг области кода могут содержать пробелы - один после открывающего обратного апострофа, один перед закрывающим обратным апострофом. Это позволяет использовать обратные кавычки внутри области кода, даже в начале или в конце:
...
В Markdown используется синтаксис, подобный стилю ссылок. Это позволяет два типа: встроенный и ссылочный.
