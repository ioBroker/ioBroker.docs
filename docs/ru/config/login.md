...
Вход в систему разрешен **для каждого экземпляра**, а не для всей системы в целом. Каждый интерфейс имеет свой собственный веб-сервер и, следовательно, свой собственный коммутатор: администратор, каждый экземпляр `web`, `vis` через связанный экземпляр `web` и так далее. Пользователи и группы везде одинаковы, см. [Управление доступом](/docs/config/userrights.md).
