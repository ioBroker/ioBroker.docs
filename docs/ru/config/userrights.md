...
Эти права доступа к объекту называются **ACL**, сокращение от *Access Control List*. Они имеют структуру, аналогичную правам доступа к файлам в Linux: трехзначное число, например, `664`. Однако они читаются в **шестнадцатеричном** формате, а не в восьмеричном, как в Linux. В административном интерфейсе и в командной строке их нужно вводить точно так, как показано здесь; только в скриптах и JSON-файлах необходимо использовать `0x664`, иначе отобразится совершенно другое значение.

На этой странице показано, что нужно делать в повседневном использовании. Подробное описание того, как ioBroker принимает решения - относительно объектов, состояний и файлов, со всеми операциями и значением каждой цифры - приведено в разделе [Списки контроля доступа (ACL) в подробном описании](/docs/config/acl.md).
...
| второй | **группа владельцев**, т.е. все, кто входит в эту группу |
...
Права доступа, назначаемые вновь созданным объектам, определены в [Системные настройки](/docs/admin/settings.md) в разделе *Стандартный ACL*. Объекты, которые уже имеют ACL, остаются без изменений. Объекты без ACL получат новые значения задним числом.
