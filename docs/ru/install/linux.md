...
Мы не рекомендуем этого из-за известных аспектов безопасности. Чтобы установить ioBroker, достаточно использовать команду sudo и предварять соответствующую команду.
...
В принципе, это безвредно, поскольку команда ``nodejs`` давно не использовалась, но ее можно« починить »с помощью символической ссылки с помощью команды` `sudo ln -s /usr/bin/node /usr/bin/nodejs``.
...
Проблемы с разрешениями могут возникнуть после изменения установки.
