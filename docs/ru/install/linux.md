...
В принципе, это безвредно, так как команда ``nodejs`` давно не использовалась, но ее можно« починить »через символическую ссылку с помощью команды` `sudo ln -s /usr/bin/node /usr/bin/nodejs``.
