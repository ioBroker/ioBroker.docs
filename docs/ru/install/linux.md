...
### Установка Node.js
!> с текущим установщиком ioBroker (см. ниже) **в системе без node.js** текущая рекомендуемая версия node.js устанавливается автоматически! Предварительная установка node.js **больше не требуется**
...
только если **ВСЕ** эти команды не дают результата (т.е. больше не отображают номер версии), продолжайте с шага 4 этого раздела, в противном случае или если версия не соответствует той, которую вы хотите, выполните следующие действия заранее :
...
В принципе, это безвредно, потому что команда ``nodejs`` давно не использовалась, но ее можно« исправить »с помощью символической ссылки с помощью команды` `sudo ln -s /usr/bin/node /usr/bin/nodejs``.

---

Если версии разные, пожалуйста, пройдите через раздел [Установка Node.js](#installation-nodejs) еще раз.
...
больше никогда. Это особенность Linux, а не ioBroker!
