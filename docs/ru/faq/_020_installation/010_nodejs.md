...
ioBroker работает на Node.js. Рекомендуется использовать версию с долгосрочной поддержкой (LTS). **Node.js 22**.

!> **Использовать нечетные номера версий нельзя.** (21, 23, 25…). Это секторы развития, не имеющие долгосрочной поддержки.
...
Они оба должны быть совместимы. `nodeCurrent`, `nodeNewest` и
`nodeNewestNext` Если на вкладке «Хосты» отображаются несоответствия, необходимо обновить данные.
