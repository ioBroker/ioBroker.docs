...
Это минимальный образ SD-карты для Rock64. Подходит для карт емкостью 4 ГБ и больше. Поскольку он помещается только на карту емкостью 2 ГБ, рекомендуемый минимальный размер — 4 ГБ. В любом случае рекомендуется использовать карты емкостью 16 ГБ или больше, чтобы не всегда записывались одни и те же ячейки.
...
Образ содержит Armbian 5.90, основанный на Debian «Stretch» от 28 июня 2019 года после загрузки с [https://dl.armbian.com/rock64/Debian_stretch_default.7z](https://dl.armbian.com/rock64/Debian_stretch_default.7z).
...
Кроме того, установлен node-js v 10.16..0 и, конечно же, iobroker с контроллером js по состоянию на 30 июля 2019 года.
...
Если после первого запуска Rock вам не будет предложено создать новый пароль для пользователя root и нового пользователя, в целях безопасности действуйте следующим образом:
