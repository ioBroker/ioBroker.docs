...
- `text` - текстовый компонент
  ...
  - `noClearButton` - если true, кнопка очистки не будет отображаться.
...
- `выбратьОтправить`
  ...
  Показывает раскрывающееся меню с заданными значениями экземпляра.
    ...
    - `showAllValues` - показать элемент, даже если для него не найдено метки (множественно), default=`true`
...
- `defaultFunc` — JS-функция для расчета значения по умолчанию.
- `defaultSendTo` - команда запроса начального значения из работающего экземпляра, пример: `"myInstance": {"type": "text", "defaultSendTo": "fill"}`
