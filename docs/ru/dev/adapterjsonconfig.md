...
- `экземпляр`
    ...
    - `all` - Добавить к опциям опцию "все" со значением `*`
...
- `staticLink` — статическая ссылка
    ...
    - `variant` - тип кнопки ("контурная", `содержащая`, `текстовая`)
...
- `table` - таблица с элементами, которые можно удалять, добавлять, перемещать вверх и вниз.
    ...
    - `import` - [необязательно] - если кнопка импорта должна отображаться. Импорт из csv-файла.
...
- `fileSelector` (только Admin6)
    - `pattern` - Шаблон расширения файла. Разрешено `**/*.ext` также отображать все файлы из подпапок, `*.ext` для отображения из корневой папки или `folderName/*.ext` для отображения всех файлов в подпапке `folderName`. По умолчанию `**/*.*`.
...
- `port` - Специальный ввод для портов. Он автоматически проверяет, используется ли порт другими экземплярами, и отображает предупреждение.
  - `min` - минимально разрешенный номер порта. Это может быть 0. А если тогда значение равно нулю, то проверка занятости порта не произойдет.
...
- `hideOnlyControl` - если скрыто, место будет показано, но не будет элемента управления.
