...
Дополнительные примеры можно найти во многих других адаптерах на GitHub в соответствующем каталоге администратора.
...
Для создания сложных JSON-файлов вы можете включать другие JSON-файлы.
...
##### Показывать только объекты пользовательских настроек для конкретного адаптера (все экземпляры)
...
### `alive`
просто указание, активен ли экземпляр, и его можно использовать в состояниях «скрытый» и «отключенный» (не будет сохранено в конфигурации)

Просто текст: Экземпляр запущен, Экземпляр не запущен

| Недвижимость | Описание |
|----------------|-------------------------------------------------------------------------------------------------------------------------------------|
| `instance` | проверка активности экземпляра. Если не определено, будет использован текущий экземпляр. В тексте можно использовать шаблон `${data.number}`. |
...
| `textNotAlive` | текст по умолчанию — `Экземпляр %s неактивен`, где %s будет заменен на `ADAPTER.0`. Перевод должен существовать в файлах i18n |
...
| `window` | если `openUrl` истинно, это имя нового окна. Может быть перезаписано, если ответ содержит атрибут `window`. `this.props.socket.sendTo(adapterName.instance, command \|\| 'send', data, result => {});` |
| `icon` | если значок должен быть показан: `auth`, `send`, `web`, `warning`, `error`, `info`, `search`. Вы можете использовать значки `base64` (например, `data:image/svg+xml;base64,...`) или изображения `jpg/png` (заканчиваются на `.png`). (Если вам нужно больше значков, отправьте запрос через issue.) |
...
| `checkBox` | Если определено, будет отображаться флажок с указанным именем. Если флажок установлен, будет включена кнопка «Согласовано». |
...
| `showEnterButton` | Отображать кнопку «SET». В этом случае значение будет отправлено только при нажатии кнопки. Вы можете определить текст кнопки. Текст по умолчанию — «Set» (только для «input», «number» или «slider»). |
...
#### `confirm`
- `condition` - функция JS: true показать диалоговое окно подтверждения
