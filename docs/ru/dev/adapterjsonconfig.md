...
- `panel` - Вкладка с предметами
  - `icon` - вкладка может иметь значок (base64)
  - `label` - Метка вкладки
  - `items` - Объект `{"attr1": {}, "attr2": {}}...`
  - `складной` - возможно только в том случае, если он не является частью вкладок.
...
- `!icon` - значок base64
  ...
  - `maxWidth`
  - `maxHeight`
  - `crop` - если true, разрешить пользователю обрезать изображение (только для файлов без SVG)
...
- `sendto` — кнопка отправки запроса в экземпляр (https://github.com/iobroker-community-adapters/ioBroker.email/blob/master/admin/index_m.html#L128)
    ...
    - `icon` - если значок должен отображаться: `auth`, `send`, `web`, `warning`, `error`, `info`, `search`. Вы можете использовать значки `base64`. (Если вам нужно больше значков, запросите через Issue)
...
- `staticLink` — статическая ссылка
    ...
    - `icon` - если значок должен отображаться: `auth`, `send`, `web`, `warning`, `error`, `info`, `search`, `book`, `help`, `upload` . Вы можете использовать значки `base64` (начинайте с `data:image/svg+xml;base64,...`). (Если вам нужно больше значков, запросите через Issue)
...
- `значок` - base64 svg
- `!encrypted` - зашифровано значение или нет (естественно только для текстов)
  - если зашифровано, используйте значение `__encrypted__` для отображения, а если оно было изменено, зашифруйте его с помощью `socket.encrypt`
