 ...
 * для номеров **** указанными `common.min`** /или** common.max: допустимый диапазон номеров определяется min/max, этот атрибут содержит список "специальных" числовых значений и их ( отображается) пометить как объект, например `{0: 'OFF', 254: 'ON', 255: 'BLINK'}` (min=0, max=255). Разрешается указывать только min **или** max, тогда отсутствующий предел принимается как +/- бесконечность (+/- бесконечность не включена)
  ...
  * Эти значения в настоящее время (начиная с js-controller 4.0) не проверяются и не подтверждаются js-контроллером и предназначены только для пользовательских интерфейсов и визуализаций.
...
* `common.allowInit` - [true/false] разрешить вызов "запланированного" адаптера "не по расписанию", если настройки изменились или адаптер запущен. Или разрешить запуск адаптера по расписанию один раз после изменения конфигурации, а затем по расписанию.
