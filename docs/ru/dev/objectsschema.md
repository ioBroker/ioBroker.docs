...
Регулярное выражение, которое использовалось для проверки символов, запрещенных для использования в идентификаторах, можно найти в https://github.com/ioBroker/ioBroker.js-controller/blob/master/packages/common/lib/common/tools. js#L44.
...
Каждое *состояние* должно быть представлено объектом типа `state`, содержащим метаданные для состояния. Смотри ниже.
...
* `device` - объект для группировки одного или нескольких каналов или состояний. Не должен иметь родителя, кроме пространства имен экземпляра адаптера.
...
* `common.states` (необязательный) — предоставляет больше контекста для допустимых значений для состояний с типами данных string и number:
  * для чисел **без** предоставленных common.min/common.max: содержит список допустимых числовых значений и их (отображаемую) метку как объект в форме `{0: 'OFF', 1: 'ON', '- 1': 'что угодно'}`. Допустимы только эти значения
 * для номеров **** указанными `common.min`** /или** common.max: допустимый диапазон номеров определяется min/max, этот атрибут содержит список "специальных" числовых значений и их ( отображается) пометить как объект, например `{0: 'OFF', 254: 'ON', 255: 'BLINK'}` (min=0, max=255). Разрешается указывать только min **или** max, тогда отсутствующий предел принимается как +/- бесконечность (+/- бесконечность не включена)
  ...
  * для строк содержит список допустимых значений в виде массива вида `['Старт', 'Полет', 'Приземление']` (что фактически совпадает с `{'Старт': 'Старт', 'Полет': ' Полет», «Земля»: «Земля»}`). Допустимы только эти значения
  * Эти значения в настоящее время (начиная с js-контроллера 4.0) не проверяются и не подтверждаются js-контроллером и предназначены только для пользовательских интерфейсов и визуализаций.
...
предложение: объекты-каналы `common.role` должны/могут подразумевать набор обязательных и/или необязательных дочерних объектов-состояний

возможные значения:

* `info` - курс валюты или акций, цены на топливо, ввод почтового ящика и тому подобное.
* `календарь` -
...
| государство | переключатель | Х | Х | логический |
...
* `common.adminUI.config` — [none/json/materialize/html] тип пользовательского интерфейса конфигурации. Если он не определен, адаптер будет отображаться как html. (`jsonConfig.json` или `jsonConfig.json5` от `json`, `index_m.html` от `materialize`, `index.html` от `html` ожидаются в папке `admin`)
* `common.adminUI.custom` — [none/json] тип пользовательского пользовательского интерфейса конфигурации. Если не определено, пользовательский интерфейс отображаться не будет. Можно использовать только `jsonCustom.json` или `jsonCustom.json5` в папке `admin`.
...
* `common.dataSource` - Как данные будут получены с устройства: `опрос/пуш/предположение`. Это важно вместе с `connectionType`.
...
* `common.logTransporter` - если этот адаптер получает логи от других хостов и адаптеров (например, чтобы хранить их где-то)
...
* `common.messagebox` - true, если окно сообщений поддерживается. Следовательно, адаптер может получать сообщения sendTo (используемые для электронной почты, pushover и т. д.).
...
* `common.nogit` — если это правда, установка напрямую с GitHub невозможна.
...
* `common.platform` — **обязательные** возможные значения: Javascript/Node.js, ожидается больше
* `common.preserveSettings` - строка (или массив) с именами общих атрибутов экземпляра, которые не будут удалены. Например. "история", поэтому с помощью `setState("system.adapter.mqtt.0", {..})` поле `common.history` не будет удалено, даже если новый объект не имеет этого поля. Чтобы удалить атрибут, это должно быть явно сделано с помощью `common: {history: null}`.
...
* `common.statusStates` - Структура для индикации статуса в админке в форме `"statusStates": {"onlineId": "0.connected", "errorId": "hm-rpc.0.AB203424.0.error"}` . Вместо `onlineId` можно использовать `offlineId`. Если идентификатор очень короткий (менее 2 точек), то идентификатор будет рассматриваться как относительный к текущему объекту.
...
* `common.visWidgets` — описывает `виджеты реагирования vis2`. Как `{"i18n": "component", "vis2NAMEWidgets": { "name": "vis2NAMEWidgets", "url": "vis-2-widgets-NAME/customWidgets.js", "components": [ "NAMEwidgetName" ]} }`
...
* `common.welcomeScreen` - массив страниц, которые должны отображаться на "веб-странице" index.html. `["vis/edit.html", "vis/index.html"]` или `[{"link": "vis/edit.html", "name": "редактор Vis", "img": "vis /img/edit.png", "color": "синий"}, "vis/index.html"]`
...
* `encryptedNative` - массив атрибутов конфигурации, которые будут автоматически зашифрованы при сохранении через страницу конфигурации администратора и автоматически расшифрованы во время работы адаптера, например. `["пароль", "токен"]`
...
* `extension` - этот адаптер будет запускаться не `js-контроллером`, а веб-экземпляром. Веб-экземпляр может быть определен в `native.webInstance` как '*' (если в каждой сети) или как `web.x` для конкретного веб-экземпляра. (Примеры: `камеры, прокси`). Кроме того, в `common.webExtension` должен быть указан путь к файлу плагина.
