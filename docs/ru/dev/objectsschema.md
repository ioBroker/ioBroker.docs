...
* `instance` - экземпляр адаптера. Родитель должен относиться к типу адаптера.
...
* `common.workingID` (строка, необязательно) - если это состояние имеет вспомогательное состояние РАБОТА. Здесь должно быть указано полное название или только последняя часть, если первые части совпадают с фактическими. Используется для HM.LEVEL и обычно имеет значение "РАБОТАЕТ".
...
##### Штат `common.history`
Для функции истории требуется адаптер истории или любой другой адаптер хранения типа history.

При достижении максимума длина фифо уменьшается до минимума. установите значение null или оставьте значение undefined, чтобы использовать значения по умолчанию
...
* `light.color.hsl` - Установить цвет в Hue / Saturation / Luminance (Цветовой оттенок света - LivingColors ...)
...
* `common.allowInit` - [true / false] разрешить" запланированный "адаптер вызываться" не по расписанию ", если настройки изменены или адаптер запущен. Или разрешите запуск адаптера по расписанию один раз после изменения конфигурации, а затем по расписанию.
* `common.availableModes` - значения для` common.mode`, если возможно более одного режима
* `common.blockly` - [true / false], если в адаптере есть пользовательские блоки для блочного. (требуется admin / blockly.js)
...
* `common.logTransporter` - если этот адаптер получает журналы с других хостов и адаптеров (например, чтобы хранить их где-нибудь)
...
* `common.platform` - **обязательный** возможные значения: Javascript / Node.js, скоро будут
...
* `encryptedNative` - массив атрибутов конфигурации, которые будут автоматически зашифрованы при сохранении на странице конфигурации администратора и автоматически расшифрованы во время выполнения адаптера, например `[" пароль "," токен "]`
...
* `common.engine` - (необязательно) *экземпляр движка скрипта* который должен запускать этот скрипт (например, 'javascript.0') - если не указано, движок выбирается автоматически
