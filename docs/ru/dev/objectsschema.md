...
* `common.noConfig` - [true/false] например, не показывать диалоговое окно конфигурации
...
* `common.statusStates` - Структура для индикации статуса в админке в форме `"statusStates": {"onlineId": "0.connected", "errorId": "hm-rpc.0.AB203424.0.error"}` . Вместо `onlineId` можно использовать `offlineId`. Если ID очень короткий (менее 2 точек), то ID будет считаться относительным для текущего объекта.
