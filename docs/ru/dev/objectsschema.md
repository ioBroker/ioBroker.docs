...
* `common.custom` (необязательно) - структура с пользовательскими настройками для конкретных адаптеров. Например, `{"influxdb.0": {"enabled": true, "alias": "name"}}`. Атрибут `enabled` является обязательным, и если он не равен true, весь атрибут будет удален.
...
* `light.color.hsl` - Установить цвет в Оттенке/Насыщенности/Яркости (Оттенок цвета света - LivingColors...)
...
* `common.messagebox` - true, если поддерживается message box. Следовательно, адаптер может получать sendTo сообщения (используется для email, pushover,...)
...
* `common.statusStates` - Структура для индикации статуса в админке в форме `"statusStates": {"onlineId": "0.connected", "errorId": "hm-rpc.0.AB203424.0.error"}`. Вместо `onlineId` можно использовать `offlineId`. Если ID очень короткий (менее 2 точек), то ID будет рассматриваться как относительный к текущему объекту.
