...
hash: oAyQ9vKMDO3DXqf34PzY3Hyx0hMDapDz3tkFDQZ31eE=
---
...
* `level.current.min` - минимальный ток для зарядки аккумуляторных устройств.
...
* `indicator.maintenance.waste` - Мусорная корзина дурацкая.
...
* `weather.icon` — URL-адрес значка текущего состояния на данный момент.
