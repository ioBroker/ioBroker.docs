...
hash: O7OSlUwGIrI165MnCgADCT+EoYM+shIZyg2pajrTo8o=
---
...
* `value.timer` - длительность в секундах (или другой распространенной единице времени) (эквивалент `level.timer`)
* `value.interval` (common.unit=`s`, `m`, `h`, `ms` или `d`) - Интервал в секундах (или другой распространенной единице времени) (может быть 0,1 или меньше)
...
* `level.timer.off` - время в секундах (или другой распространенной единице времени), по истечении которого устройство снова выключается, например, время включения лампы или розетки.
* ...
...
* `media.duration` - (`common.type=number`) секунд (или другая распространенная единица измерения времени)
* `media.elapsed.text` - например, "1:30"
* `media.elapsed` - (`common.type=number`) секунд (или другая распространенная единица времени)
...
* `dayofweek.forecast.0` - день недели сегодняшнего дня в текстовом формате
...
* `time.span` - разница во времени в миллисекундах (или другой распространенной единице измерения времени) (common.type=number), т. е. время с момента последнего обновления, продолжительность операции, время до следующей попытки и т. д.
* `time.interval` - значение интервала в миллисекундах (или другой распространенной единице измерения времени) (common.type=number), т.е. некоторый интервал опроса.
