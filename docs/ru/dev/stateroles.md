...
* Параметр `switch.setting` (Вкл./Выкл.) может использоваться для определения параметра, который можно включить или выключить (например, функция блокировки от детей).
...
### Общие состояния
Если не удается найти подходящую роль или сценарий использования не является специфическим, вы можете использовать описанные ниже **общие** роли.
