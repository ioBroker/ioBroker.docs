...
hash: 2X7/inue1TdfVdFCXjpZY38F0SedQN63dzDqF9Si8Cc=
---
...
Рабочие состояния используются для управления нормальной функциональностью устройства. Лампа RGB может иметь следующие три объекта (или более) с различными ролями, которые связаны друг с другом:

* `переключатель` (Вкл/Выкл)
* `level.color.rgb` с цветовым кодом лампы #RRGGBB
* `level.brightness` со значением яркости

Также режим уборки или уборка комнаты робота-пылесоса является таким рабочим состоянием. Эти состояния используют приведенное ниже определение без каких-либо корректировок.

Пожалуйста, используйте максимально подробное имя роли, которое предоставляет больше всего информации (например, `level.color.temperature` следует использовать вместо `level` для цветовой температуры, или `switch.power` лучше, чем `switch` для управления питанием устройства). Кроме того, при использовании подробных имен ролей (более одного уровня) важно не использовать одну и ту же роль дважды в канале устройства.
...
Состояния, которые настраивают дополнительные «нерабочие» параметры устройств, также могут использовать приведенные ниже определения основных ролей, чтобы предоставить больше контекста типа и использования предоставленного значения, **но добавьте «.setting.» в качестве второго уровня имени роли**. Например:

* `level.setting.color.temperature` с числом 0..100% можно использовать для установки «Цветовой температуры запуска» лампочки
...
Обратите внимание: эти типы ролей были определены в июне 2025 года, поэтому многие (старые) адаптеры могут их не использовать. В будущем этот тип состояния может/должен использоваться при необходимости.

### Общие состояния
Если не удается найти подробную соответствующую роль или вариант использования не является конкретным, вы можете обратиться к нам за помощью, используя определенные ниже **Общие** роли.
