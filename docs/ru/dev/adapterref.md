...
На этой странице предполагается, что фреймворк уже существует. Точка входа находится в [обзор](/docs/dev/adapterdev.md), файл `io-package.json` - в [io-package.json](/docs/dev/iopackage.md), а структура самих объектов - в [основная концепция](/docs/dev/objectsschema.md).
...
Для каждого экземпляра контроллер JavaScript создает следующие параметры: `alive`, `connected`, `uptime` и три состояния памяти в разделе `system.adapter.<name>.<instanz>`.

Эти параметры опускаются в режимах работы `none` и `once`.
...
| `objectChange(id, obj)` | Подписанный объект изменился. `obj` становится `null` при удалении. |
...
Подписка не предоставляет начального значения, а только изменяется. Начальное значение можно найти в `onReady`.
...
`getStatesAsync('muster*')` возвращает несколько записей одновременно, `getForeignStatesAsync` возвращает одну и ту же запись при переходе между экземплярами. Заполнители используются только для форм множественного числа.
...
`expire` используется самим js-контроллером для `alive`: если экземпляр не отвечает в течение 30 секунд, он считается остановленным.
...
Проще всего создать состояние в `instanceObjects` в `io-package.json`, тогда оно будет создаваться автоматически с каждым новым экземпляром.
...
Это включает в себя `clearInterval` и `clearTimeout` одного и того же класса. Обработчик `unload` очищает данные, а затем вызывает `callback()`. Без этого контроллер JavaScript внезапно завершает работу экземпляра после периода ожидания.
...
* `--install`: запускает адаптер даже без настройки для
