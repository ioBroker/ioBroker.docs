...
Объекты и текущие значения статуса можно просмотреть в адаптере администратора на вкладке «Объекты».

Название объекта состоит из разных частей. Каждая часть обозначена "." отделены друг от друга. Есть системные объекты (имя начинается с _ или "system.") И объекты адаптера (имя начинается с adapterName).

?> Примечание. Здесь в описании **имя адаптера** обозначает имя адаптера, который разработчик хочет создать.

Состояния можно сгруппировать по каналам и по каналам в устройствах. Вот пример Homematic групп и каналов:
...
Идентификатор состояния всегда должен начинаться с имени канала, а имя канала - с имени устройства. Например, в названии статуса выше hm-rpc.0.IEQ1234567.0.INFO часть hm-rpc.0.IEQ1234567.0 - это имя канала, а hm-rpc.0.IEQ1234567 - имя устройства.
...
Объект конфигурации для каждого экземпляра находится в хранилище данных под идентификатором system.adapter.adapterName.X, где X - номер экземпляра адаптера. Он содержит настройки для этого экземпляра адаптера. Обычно он состоит из «общих» и «собственных» настроек. Общие настройки:
...
Все идентификаторы объектов экземпляра адаптера начинаются с имя адаптера.X, где X - номер экземпляра адаптера.
...
?> Примечание: lib / utils.js - это общий файл для всех адаптеров, с помощью которого можно определить положение контроллера js и соответствующий путь к iobroker.js-controller / lib / adapter.js. Большинство текущих utils.js можно скачать здесь. Не меняйте этот файл!
...
Например, адаптер `hm-rpc` создает для каждого экземпляра статус `updated`, чтобы передать другому адаптеру сигнал о том, что в хранилище данных появятся новые устройства и что они должны быть обработаны §§SSSSS_2 §§.
...
### Package.json
package.json - стандартный файл описания для пакета npm. Полное описание можно найти на https://docs.npmjs.com/files/package.json.
...
### Предоставлять
Рекомендуется разместить код на Github. После того, как код станет стабильным и адаптер может быть установлен, его можно использовать совместно с другими пользователями, попросив их установить адаптер следующим образом:
...
Его можно вызвать в каталоге адаптера. Убедитесь, что все другие файлы, кроме необходимых, были удалены (например, `.idea`), или добавьте к файлу `.gitignore`.
...
На https://github.com/ioBroker/ioBroker.template вы можете найти несколько шаблонов для вашего собственного адаптера.
...
* `--force` - запускает адаптер, даже если он деактивирован в конфигурации
...
* `log` - объект логгера
* `version` - версия адаптера
* `States` - (только для экспертов)
* `Объекты` - (только для экспертов)
* connected - если адаптер подключен к хосту
...
Опрос Для того, чтобы прочитать свои собственные состояния в начале или прочитать значения с интервалом, используйте функцию `adapter.getState`, как здесь:
...
Это изменение не выполняется адаптером hm-rpc, потому что ack имеет значение true. И это подтверждение от физического устройства.
...
`adapter.setState('myState', 1, true);` // показать новый статус вашей страны
...
* `expire`: (необязательно) Можно установить лимит времени истечения в секундах. По истечении этого времени переменная устанавливается на «ноль». Это, например, через «активные» состояния экземпляров адаптера. Если экземпляр адаптера не переходит в активный статус в течение 30 секунд, он помечается как «неработающий». Чтобы установить статус с истечением срока действия, используйте следующий код setState ('Variable', {val: true, expire: 30})
...
* `Schedule` - запускается согласно расписанию, найденному в system.adapter ... common.schedule - реагирует на изменения в .schedule перепланированием с новым статусом
...
ExtensObject только читает объект, сливается с конкретным объектом и записывает объект обратно.
...
#### Info.connection
Когда адаптер устанавливает соединение и осуществляет мониторинг (например, с управляемым устройством), он должен создать переменную `info.connection` и ждать.
...
* Флаги для приложения: `--force, instance, log level` (вы можете запустить адаптер как` node xxx.js 1 Debug --force`, 1 - индекс экземпляра (по умолчанию 0, отладка - уровень журнала и` - -force` означает , что настройки «включены: ложь», игнорируется).
