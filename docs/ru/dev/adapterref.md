...
Для каждого экземпляра объект конфигурации находится в хранилище данных под идентификатором «system.adapter.adapterName.X», где X - номер экземпляра адаптера. Он содержит настройки для этого экземпляра адаптера. Обычно он состоит из «общих» и «собственных» настроек. Общие настройки:
...
?> Примечание: одну и ту же версию кода нельзя опубликовать дважды Поэтому увеличьте версию в `package.json` и `io-package.json` перед публикацией.
...
* `lc`: отметка времени последнего изменения. Тот же формат, что и ts, но отметка времени значения изменяется. Значение может быть обновлено, но значение может оставаться прежним. В этом случае lc не меняется.
...
* `Расписание` - запускается в соответствии с расписанием, найденным в system.adapter ... common.schedule - реагирует на изменения в .schedule путем перепланирования с новым статусом
