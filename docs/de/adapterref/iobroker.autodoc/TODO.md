...
editLink: https://github.com/ioBroker/ioBroker.docs/edit/master/docs/de/adapterref/iobroker.autodoc/TODO.md
title: AutoDoc-Adapter - TODO-Liste
...
- [ioBroker.repositories - Best Practices](https://github.com/ioBroker/ioBroker.repositories#development-and-coding-best-practices) - Zuhören, Einreichen, Programmierkonventionen
...
- [type-detector](https://github.com/ioBroker/ioBroker.type-detector) - State-Rollen / belegen
...
## Übersicht - Umsetzung vs. Rest (Stand der **Version** wie in `package.json` / `io-package.json`: derzeit ** `0.9.48` ** auf ** `main`/`dev` ** - ** `npm` Latest** gegen [npm - iobroker.autodoc](https://www.npmjs.com/package/iobroker.autodoc) prüfen; bei Releases diese Zeile + **Tabellen-Stichtag** mitziegen; Veröffentlichungen Version/`news`/Tag/GitHub wie in **[CONTRIBUTING](/#/docs/adapterref/iobroker.autodoc/CONTRIBUTING.md#maintainer-checklist-release-order)**; Tabellen-Stichtag **2026-09-22** - Branch je nach Arbeitskopie, z. B. `main` / `dev`
| Thema | Status | Kurz |
| ----- | ------ | ---- |
| Phasen **1-4** (Basis … Profil-Redesign) | ✅ | Modular, drei Profile, Discovery, Renderer, i18n EN/DE/FR, … |
| **Delta seit letztem Lauf** (Admin optional ausblendbar; Changelog-Zusammenfassung in **Export-Sprache**; User Kurzinfo bei echten Inventar-Deltas) | ✅ | ** `hideAdminDeltaSinceLastRun` **, `localizeCompareSummary` / `docChangeFormat`, Benutzer HTML+MD - ** `dev` **; README |
| **0.9.x** RC-Funktionen (Aliase, Diagnose, QR/Copy, `exportPath`, …) | ✅ | Siehe README-Changelog |
| **KI** (Anbieter, Tab `hidden`, Timeouts, Temperaturen, **AI-Kontexthinweise**, `guestHelpNote` / `homeRoutinesNote` / `ownerPlaybookNote`) | ✅ | Sprachqualität kleiner Modelle bleibt iterativ |
| **Benutzerdefinierte Vorlagen** (Kapitel ausblenden, Custom-Sections, Theme-Teile) | 🟡 | [PLAN.md - Benutzerdefinierte Vorlagen](/#/docs/adapterref/iobroker.autodoc/PLAN.md#custom-templates-detail); **0.9.17:** Admin-Reihenfolge + **Farb-Presets** (ohne Roh-CSS); **main:** Reihenfolge **User/Onboarding** (`userChapterOrderJson`, `onboardingChapterOrderJson`); **0.9.33:** PDF; offen: DnD |
| **Admin-HTML Lesbarkeit** (lange Listen eingeklappt; Score ohne „Strafe“ für bewusst deaktivierte Instanzen) | ✅ | In **0.9.17** README-Changelog (Block „Auch auf `dev`…“) + Feature-Bullets oben drunter |
...
| **Phase 5** (PDF, Backup-Adapter, Restliche benutzerdefinierte Vorlagen) | 🟡 | **PDF 0.9.33** ([§ 1.2a](#phase-5-pdf-offline-mermaid)); **Backup** bewusst **zurückgestellt** (Resonanz nach **latest**-Repo - [§ 1.2](#phase-5-features)); DnD offen |
...
| **Phase 5.x.2** Quick Start / Raumguides | 🟡 | Kern **0.9.20**; Gäste-Kürzung **0,9,26**; Feinschliff Sortierung/Caps/Automations-Zähler **0.9.48** - optional: weitere Caps/i18n - [§ 1.3 - 5.x.2](#phase-5x) |
| **Phase 5.x.3** Meerjungfrau | ✅ | **0.9.27** kuratiert; **0.9.28** Auto-Topologie; **0.9.48** klarere Linux/Chromium-Hinweise + Hilfsskript |
| **System-Visitenkarte** / Forum-Copy | ✅ | `textSendTo` **getForumCard** + Status `info.forumCardPlain`; Diagnose-HTML nutzt `forumCard.js` |
...
| **npm + ioBroker.repositories** | 🟡 | **npm** ** `0.9.48` ** ✅ (Trusted Publishing / **E2008** seit **0.9.48**). **aktuell:** Eintrag in ** `sources-dist.json` ** fehlt (**W4001**) - PR bei Bedarf. **stable:** zurückgestellt (**[#54](https://github.com/crunchip77/ioBroker.autodoc/issues/54)**). Tag + GitHub Release: **[CONTRIBUTING](/#/docs/adapterref/iobroker.autodoc/CONTRIBUTING.md#maintainer-checklist-release-order)** |
| **Dokumentations-Score** (Wartung - Checkliste mit echten Kriterien) | ✅ | **0.9.30-0.9.31:** Dreiteiliger Score (Datenerfassung / Manuelle Inhalte / Dokumentationstiefe); instancesWithoutRoom komplett raus; neue Checks: Diagramm, Räume m. Geräte, eigene Kapitel, KI-Provider - [§ 1.6](#dokumentations-score-checkliste) |
| **Admin-Markdown: Diagnose-Kapitel** (Parität zu Admin-HTML) | ✅ | `renderDiagnosis` / `renderDiagnosisMarkdown` - TOC-Zeile, ausblendbar wie HTML; **dev:** Kapitel-Einleitung „Schnappschuss“; **Automatische Prüfungen** (Node-Heuristik) getrennt von **Allgemeine Erinnerungen** (OS-Hinweise). Details [§ 1.6](#admin-markdown-diagnose-optional), AnhangA |
...
| **Nutzer-Handbuch / Wiki** (`docs/user-guide/`) | 🟡 | EN/DE ** `README` ** + **Wiki Schritt6** (Ausblenden/Reihenfolge); Admin ** `staticLink` **‑Paar unter Kapitelsichtbarkeit (JSON‑Kochbuch + Wiki); PNG + **SCREENSHOTS.md** - [§ 1.1b](#nutzer-handbuch-wiki); optional: weitere Sprachen |
| **Admin-UI React** (statt/nur wo nötig neben `jsonConfig`) | *optional* | Nur wenn jsonConfig für geplante Features zu eng ist - [§ 1.8](#admin-react-optional) |
| **Admin-UI React** (statt/nur wo nötig neben `jsonConfig`) | *optional* | Nur wenn jsonConfig für geplante Features zu eng ist - [§ 1.8](#admin-react-optional) |
...
| # | Thema | verweisen |
| - | ----- | ------- |
| 1 | **Benutzerdefinierte Vorlagen - Rest (0.9.17 / `main`)** | Erledigt: `adminChapterOrderJson`, `htmlThemePreset` - weiter: **Phase 5** (PDF, Benutzer/Onboarding, DnD) - [PLAN](/#/docs/adapterref/iobroker.autodoc/PLAN.md#custom-templates-detail), [§ 1.2](#phase-5-features) |
| 3 | **Phase 5.x.2** Quick Start / Raumguides | 🟡 Kern **0.9.20**; Feinschliff **0,9,48**; optional mehr: [§ 1.3 - 5.x.2](#phase-5x) |
| 4 | **Phase 5.x.3** Meerjungfrau (gestaffelt) | ✅ **Stufe 1** **0,9,27**; **Stufe 2** **0.9.28** (`autoMermaidHostGraph`): [§ 1.3 - 5.x.3](#phase-5x) |
| 5 | **Phase 5:** PDF ✅ - **Sicherung** / **Benutzerdefinierte Restvorlagen** (DnD) | Backup **zurückgestellt** bis User-Resonanz nach Eintrag in **latest** (Repo); DnD weiter offen - [§ 1.2](#phase-5-features), [Backup/Backitup](#backup-backitup-festlegung) |
| 6 | **npm** ✅ **0,9,48** (Herkunft). **neueste** 🟡 Repos-PR. **stabil** ⬜ später | [§ 1.1](#release-veroeffentlichung), [CONTRIBUTING.md](/#/docs/adapterref/iobroker.autodoc/CONTRIBUTING.md) |
...
- **Typische Archive:** [ioBroker.backitup](https://github.com/simatec/ioBroker.backitup) erzeugt u. A. ** `*.tar.gz` ** (z.B. `iobroker_…_backupiobroker.tar.gz`, `javascripts_…` - vgl. `lib/list.js` im Backitup-Repo). **Nicht nur ZIP** - AutoDoc-Parser/Doku müssen **tar.gz** abbilden, sobald das Backup verarbeitet wird.
...
> **npm:** Das öffentliche Paket heißt **[** `iobroker.autodoc` **](https://www.npmjs.com/package/iobroker.autodoc)** (`package.json` → ** `name` **). Hosts können den Tarball ohne Git installieren. **Standard-Adapterlisten** nutzen **[ioBroker.repositories](https://github.com/ioBroker/ioBroker.repositories)**. **neueste:** PR **#5978** Krieg zusammengeführt; ** `autodoc` fehlt derzeit in `sources-dist.json` ** - **W4001** ist deshalb wieder aktiv, der Checker behandelt den Adapter als **neu** (Warnungen → Fehler). **stabil:** **nicht** beantragen, solange Tester fehlen (**#54**). Tags/Veröffentlichungen: **[Beitrag](/#/docs/adapterref/iobroker.autodoc/CONTRIBUTING.md#maintainer-checklist-release-order)**.
...
- [ ] [Adapter Checker](https://adapter-check.iobroker.in/) **ohne vermeidbare Fehler** - **E6034/E4052** ✅ **0.9.47**; **E2008** ✅ **0,9,48**; **W4001** bis **neuester**-Wiedereintrag. **W5042** bewusst: **[CONTRIBUTING - Optional Puppeteer + mermaid-cli](/#/docs/adapterref/iobroker.autodoc/CONTRIBUTING.md#optional-puppeteer-mermaid-cli)**
...
- [x] `npm publish` über GitHub ** `deploy` ** (Vertrauenswürdige Veröffentlichung)
...
> ** `docs/user-guide/` ** (`README.md` **EN**, ** `README.de.md` ** DE als **Wiki zu Admin-Tabs** + Muster-Szenario): **Wiki Schritt6** (`#wiki-admin-json-cookbook`) für **Ausblenden/Reihenfolge**-Rezepte; **engl.** Haupt-`README.md` für **JSON/Mermaid/CSS**‑Kochbücher - **Verbund** zur Instanzkonfiguration über ** `staticLink` ** (u.a. Tab **HTML-Export**, Abschnitt Kapitelsichtbarkeit) **und** Feldhilfen; **Screenshots**/Unterschriften siehe ** `assets/SCREENSHOTS.md` **; optional: weitere Sprachen.
...
### 1.2 Phase 5 - Funktionen (Wünschenswert)
...
- [x] Benutzerdefinierte Vorlagen - **Reihenfolge Benutzer/Onboarding** (`userChapterOrderJson`, `onboardingChapterOrderJson`) - [PLAN.md - Benutzerdefinierte Vorlagen](/#/docs/adapterref/iobroker.autodoc/PLAN.md#custom-templates-detail)
- [ ] Benutzerdefinierte Vorlagen - **Rest**: Drag & Drop - [PLAN.md - Benutzerdefinierte Vorlagen](/#/docs/adapterref/iobroker.autodoc/PLAN.md#custom-templates-detail)
...
- [x] **SVG-Rendering** bei Generierung über ** `@mermaid-js/mermaid-cli` ** (`lib/mermaidServerSvg.js`, optionalDependency) - pro `<pre class="mermaid">` in den drei HTML-Profilen; **hell/dunkel** aus `htmlColorScheme` (**auto** → Default-Theme); ohne Paket bleibt **jsDelivr**-Mermaid wie bisher.
...
1. **Admin:** unter „Meine Dokumentation“ ** `manualMermaidDiagram` ** z. B. „Flussdiagramm LR\n A-->B“ eintragen; optional ** `autoMermaidHostGraph` ** aktivieren.
...
- **Symptom A - fehlende Bibliotheken:** `Failed to launch the browser Process` und z. B. ** `libnss3.so: shared object file kann nicht geöffnet werden` ** - **gebündeltes Chromium** benötigt **Distro-Pakete** (häufig bei schlanken Images).
- **Symptom B - Docker / Unraid / LXC:** **`Keine verwendbare Sandbox** / **SUID-Sandbox** - der Kernel/Namespace erlaubt die **Chrome-Standard-Sandbox** oft nicht.
...
- [x] **Client-Mermaid (jsDelivr):** Nur noch **Fallback** im **HTML**-Export - Skript wird nur eingefügt, wenn noch ** `<pre class="mermaid">` ** vorkommt (**0.9.34**); Vollständig eingebettetes SVG lädt kein CDN. **Markdown** unveränderte Fences mit Quelltext.
...
#### 5.x.1 Notfall & Fehlerbehebung (Hybrid)
**Bereits da (abgrenzen):** Felder manuell **„Hilfe & Notfälle“** / **„Routines in deinen eigenen Worten“** / **„Playbook“** (`guestHelpNote`, `homeRoutinesNote`, `ownerPlaybookNote`, u. a. 0.9.9 / 0.9.25) - Freitext in Onboarding/User, keine erfundene Diagnose.

Noch offen (größere Ausbaustufe als reiner Freitext):

- [x] Strukturierter Block **über** reine Notizfelder hinaus: **Kurzzeilen** (WLAN/Strom/Wasser/Sonstiges) + **automatische Doku-Links** (User/Onboarding/Admin, Logik gleiche wie QR - **0.9.18**)
...
- [x] **dev (nach 0.9.39):** **Delta seit letztem Lauf** - optional ausblendbar (`hideAdminDeltaSinceLastRun`); **Changelog**-/Vergleichs-Einzeiler in Export-Sprache; **Benutzer** Kurzinfo bei echten Deltas (HTML+MD); Testet „docChangeFormat.test.js“.

#### 5.x.2 Schnellstart & Raumguides
...
- [x] **0.9.48:** Quick-Start-Caps (Skripte/Funktionen); Raumsortierung nach deduplizierter Gerätezahl; Automatisierungs-Zähler ohne Gäste-Skriptnamen
...
**Hinweis (Admin-UI-Sprache):** **Voll geprüft** (inhaltlich): **EN / DE / FR**. Für **es, it, nl, pl, pt, ru, uk, zh-cn** sind **alle** `jsonConfig`-Keys mit Text aus ** `en.json` ** hinterlegt, bis Muttersprachler nachziehen; Falsche „Übersetzungen“ also vermeidbar. Beiträge: [`CONTRIBUTING.md` - Admin-UI i18n](/#/docs/adapterref/iobroker.autodoc/CONTRIBUTING.md#admin-ui-translations-i18n).

#### 5.x.3 Meerjungfrau / kleine Graphen
- [x] Stufe 1: Mermaid aus **kuratiertem** Inhalt (`manualMermaidDiagram` → `manualContext.mermaidDiagram`, **0.9.27**)
- [x] Ausgabe **Markdown** (`mermaid` fence) + **HTML:** mit installierter CLI **eingebettetes SVG** (`mermaid-svg-embedded`); sonst **pre.mermaid** + Mermaid **10.9.1** von jsDelivr wenn Diagramm vorhanden
...
- [x] **System-Visitenkarte:** **jsonConfig** `textSendTo` `getForumCard` (+ Copy); Geben Sie „info.forumCardPlain“ an; gemeinsame Logik `lib/forumCard.js`
...
- [x] **UX-Korrekturen 0.9.29:** `scriptsWithoutDescription` entfernt; `unassignedCount` nur aktive Instanzen; Adapter-Grid-Benutzeransicht; Neu-Rendering im Meerjungfrauen-Dunkelmodus.
- [x] **Dreiteiliger Score 0.9.30:** Score 1 = Datenerfassung; Partitur 2 = Manuelle Inhalte; Note 3 = Dokumentationstiefe. Gesamtpunktzahl = Durchschnitt. HTML + Markdown, i18n EN/DE/FR.
- [x] **Score 3 Rework 0.9.31:** `instancesWithoutRoom` vollständig aus Score entfernt; ersetzt durch `checkHasCustomSections` und `checkAiConfigured` (bedingt). Admin-Config-Felder bleiben für Rückwärtskompatibilität ohne Effekt.
...
- [x] **Admin-Markdown: Diagnose-Kapitel:** `renderDiagnosisMarkdown` in `lib/markdownRenderer.js`; Parität zur Admin-HTML-`renderDiagnosis` (inkl. ausblendbar über versteckte Kapitel / TOC wie bei anderen Admin-Kapiteln). Verweis **AnhangA**.
...
- [x] **teilweise umgesetzt:** **Mermaid** (`manualMermaidDiagram`): **Placeholder** im Textfeld (neutral `DEMO*`, keine `native`-Defaults). **JSON-Felder** (**Kapitelreihenfolge / Ausblenden / Custom section**): **Placeholder** im UI + Verweis im Hilfetext **„Custom section (JSON)“** auf ** `docs/user-guide/README.de.md` ** (Musterszenario); weiterhin **kein** großes **``default `**-JSON in ` io-package ` ` native`.
- [x] **0.9.32:** Hilfetexte **Mermaid** (Client vs. eingebettetes SVG / Auto vs. Kurzdiagramm nach Bedarf); ** `documentation.exportHashes` **; Dokumentation **States** ** `vollständig` ** / ** `Metadaten` **; **KI**: Header/Blocke ohne Provider bzw. Skript-Analyse weniger grell.
- [x] **Weitere Mini-Beispiele** in `jsonConfig`/`i18n` (Projektname, Beschreibung/Score, Timer, Basis-URL, States-Modus-Tipp, Adapter-/Raum-Notizen, Benachrichtigungen, JSON **hide**-Felder, Logo-Pfad, Extra-CSS, KI-Kontext) - EN/DE/FR + englische Keys in den übrigen Locales (**Repo-Stand**, nicht eine einzelne npm-Version gebunden).
...
- [x] **Onboarding-Kapitelreihenfolge** + **Base-URL-Kontext:** `quickstart` vs. Discovery-Block (`quickStart.hasContent`) in Hilfetexten; **Advanced**-Hinweis‑Box für ** `baseUrl` ** (Gäste/QR/Docker/Proxy) - alle Locales für Callout (EN in ES/IT/…).
...
### 1.8 Admin-UI - Reagieren statt / neben `jsonConfig`? *(optional - Zukunft)*
...
- [x] **0.9.47:** Repository-Checker (#60): Lizenzlink, Maintainer-E-Mail, CI-Knoten 26
- [x] **0.9.48:** Admin **Automatisierung im Überblick**; Raum/Funktion-Gerätehierarchie (`lib/roomDeviceResolver.js`); Schnellstart-/Automatisierungs-Feinschliff; HTML einklappbar; Mermaid/Linux-Hinweise; npm Provenienz (**E2008**)
...
- [x] ** `main.js`:** nach `compareVersions` Summary für Snapshot/Changelog anders; `i18n.setLanguage(config.sprache)` für den Export
...
- [x] **Konfig / `native`:** `hideAdminDeltaSinceLastRun` (Standard false); Admin-i18n EN/DE/FR; `RENDERER_VERSION`-Bump bei HTML-Template

### Dev (nach 0.9.28) - Adapter-Ansicht, Kapitel-Reihenfolge, mermaidAuto ✅
- [x] **Adapter-Details Admin-HTML:** Tabelle → zugeklappter `<details>`-Block + responsives **Karten-Grid** (autofill, min. 300 px); Instanz-Details per Klick ausklappbar; JS-Filter auf Karten umgestellt; „enabledShort“ i18n (EN/DE/FR)
- [x] **Adapter-Details Admin-Markdown:** kompakte Übersichtstabelle (Name, Beschreibung, Badges, Aktiv/Gesamt) + Instanz-Details je aktivem Adapter + deaktivierte Adapter im `<details>`-Block; gesamter Abschnitt in „<details>“ zugeklappt (alle drei Profile); `mermaidAutoTopologyMdHint` i18n statt rohem Code-Block
- [x] **Benutzer-/Onboarding-Kapitelreihenfolge:** `userChapterOrderJson` + `onboardingChapterOrderJson` (analog zu `adminChapterOrderJson`); `parseUserChapterOrder` / `parseOnboardingChapterOrder` in `lib/docTemplateConfig.js`; `docModel.userChapterOrder` / `.onboardingChapterOrder`; HTML + Markdown Dispatcher; i18n EN/DE/FR
...
### Version 0.9.17 - Benutzerdefinierte Vorlagen (Rest, Teil) ✅
...
- [x] `io-package` native, `jsonConfig`, **i18n** EN/DE/FR; **Nachricht**; `RENDERER_VERSION` angehoben
...
- [x] Admin-Benutzeroberfläche: „jsonConfig.json5“ + i18n EN + DE + FR
- [x] Drei Profile: Admin, Benutzer, Onboarding
...
- [x] HTML-Renderer mit Sidebar, Stat-Cards, Adapter-Cards und Profil
...
### Phase 3 - Tiefe ✅
- [x] 3.1 Benachrichtigungen: `sendTo` nach Generierung (Telegram, Email, Pushover, generic)
- [x] 3.2 Abhängigkeitsanalyse: `lib/dependencyAnalyzer.js`, stateRefs + Querverweis
- [x] 3.3 AI-Enhanced Docs: `lib/aiEnhancer.js`, steckbare Anbieter (Anthropic/Groq/Ollama), Opt-in, Admin-Profil wird übersprungen
...
### Phase 4 - Profil-Neugestaltung ✅
#### 4.1 Discovery-Erweiterungen ✅
- [x] `system.config` auslesen: Stadt, Land, Systemsprache → `rawData.systemConfig`
- [x] Geräte-Namen-Auflösung: Raum-Mitglieder → Geräte-Objekte über `getForeignObjectAsync`
- [x] Opt-in Live-States: Schlüssel-Rollen (`level.temperature`, `sensor.door`, `sensor.window`, `alarm`) lesen
- [x] Neue Konfigurationsoption `readLiveStates` in `jsonConfig.json5` + i18n

#### 4.2 Rollenzuordnung ✅
- [x] `lib/roleMapper.js` - 29 Muster → 14 Kategorien + Symbole
...
- [x] Stadtbewusster Willkommenstext, Räume mit Device-Grid + Icons + Live-Values
- [x] „Was läuft automatisch?“ als einfache Sätze, Adapterkarten (freundlich)
- [x] AI-Box prominent, Hinweis wenn kein manueller Kontext vorhanden ist

#### 4.6 Benutzer-/Familienprofil ✅
- [x] Räume mit Device-Cards, Skripte nur Name+Beschreibung, Adaptertitel nur
...
- [x] Adapter-Badges: „none“/„annahme“ ausblenden, Tier mit Qualitätsbezeichnung
...
#### 4.x UI-Verbesserungen Sitzung 2 ✅
...
- [x] Optionale **Temperaturen** Benutzer vs. Onboarding (`jsonConfig`); leer = Anbieter-Default
- [x] **HTTP-Timeout** pro Anfrage konfigurierbar
...
### Architektur - Staaten entlasten ✅ (Umsetzung)
...
- [x] ** `documentationStatesMode` **: bis **0.9.38** `full` \| `metadata` (Standard ** `metadata` ** neu); ab **0.9.39** entfernt - immer ** `/files/` ** + Platzhalter-States
...
- [ ] **stabil** - **zurückgestellt** bis ausreichend Tester (**#54**)
