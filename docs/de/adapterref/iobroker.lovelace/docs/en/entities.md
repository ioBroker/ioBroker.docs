...
Ein Zustand vom Typ `array` oder `object` wird als echtes Array oder Objekt geliefert: ioBroker speichert diese als JSON-String, und der Adapter parst ihn, sodass Karten, die ein Array durchlaufen (z. B. flex-table-card), ihre Zeilen erhalten. `mixed` Der Zustand wird ebenfalls analysiert, wenn sein Wert wie JSON aussieht, andernfalls wird er unverändert weitergegeben.
...
Die Basiskarte selbst wird mit Kacheln der OpenStreetMap Foundation gezeichnet, die der Adapter abruft und für den Browser zwischenspeichert. Er benötigt ausgehenden Internetzugang auf dem ioBroker-Host, jedoch weder ein Konto noch einen API-Schlüssel. (Die Kacheln stammten früher von CARTO, das sie nun mit dem Wasserzeichen „API-SCHLÜSSEL ERFORDERLICH“ versieht.)
