...
The dev-server runs in the adapter's folder, i.e., where the`io-package.json` lies.
...
!> After a change to the`io-package.json` A restart is not enough.`dev-server upload` introduces new instance objects, changed configuration settings, or a new operating mode into the environment.
...
Both are already included in the Adapter Creator framework. See [io-package.json](/docs/dev/iopackage.md) for information on handling the translations and [adapter](/docs/dev/adaptertesting.md) tests, in addition to the included tests.