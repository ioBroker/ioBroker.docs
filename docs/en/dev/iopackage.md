...
Besides the usual `package.json` Each adapter contains a second file: the `io-package.json`It contains everything that **ioBroker** This information is needed about the adapter and is irrelevant to npm. It determines the entry in the adapter list, the instance objects, the icons on the tile, and the startup behavior.
...
| `instanceObjects` | Objects that are at **everyone** A new instance will be created.                 |
...
- `common.plugins.sentry`: Enables crash notifications. See
...
`common.dependencies` calls what is based on **the same** A host must be present, for example. `[{"js-controller": ">=5.0.19"}]`. `common.globalDependencies` calls what **somewhere** must run in the system, typically `[{"admin": ">=6.0.0"}]`.
...
Every password and every login credential belongs in **both** Listen:
`encryptedNative` **and** `protectedNative`The first setting ensures that the value is encrypted in the database, the second prevents any other adapter from reading it. Setting only one setting creates a vulnerability.
...
`common.messagebox: true` Creates a message box for each instance. Without this entry, no message will arrive. `sendTo` on. See
[Messages between instances](/docs/dev/messagebox.md).

## Translations

All multilingual fields are not maintained manually. The project includes...
`@iobroker/adapter-dev`, and the appeal

```bash
npm run translate
```

translates the new English texts from the `io-package.json` and from the i18n files into all supported languages. New texts will therefore be created. **English only** Enter the information; the command does the rest.

## The complete list

This chapter lists the fields you will need in everyday life. The complete list of all attributes and their special cases can be found in the...
[Object schema](/docs/dev/objectsschema.md).