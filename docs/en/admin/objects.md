...
What objects and states actually are can be found under [Objects](/docs/basics/objects.md) and [States](/docs/basics/states.md) .
...
Rooms and trades are created in the [Categories](/docs/admin/enums.md) tab.
...
Without expert mode, custom objects can only be used under`0_userdata.0` and`alias.0` They will be created. This is not a limitation, but rather the correct place: Objects in the adapters' namespaces will be overwritten the next time they are started.
...
4. The **gear icon** opens the _custom settings_ . Here you define whether and how the data point is recorded. It only appears if at least one instance of history, InfluxDB, or SQL is installed.