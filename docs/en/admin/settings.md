...
The system settings apply to the entire installation. They can be accessed via the following point: **system** They are opened at the very bottom of the menu bar and are divided into several tabs.
...
| **Standard protocol level**               | The log level, which **new** New instances will be created. Existing ones will remain unchanged.                                                                                               |
...
- **stable**The tested versions. This is the right choice for a system that needs to run.
- **beta** (also _Latest_): the latest versions, not yet fully tested.

The checkmark in the column **Active** determines which repository is used. Is _beta_ When active, a corresponding warning appears in the Adapter tab.

If there are additional entries from an old installation, they should be removed. They are no longer maintained.

Over **Only allow the automatic execution of the following upgrades.** This determines whether the ioBroker adapter is allowed to update itself and up to which version level.
...
Ex-works `defaultPrivate` and `defaultPublic` registered. These standard certificates are the same in every installation and therefore **not sure**They only allow an encrypted connection, without any possibility of verification. For external access, separate certificates are required, either self-generated, purchased, or via Let's Encrypt.

A certificate can be stored as a file or specified as an absolute path, for example `/opt/certs/cert.pem`.

!> New certificates first with the **web**Try the adapter, not with the administrator account. Otherwise, you might lock yourself out.

### Rights to the certificate files

If a path is specified, the user must `iobroker` be allowed to read the file:
`644` for the file, `755` for the parent directories. If the necessary permissions are missing, the log will report something like:
...
The process: ioBroker creates an account with the email address entered here and starts a small web server the first time the address is accessed. **Port 80**Let's Encrypt stores a verification string there and reads it from \[location].
`http://<adresse>/.well-known/acme-challenge/` It then sends the certificate. It is valid for approximately 90 days and is then automatically renewed.

Port 80 must be free and accessible from the outside. If another service is using it, the check will fail.

If that doesn't work or you don't want to open a port: For remote access, the
[IoT adapter](/adapters/iot)
The simpler way, because it doesn't require any open ports.

## Standard ACL

Determines which rights **newly created** Objects, states, and files are separated by owner, group, and everyone else.

<img src="media/admin_einstellungen_acl.png" alt="Der Reiter Standard ACL" width="900" />

This page does not change the permissions of existing objects. Users and groups themselves are managed in the tab
[user](/docs/admin/users.md) managed.
...
- **Licenses**License keys for paid adapters are stored here. Information on which adapters require a license and what the license is tied to can be found under \[link/section name]. [Adapter licenses](/docs/licenses/adapter.md).
- **Access data**: central login credentials that multiple adapters can access, instead of storing them individually.