...
flow to ioBroker to IFTTT by building an applet sending data from ioBroker via IFTTT to Telegram. Certainly it
is possible to use an adapter and thus more direct, but it does give a good example of how it works.
...
**4. Now we name our event, choose here the same name as you will later in Blocky. ioBroker.iot sends only events with the name "state" and this you should enter as "Event name".**
...
**8. Now we set up where the message should go and how it looks. Please fill the fields as shown in the picture.
...
below tells you your key, please copy it as we need it for ioBroker in the next step**
...
tab and paste the key in the field labeled "IFTTT key". (don't even try, the key here is random and just for show)**
...
**If you prefer to write JavaScript directly instead of using blocky, the function to use looks like this:**
...
```
**Also, it's possible to set the variable `iot.0.service.ifttt` with some value, and it will be sent, too.**

**13. As a simple example, here is a Blocky script which grabs the state of a HomeMatic door contact and sends it to
...
**8. Now we need the API url (requires you having an ioBroker cloud account, either free or pro) from the 
iot adapter page, it's under the "IFTTT and Services" tab. (oh and don't try, the key in the picture is fake)**

In case you use custom services, you need to set "White list for services" either to "*" to allow
