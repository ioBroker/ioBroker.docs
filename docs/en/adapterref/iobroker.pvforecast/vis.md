...
- JSON Graph / Chart (included in ``vis-materialdesign``)

## JSON Table

Choose the ``JSONTable`` object ID as data source. e.g. ``pvforecast.0.summary.JSONTable``.

## JSON Graph / Chart

Adapter [ioBroker.vis-materialdesign](https://github.com/Scrounger/ioBroker.vis-materialdesign) is required to use the JSONGraph widget

```
iobroker add vis-materialdesign
```

Choose ``JSONGraph`` object ID as data source. e.g. ``pvforecast.0.summary.JSONGraph``.

You can configure some settings (like label rotation and size) in the adapter settings.

## Example

![VIS example](./img/vis-example.png)
