...
ioBroker leads **two** Databases: one for the **objects**, that is, the description of all devices, channels, and data points, and one for the
**Conditions**, that is, their current values. Both are managed by the js-controller, and both always only hold the **current** Stand.

!> This has nothing to do with the adapters that the **Course**
Take notes. `history`, `influxdb` and `sql` They store their data separately and are located under [Data recording](/docs/config/history.md) This has been discussed. Setting up Redis doesn't make your measurement data available faster, and setting up InfluxDB doesn't reduce the load on the JS controller. This confusion is the most common misconception here.
...
This page is about the **first** Column, more precisely, the possibility of keeping it in Redis instead of in files.

The two databases will **separated** discontinued. The usual way is to only the **Conditions** to Redis and place the objects in `jsonl` to allow it. This brings the greatest benefit, because states change constantly and objects almost never, and it uses the least amount of memory. `iobroker status`
shows what is currently in use.

## What Redis is

Redis is an open-source in-memory database. Further information can be found at \[link to Redis documentation]. <https://redis.io/>
...
**RDB** This method is enabled by default; it saves the entire content to an RDB file. The saving interval can be configured and should be adjusted to your specific needs! Configuring this requires a balance between data security (how much data can you tolerate losing in a crash) and the write load on the storage medium, as the entire content is always written (if objects are also stored in Redis, this can amount to several hundred MB!).

**AOF** However, it ensures that the data is completely up-to-date. To achieve this, an AOF file is continuously written, to which all changes are appended. This file is then consolidated at regular intervals, thus reducing its size. The exact final write load, and whether this approach is suitable for SD cards, depends on the type of data being stored. If objects and files are also located in Redis, appending and infrequent consolidation is potentially much more efficient than regularly writing large amounts of data. As mentioned above, this requires more RAM. If this RAM is temporarily unavailable, everything will continue to run smoothly, depending on the settings. However, a data backup will not be created! Any such messages will only appear in the log file.

More details on persistence can be found at <https://redis.io/topics/persistence>

**Redis Slaves**Using a second Redis server is another way to ensure you always have up-to-date data as a backup. If the computer running the master Redis server fails, the data is still available on the slave server in near real-time. You can use this to create a data dump to reinstall the master server, or, as a quick fix, you can make the slave server the master and change the database IPs in ioBroker, bringing you back online almost instantly. You can find more detailed information about this in the \[link to documentation]. [forum](https://forum.iobroker.net/topic/26327/redis-in-iobroker-%C3%BCberblick) or under <https://raw.githubusercontent.com/antirez/redis/5.0/redis.conf>
...
**Danger**There are no official Redis builds for Windows.
...
If other hosts (slaves, etc.) are also to access this Redis server, this must be allowed. To do this, /etc/redis/redis.conf must be edited, and the line... **bind 127.0.0.1** to **bind 0.0.0.0** will be changed and directly below the **protected\_mode** on **no** be set.

Then it starts `sudo systemctl restart redis-server` restart the server with the updated configuration.

See for further details. [Multihost](/docs/config/multihost.md)
...
For "Objects", confirm the current settings ("file" as type, IP, port 9001) and for "States" now set the type to "redis", the IP address of the Redis host server (or 127.0.01 if on the same host) and 6379 as port. To avoid losing all state data, it is recommended to migrate the data, which will be prompted in the next configuration questions. After the migration, ioBroker can be used with **iobroker start** It will need to be restarted. If slave systems are also used, the same settings must be applied to all of them. **iobroker setup custom** These measures will be taken. However, the question of migration must be answered in the negative!
