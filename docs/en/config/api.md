...
| **ws**         | WebSocket               | on the `web`-Adapter | Applications that make changes **immediately** were supposed to be informed without asking. |

The rule of thumb: **query** via HTTP **listen** via WebSocket. Anyone who accesses the same address every ten seconds to see if anything has changed actually needs a WebSocket.

## simple-api

The easiest access. After creating an instance, it delivers
`http://<adresse>:8087/help` The list of possible calls. The most important ones:
...
?> `getPlainValue` It returns only the value, without quotation marks or parentheses. This is exactly what a shell script needs to pass the value on.

The history can also be queried if a
[recording](/docs/config/history.md) running:
...
Conditions can also be described **subscribe**, and about
`http://<adresse>:8093/oauth/token` Is there a login option using a token instead of a username and password in the address?
...
Since version 4, the adapter works with pure WebSockets; socket.io is only emulated. An example is included in the
...
1. **Enable registration** at the relevant instance. See
   [authentication](/docs/config/login.md).
2. **Create your own user account** with only the rights that the calling program needs, not `admin`See
   [Access management](/docs/config/userrights.md).
3. **Encrypt**Otherwise, the login credentials will be transmitted in plain text over the line. See [Encryption](/docs/config/encryption.md).
...
Making it accessible remotely is possible without an open port via the
[IoT adapter](/docs/cloud/iot.md). Port forwarding to one of these interfaces is the worst of the possible solutions.

## The reverse way

It should not be queried, but **reported** If a foreign system sends something to ioBroker, other methods are possible:

- **MQTT** via the adapter of the same name, if the other system supports it.
- **URL services** via the cloud, if the message comes from the internet and no port should be opened. See
  [Services](/docs/cloud/services.md).
