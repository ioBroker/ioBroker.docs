...
它面向那些想要查看基本在线/电池/状态信息之外更多内容的用户：RTK 和基站状态、固件和 OTA 详细信息、网络和 SIM 卡信息、GPS 和姿态数据、地图生命周期时间戳、割草机错误详细信息、耗材寿命、雨量设置、区域元数据和可写割草控制等信息都以 ioBroker 状态的形式公开。

此适配器可在 ioBroker `latest` 存储库中找到。请在 [ioBroker 论坛帖子](https://forum.iobroker.net/topic/84983) 中报告反馈和测试结果。
...
- 只读 PNG 地图图像状态，包括原生地图、RTK 割草区域掩膜和历史割草路径
...
| 获取地图（CPU 使用率高） | 下载并渲染原生地图、RTK 掩码和历史割草路径 | `false` |
| 生成包含路径的地图（CPU 使用率更高）| 下载并渲染地图图像中的历史割草路径。需要获取地图数据。| `false` |
| 错误描述语言 | 用于 Anthbot 云错误描述的语言 | `English` |
| 错误描述语言 | Anthbot 云错误描述所用的语言 | `英语` |

当任一地图选项被禁用时，适配器将停止请求或渲染该地图数据，并且不会改变现有地图状态值。
...
| `<serial>.metrics.map.totalArea` | 编号 | `m2` | 地图总面积 |
...
| 状态 | 类型 | 描述 |
| --- | --- | --- |
| `<serial>.map.image` | 字符串 | 原生导航地图（PNG 数据 URI） |
| `<serial>.map.imageWithMowedPath` | 字符串 | 包含已下载的历史割草路径的本地导航地图，数据 URI 为 PNG 格式 |
| `<serial>.map.mowedPath` | 字符串 | 包含用于 `map.imageWithMowedPath` 的确切历史路径点的 JSON 数组 |
| `<serial>.map.mowedPath` | 字符串 | 包含用于 `map.imageWithMowedPath` 的精确历史路径点的 JSON 数组 |

这三个图像状态均为只读，并使用 `media.image` 角色。`map.mowedPath` 状态也为只读，并使用 `json` 角色。适配器下载 Anthbot 的 `multi_maps` 地图文件，提取 `maps/remote_map_navi.map`，并使用与应用程序兼容的浅色调色板渲染原生栅格。`map.image` 仅包含地图；`map.imageWithRtkMask` 添加完整的 `maps/rtk_mask_map` 割草区域栅格；`map.imageWithMowedPath` 请求 `req_history_mapping_path`，下载 `path_<SN>.txt`，以蓝色渲染历史路径，并将当前割草机姿态添加为黄色机器人图标，并在其下方添加原生充电器标记。`map.mowedPath` 包含与该 PNG 文件相同的 JSON 路径点。其 `x` 和 `y` 值使用原生历史路径厘米坐标；将其除以 `100` 即可转换为姿态状态使用的本地地图米坐标。捆绑的应用程序资源以割草机前部朝下为方向，并使用 `location.pose.yaw - 90°` 进行旋转（例如，大约 `-16°` 的实时偏航角指向右前）；生成的备用图像使用相同的方向。充电器标记和只读的 `location.charger.x`/`location.charger.y` 状态从 `maps/remote_map.json` 中的 `charger_point` 读取；状态坐标以米为单位显示。两个叠加图像均以红色渲染配置的禁区。历史图像永远不会回退到短暂的实时 `curpath`。当本地地图标识、时间戳、历史路径、割草机姿态或充电点发生变化时，图像和路径状态会刷新。如果地图数据缺失或无效，则状态会为空，而适配器会继续轮询。

这两个地图设置有意控制不同的工作：

- `fetchMap = false`：不请求地图存档、栅格、PNG 或历史路径。地图和充电器坐标状态的现有值保持不变，因此先前生成的地图和充电器位置仍然可以显示，但它们不会更新。
- `fetchMap = true` 且 `generateMapWithPaths = false`：将更新原生地图和 RTK 掩码状态。不会请求历史路径，`map.imageWithMowedPath` 和 `map.mowedPath` 保持不变。对于 CPU 占用较低的地图视图，请使用 `map.image` 或 `map.imageWithRtkMask`。
- 两个设置都为“true”：生成所有三个图像状态和 `map.mowedPath` 状态；`map.imageWithMowedPath` 包含历史路径和当前机器人图标。

#### 可视化：带有集成机器人图标的地图
当两个地图设置都启用时，使用此直接路径。将 VIS 图像控件绑定到 `<serial>.map.imageWithMowedPath`；适配器已在当前位姿绘制历史路径和机器人图标。无需第二个 VIS 图标控件。保持图像容器的 PNG 原始宽高比 (`404:488`)。

#### VIS：带有独立叠加图标的地图
当 `fetchMap = true` 和 `generateMapWithPaths = false` 时，使用此低 CPU 路径。将图像控件绑定到 `<serial>.map.image` 或 `<serial>.map.imageWithRtkMask`，然后将透明图标控件绝对放置在其上方。根据 `<serial>.location.pose.x` 和 `<serial>.location.pose.y` 绑定或计算其位置。这些状态单位为米；地图元数据提供原点和分辨率。将两个控件保持在同一个相对定位的容器中，并使用相同的宽高比 (`404:488`)，否则 `object-fit: contain` 可能会引入黑边并偏移叠加图标。如果 `fetchMap = false`，地图图像可能仍显示旧状态，但适配器不会刷新地图或其单独的叠加位置。

部署姿态状态米转换后，`location.pose.x` 和 `location.pose.y` 为米。使用 `maps/remote_map.json` 中的地图头值将它们转换为地图像素：
...
对于当前快照，`pose.x = 0.094` 和 `pose.y = 0.356` 将图标放置在大约像素 `(309, 289)` 或 `left: 76.5%` 和 `top: 59.2%` 处。从 `<serial>.location.pose.yaw` 定义 `poseYaw`；对于前向下图标，应用 `poseYaw - 90`，使围绕 `-16` 的实时偏航角指向右侧。适配器目前不将地图宽度、高度、分辨率或原点作为状态发布，因此割草机创建新地图时必须刷新这些值；请勿将地图数组像素与割草机的米坐标混合使用。

#### 偏航角：起源和计算
`<serial>.location.pose.yaw` 是 Anthbot 报告的割草机航向，即 `pose.yaw`。它以度为单位，并非从毫米转换而来。适配器仅将 `pose.x` 和 `pose.y` 从毫米转换为米；偏航角保持不变。X/Y 坐标描述割草机的位置，因此偏航角不能仅根据当前位置计算。

对于源图像中割草机前端朝下的独立 VIS 图标，使用固定的 `90°` 偏移量计算图像旋转：
...
`-90°` 偏移量用于将 Anthbot 的航向约定与前向下的资源对齐。集成地图图标及其生成的备用图标也使用相同的公式。例如：`yaw = 90°` 会将图像旋转至 `0°`（前向下），而实时值 `yaw = -16°` 会将图像旋转至 `-106°`（等效于 `254°`），并将前端指向右侧。如果无法获取偏航角，则保持资源默认的前向下方向。

#### 机器人图标来源
经检查的 Anthbot 应用包包含本地地图标记资源，包括 Genie `pic_device_map` 资源、特定型号的 S2/S3/M9Pro 变体以及 `view_map_battery_position` 充电器标记。这些是应用选择的打包 UI 文件；云端/API 有效负载和下载的地图存档不提供可重用的图标 URL 或图标状态。适配器会为已知型号打包匹配的应用派生标记资源，如果资源缺失、无法读取或型号未知，则使用其自身包含的机器人标记。适配器不依赖于已安装的应用。

＃＃＃ 地点
| 状态 | 类型 | 描述 |
| --- | --- | --- |
| `<serial>.location.gps.latitude` | 数字 | 来自防丢定位数据的 GPS 纬度 |
| `<serial>.location.pose.x` | 数字 | 本地割草机位置 X（单位：米） |
| `<serial>.location.pose.y` | 数字 | 本地割草机 Y 轴位置（单位：米） |
| `<serial>.location.pose.yaw` | 数字 | 本地割草机姿态偏航角 |
| `<serial>.location.pose.type` | 字符串 | 报告的姿态类型 |
| `<serial>.location.charger.x` | 数字 | 充电器 X 坐标（以米为单位），数据来自原生地图元数据 |
| `<serial>.location.charger.y` | 数字 | 充电器 Y 坐标（以米为单位），数据来自原生地图元数据 |
| `<serial>.location.charger.y` | 数字 | 来自原生地图元数据的充电器 Y 坐标（以米为单位） |
