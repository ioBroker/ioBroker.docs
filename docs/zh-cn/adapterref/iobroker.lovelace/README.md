...
***只会检测到定义了 `function` 和 `room` 类别的对象和通道***

您可以定义友好名称，这将在实体中使用。

＃＃＃ 手动的
可以在对象树中手动定义对象，例如 sql 或 histroy。必须提供实体的类型和可选的对象名称。
使用此方法只能创建简单的实体，如 input_number、input_text 或 input_boolean。它可能没有超过一种状态或属性。

## 面板
### 报警面板
ioBroker 还不支持这样的设备，但是可以模拟。如果您创建这样的脚本：
...
### 数字输入
如果选择了自定义对话框中的 input_number 实体类型，则可以手动完成此操作。
此类型需要 `min` 和 `max` 值 `common` 和可选的 `step` 可以添加。
...
使用 AccuWeather 驱动程序 v1.1.0 测试 https://github.com/iobroker-community-adapters/ioBroker.accuweather。
...
### 图片实体
您可以为它使用静态图片或使用任何提供 URL 作为状态的状态。
例如。：
...
要显示它，您可以在对话框中再次禁用它，或者只使用 `?toolbar=true` 参数调用 URL。

### 降价
您可以像在 [iobroker.vis](https://github.com/ioBroker/ioBroker.vis#bindings-of-objects) 中那样在降价中使用绑定。
...
自定义卡片通常作为源存储在 github 上，必须在使用前编译。
您应该检查 github 上的 `Releases` 菜单并尝试在那里找到编译的文件。
就像这个：[https://github.com/kalkih/mini-graph-card/releases](https://github.com/kalkih/mini-graph-card/releases)（查找文件 `mini-graph-card-bundle.js`）
...
在 lovelace 配置文件中。在 lovelace [这里](https://www.home-assistant.io/lovelace/views/#background) 中阅读更多关于背景的信息。
...
＃＃ 语音控制
来自 Web 界面的所有命令都将写入 lovelace.X.conversation 状态，带有 `ack=false`。
您可以编写一个脚本来响应请求并回答：
...
如果这没有帮助，您可以在 ioBroker 的原始编辑器中打开对象 lovelace.*.configuration 并在那里查看。
您还可以从备份中恢复该对象。它包含可视化的完整配置。

## Lovelace 的原始来源
使用过的资源在这里 https://github.com/GermanBluefox/home-assistant-polymer 。

＃＃ 去做
必须从当前用户而不是 default_user 获取安全性

＃＃ 发展
＃＃＃ 版本
使用的 home-assistant-frontend@20220707.0 版本
...
- `src/data/weather.ts` - 添加支持以显示来自 url 的天气图标。
...
- `src/dialogs/more-info/controls/more-info-weather.ts` - 添加支持以显示来自 url 的天气图标。
- `src/entrypoints/core.ts` - 修改认证过程
- `src/layouts/home-assistant-main.ts` - 删除应用侧边栏
- `src/panels/lovelace/cards/hui-weather-forecast-card.ts` - 添加支持以显示来自 url 的天气图标。
- `src/panels/lovelace/entity-rows/hui-weather-entity-row.ts` - 添加支持以通过 auth 从 url 显示天气图标。
...
- `package.json` - 删除 husky 提交挂钩

+ 在 `./build` 文件夹中检出修改后的版本。然后。

1. 进入./build 目录。
2. `git clone https://github.com/GermanBluefox/home-assistant-polymer.git` 它是 https://github.com/home-assistant/frontend.git 的一个分支，但是修改了一些东西（请参阅前面的文件列表）。
3.`cd home-assistant-polymer`
4.`git结账大师`
5.`纱线安装`
6. `gulp build-app` 用于发布或 `gulp develop-iob` 用于调试版本。要在更改后构建 Web，您可以调用 `webpack-dev-app` 以加快构建速度，但无论如何您都需要在版本准备就绪后调用 `build-app`。
7. 将所有文件从 `./build/home-assistant-polymer/hass_frontend` 复制到这个 repo 中的 `./hass_frontend`
8. 多次运行 `gulp rename` 任务（直到没有变化发生）。
9. 更新自述文件中的版本以及 server.js 中的版本常量。
