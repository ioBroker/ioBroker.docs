...
![新PM](https://nodei.co/npm/iobroker.vis.png?downloads=true)
...
只需写入属性```{object.id}```，它将被绑定（不在编辑模式下）到这个对象的值。
...
- `\*` - 相乘。参数必须放在括号中，例如“*(4)”。在此示例中，我们将 value 乘以 4。
- `\+` - 添加。参数必须放在括号中，例如“+(4.5)”。在此示例中，我们将值添加到 4.5。
- `\-` - 减法。参数必须放在括号中，例如“-(-674.5)”。在此示例中，我们从值 -674.5 中减去。
- `/` - 除法。参数必须放在括号中，例如“/(0.5)”。在此示例中，我们将值除以 0.5。
- `%` - 模数。参数必须放在括号中，例如“%(5)”。在这个示例中，我们取模 5。
- `round` - 对值进行四舍五入。
- `round(N)` - 在点后用 N 位四舍五入值，例如34.678;round(1) => 34.7
- `hex` - 将值转换为十六进制值。所有字母都小写。
- `hex2` - 将值转换为十六进制值。所有字母都小写。如果值小于 16，则将添加前导零。
- `HEX` - 与十六进制相同，但大写。
- `HEX2` - 与 hex2 相同，但大写。
- `date` - 根据给定格式格式化日期。格式与 [iobroker.javascript] 中的相同（https://github.com/iobroker/iobroker.javascript/blob/master/README.md#formatdate）
...
- `date(format)` - 将值格式化为日期。格式如：“YYYY-MM-DD hh:mm:ss.sss”
...
显示对象的时间戳写`.ts`或`.lc`（用于最后更改）在对象ID的末尾，例如：
...
您可以使用 *any* javascript 函数。参数必须用':'定义，否则将被解释为公式。

注意类型。它们都定义为字符串。可以肯定的是，该值将被视为使用 parseFloat 函数的数字。
...
* `view` - 实际视图的名称
* `wname` - 小部件名称
* `widget` - 是一个包含所有小部件数据的对象。只能在 JS 部分使用，例如 `{a:a;widget.data.name}`
* `wid` - 实际小部件的名称
* `language` - 可以是 `de`、`en` 或 `ru`。
* `instance` - 浏览器实例
* `login` - 是否需要登录（例如显示/隐藏注销按钮）
* `local_*` - 如果状态名称从`local_`开始，它不会报告给ioBroker，但会更新所有小部件，这取决于这个状态。 （当前浏览器会话的局部变量）
...
## 过滤器
要在一个视图上显示全部数量的小部件，您可以使用过滤器来减少同时显示在视图上的小部件数量。
...
- `control.data` - 命令参数。具体命令说明见。
- `control.command` - 命令名称。写这个变量触发命令。这意味着在写入命令之前，“实例”和“数据”必须准备好数据。

命令：

* `alert` - 在 vis 中显示警报窗口。 “control.data”具有以下格式“消息；标题；jquery-icon”。标题和 jquery-icon 是可选的。图标名称可以在 [这里](http://jqueryui.com/themeroller/) 找到。要显示图标“ui-icon-info”，请编写 ```Message;;info```。
* `changeView` - 切换到想要的视图。 “control.data”必须有视图的名称。您也可以将项目名称指定为“项目/视图”。默认项目是“主要”。
...
* `dialog` - 显示对话窗口。对话框必须存在于视图中。之一：
    ...
    - `container - ext cmd - 在 jqui Dialog 中查看`,
    -`容器-图标-在jqui对话框中查看`，
    -`容器-按钮-jqui对话框中的视图`。
...
- `control.data`: `project/view` 形式的项目和视图名称，例如`main/view`（和 `ack=true`）
...
＃＃ 默认视图
您可以为每个视图定义所需的分辨率（菜单=>工具=>分辨率）。这只是编辑模式下的视觉边框，用于向您显示某些特定设备上的屏幕尺寸。在实时模式下，它将不可见，并且所有超出边界的小部件都将可见。

此外，您可以定义是否必须将此视图用作此分辨率的默认值。

因此，每次调用 `index.html`（没有 `#viewName`）时，都会打开最适合此分辨率的视图。
如果只有一个视图具有 *"Default"* 标志，则此视图将独立于屏幕分辨率或方向打开。

例如。您可以创建两个视图“Landscape-Mobile”和“Portrait-Mobile”，当您更改方向或屏幕尺寸时，这两个视图将自动切换。

有一个帮助小部件“基本 - 屏幕分辨率”显示实际屏幕分辨率和最适合此分辨率的默认视图。

## 设置
### 如果睡眠时间超过则重新加载
有一条规则，在断开一段时间后，整个 VIS 页面将重新加载以同步项目。
您可以在菜单“设置...”中对其进行配置。如果您将时间间隔设置为“从不”，则该页面将永远不会重新加载。

### 重新连接间隔
设置断开连接尝试之间的间隔。如果设置为 2 秒，它将每 2 秒尝试建立连接。
