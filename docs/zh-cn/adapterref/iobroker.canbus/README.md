...
![新平台](https://nodei.co/npm/iobroker.canbus.png?downloads=true)
...
* 为每条消息配置解析器，以便从原始消息缓冲区读取数据或将数据写入原始消息缓冲区
* 数字类型
* 布尔值包括位掩码支持
* 不同字符编码的字符串
* 自定义脚本来读取/写入原始数据缓冲区
...
* 可选支持固定数据长度（DLC）
* 可选支持 RTR 标志
* 可选的原始状态包含原始 CAN 消息对象
* 可选为每个解析器在给定间隔内自动设置某个值（对于轮询数据很有用）

＃＃ 要求
* Linux 操作系统（因为使用了 socketcan 库）
* 内核支持的 CAN 硬件，创建类似“can0”的接口
* 有关 CAN 总线上发送的消息的一些知识

## 解析器
使用解析器，您可以从 CAN 消息缓冲区读取数据或向其写入数据。

有针对以下数据类型的预定义解析器。
此外，您还可以编写自己的脚本来使用*自定义解析器*读取/写入值。
...
这些脚本应该是纯 javascript，并且将在有限的范围内运行。

在脚本中您可以使用以下功能：

* 全局变量`undefined`、`NaN`、`isNaN`、`Infinity`、`isFinite`、`atob`、`btoa`、

`encodeURI`、`encodeURIComponent`、`decodeURI`、`decodeURIComponent`、`parseFloat`、`parseInt`、`JSON`、`Number`、§§ SSSSS_15§§、`Array`、 `BigInt`、`Blob`、`Boolean`、`Date`、`Map`、`Math`、`Object`、`RegExp`、§§ SSSSS_25§§， `Intl`， `Buffer`、`Promise`、`setTimeout`、`clearTimeout`

* `async`/`await`
* 适配器日志函数 `log.warn('something')`、`log.info('something')`、`log.debug('something')`
* `getStateAsync('id')`、`getObjectAsync('id')`、`setStateAsync('id', 'value', ack)`，其中 `id` 是当前适配器实例下状态/对象的部分 ID
...
`buffer` 始终以 Node.js 缓冲区的形式包含当前 CAN 消息内容。
...
在自定义读取脚本的开头，`buffer` 将是已接收/当前 CAN 消息数据的副本（如在 `.json` 状态中）。
`value` 将是 `undefined` 并且应由脚本设置。
...
因为只有当前三个字节匹配时才会设置`value`，所有其他数据将被忽略并且不会为状态设置新值。

#### 自定义写入脚本
在写入脚本中，您必须修改（或替换）`buffer` 变量。

在自定义写入脚本的开头，`buffer` 将是当前 CAN 消息数据的副本（如在 `.json` 状态中）。
`value` 设置为应写入 `buffer` 的状态的值。

自定义写入脚本末尾的`buffer`变量的内容将用作 CAN 消息的新数据。

您还可以通过在自定义写入脚本中调用`return false;`来取消写入。
这允许您在未满足某些条件时阻止写入。
