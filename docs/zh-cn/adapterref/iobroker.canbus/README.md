...
* 可选支持 RTR 标志
* 可选原始状态包含原始 CAN 消息对象
* 可选地为每个解析器在给定的时间间隔内自动设置某个值（对于轮询数据很有用）
...
如果 `value` 等于 `undefined`，则它将被忽略。您可以使用此功能按数据部分过滤自定义读取脚本中的消息。
...
仅当前三个字节匹配时才会设置`value`的原因，所有其他数据将被忽略并且不会为状态设置新值。
...
它们保存消息数据的字符串化 JSON 数据，可用于处理每个接收或发送的消息，而与配置的消息无关。
通过将 JSON 数据写入`raw.send`状态，您可以发送包含任何所需数据的 CAN 消息。
