...
editLink: https://github.com/ioBroker/ioBroker.docs/edit/master/docs/zh-cn/adapterref/iobroker.pvforecast/vis.md
title: ioBroker.pvforecast - 可见
...
- JSON 表（直接工作）
- JSON图形/图表（包含在“vis-materialdesign”中）

## JSON 表
选择一个``JSONTable`` Object-ID als Datenquelle. Beispielsweise ``pvforecast.0.summary.JSONTable``。

## JSON图形/图表
适配器 [ioBroker.vis-materialdesign](https://github.com/Scrounger/ioBroker.vis-materialdesign) 是使用小部件所必需的。

```
iobroker add vis-materialdesign
```

选择一个``JSONGraph`` Object-ID als Datenquelle. Beispielsweise ``pvforecast.0.summary.JSONGraph``。

可以在适配器设置中指定图形的某些属性（例如标签的旋转和字体大小）。

＃＃ 例子
![可视化示例](../../../de/adapterref/iobroker.pvforecast/./img/vis-example.png)
