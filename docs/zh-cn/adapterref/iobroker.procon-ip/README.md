...
ioBroker 适配器，适用于 Pool Digital ProCon.IP 泳池控制器。
它旨在与您的 ioBroker 家庭自动化系统集成，例如
构建涉及其他设备的逻辑或与您常用的语音助手配对：
...
## ProCon.IP 池控制器是什么？
ProCon.IP 泳池控制器是一款适用于家庭泳池的低成本网络连接控制单元。它通过软件控制继电器，可以控制多个水泵（用于泳池过滤器和不同的投加装置），控制方式既可以简单地按时间表进行，也可以根据其众多测量输入通道（例如输入/输出流量传感器、Dallas 单线温度计、氧化还原和 pH 电极）的读数/数值进行控制。此外，它还支持按需切换继电器，这使得它们也适用于开关灯光（或其他任何您需要的功能）。
并非所有功能都可以通过 API 访问。实际上，有一个已记录的 API 用于读取（轮询）CSV 格式的值（`/GetState.csv`）。在我的记忆中，还有另一个 API 可以使用定时器控制继电器的开关和开启。但我再也找不到第二个了。所以，虽然不够美观，但功能强大：ProCon.IP 有两个原生 Web 界面，可以对其进行分析，从而对特定功能（例如切换继电器）进行某种形式的逆向工程。

欲了解更多信息，请参阅以下链接（抱歉，目前只有德语版本；还没有找到英文文档/信息）：
...
**需要说明的是：我与泳池控制单元的开发、销售、营销或支持没有任何关系。我只是开发了一个解决方案，将其与 ioBroker 集成，以便让我父母的家变得更智能。**

## 适配器的详细信息
该适配器使用 ProCon.IP 的 `/GetState.csv` API 来轮询其值，并使用另一个未记录的 API，该 API 使用按位命令来切换继电器。ProCon.IP 的原始 Web 界面也使用第二个 API。因此，未来可能会有固件升级，从而破坏与此适配器的兼容性，或至少破坏其切换继电器的功能。

＃＃＃ 兼容性
目前，该适配器已结合 ProCon.IP 固件版本 1.7.6.a 进行测试和开发。但它应该可以兼容任何先前更新或即将发布的固件版本。
