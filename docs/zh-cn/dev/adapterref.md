...
* `system.adapter.adaptername`：适配器的描述（如名称、版本号等）
...
?> 注：lib/utils.js 是所有适配器通用的文件，通过它可以确定js-controller 的位置和iobroker.js-controller/lib/adapter.js 对应的路径。大多数当前的 utils.js 都可以在这里下载。不要更改此文件！
...
如果一切正常并且用户的积极反馈进来，适配器可以在 npm 上发布。
