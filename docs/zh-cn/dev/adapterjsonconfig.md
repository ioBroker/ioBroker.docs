...
| `doNotApplyWithError` | 如果 JSON 或 JSON5 中有错误，则不允许保存该值（从 7.5.3 开始）|
...
| `bundlerType` | 如果模块用 TypeScript 编写，则将其设置为 `module`。从 Admin 7.5.x 开始 |
| `bundlerType` | 如果模块用 TypeScript 编写，则将其设置为 `module`。从 Admin 7.5.x 开始 |
...
如果提供了`sendTo`，实例将收到一条消息（`common.messagebox` 在`io-package.json` 中必须为真），其中包含命令`tab`，或存储在`sendTo` 中的值（如果是字符串）。
实例必须使用如下结构进行回答：
