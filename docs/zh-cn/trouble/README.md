...
此外，请检查此故障排除页面和常见问题解答 --LINK-- 在论坛请求之前，该主题是否已包含在那里。如果您已经尝试过这些文档中的操作，请将它们同时写下来。

请检查日志以查看相关适配器是否具有在出现问题时有用的条目。始终将日志和脚本或类似内容放入剧透中。
...
* **傻** 最详细的日志级别，来自 js 控制器的消息也被记录，仅在明确请求时使用。

实例的日志级别可以在 Admin WebUI 中设置。为此，请在“实例”下激活专家模式，并在实例的同名列中设置日志级别。
更改日志级别后，实例会自动重启。
