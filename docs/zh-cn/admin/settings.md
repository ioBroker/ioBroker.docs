...
您可以在不同的系统语言之间进行选择。可能并非所有语言都得到完全支持。
...
默认情况下，数据会记录到此实例中，并用于 flot 和 rickshaw 的图表中。

如果只安装了一个历史适配器（SQL/History/InfluxDB），就会使用，如果有多个，可以选择一个。

**活动存储库**

在这里，通过下拉菜单选择要安装适配器版本的所需存储库。 “存储库”下列出的存储库在下拉菜单中可用

## 存储库
![存放处](../../de/admin/media/ADMIN_Settings_repos.png)

ioBroker 可以从不同的来源获取适配器列表。在安装过程中输入以下源：

* 默认（=stable）：http://download.iobroker.net/sources-dist.json
...
--- 标题：系统设置 lastChanged：03/27/2019 ---
...
默认情况下，数据会记录到此实例中，并在 flot 和 rickshaw 图表中使用。
...
颁发证书是一个复杂的过程，但按照下面的说明，获得免费证书应该很容易。
...
Let's Encrypt 服务器发送回一个质询短语以响应请求，并在一段时间后尝试在地址“http://yourdomain:80/.well-known/acme-challenge/”处读取这个质询短语。
