...
### 3-仅显示已安装的适配器
选择此图标时，仅显示具有已安装实例的适配器（切换功能）

### 4-显示带有更新的适配器
选择此图标时，仅显示有可用更新的适配器（切换功能）。可更新适配器的磁贴具有绿色标题。如果没有适配器的更新，则会显示一条相应的消息。
...
### 5-从其自己的URL安装适配器
！> **注意：使用此选项可能会导致ioBroker安装出现问题** GitHub上的适配器可能仍在开发中，因此无法正常工作！这些只能在生产系统中谨慎使用。建议等待稳定版本！
