...
### 1 - 刷新视图
如果刚刚创建的实例不可见，单击此图标有助于使页面状态保持最新。
...
## 实例窗口中的更多信息
第一个数字表示到目前为止实例使用的内存和剩余的可用内存（以 MB 为单位）。后面的空闲内存在%。 ioBroker 服务器的名称和正在运行的进程数在方括号中。
...
### 13 - RAM 限制
您可以在此处指定应为实例提供多少主内存作为预防措施。此内存量将不再可用于其他任务，不应选择太高，尤其是在内存很少的系统中。如果实例临时需要更多内存，当然会由系统分配，然后立即释放给系统。在实例需要的内存多于预留的时间期间，所需内存以红色显示。
